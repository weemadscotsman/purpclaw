...
async function runIdleCycle(trigger = 'timer') {
  ...
  announce.idle.cycle('exported.started', {"source":"autodream"});
  ...
  announce.idle.cycle('gate.started', {"source":"gates"});
  ...
  try {
    ...
    // NOTE 2026-06-21: path corrected from /autodream/consolidate → /autodream/dream.
    // Spine was refactored; old path now 404s. Verified via direct probe.
    const http = require('http');
    const dreamResult = await new Promise((resolve) => {
      const req = http.request({
        hostname: '127.0.0.1', port: 7880, path: '/autodream/dream',
  ...
  try {
    ...
    const http = require('http');
    // NOTE 2026-06-21: path corrected from /diagnostics/run → /diagnostics/diagnose.
    // Spine endpoint renamed during cognitive cluster consolidation. Verified.
    // Diagnose is a POST route (line 261 of cognitive_spine.py), so method must be POST.
    const diagResult = await new Promise((resolve) => {
      const req = http.request({ hostname: '127.0.0.1', port: 7880, path: '/diagnostics/diagnose', method: 'POST', timeout: 15000 }, (res) => {
        let d = ''; res.on('data', c => d += c);
        res.on('end', () => { try { resolve(JSON.parse(d)); } catch { resolve({ raw: d }); } });
      });
      req.on('error', () => resolve(null));
  ...
  if (results.phases.trainingCheck && results.phases.trainingCheck.ready && AUTO_TRAIN) {
    try {
      ...
      if (exported.ready && exported.path) {
        ...
        // ── Training Forge spine job: dataset_locked → training → registered/failed.
        // (Doctrine: a finished adapter is "pending eval + gatekeeper" — not active
        // until it passes eval and the gate marks it safe.) ──
        let tfJob = null;
        try {
          const reg = require('./pipeline-registry');
          tfJob = reg.start({ pipeline: 'lora.train', project: 'PURPCLAW', lane: 'Training Forge', trigger: 'idle-engine', risk: 'medium', inputs: { dataset: exported.path, base: process.env.LORA_BASE_MODEL || 'Qwen/Qwen2.5-1.5B-Instruct' } });
          if (tfJob) { reg.step(tfJob.job_id, 'dataset_locked'); reg.step(tfJob.job_id, 'training'); reg.output(tfJob.job_id, exported.path, { kind: 'dataset' }); }
        } catch (_) {}
        ...
        await new Promise((resolve) => {
          child.on('exit', (code) => {
            ...
            try {
              const reg = require('./pipeline-registry');
              if (tfJob) reg.finish(tfJob.job_id, {
                status: code === 0 ? 'complete' : 'failed',
                claim: code === 0 ? 'LoRA adapter trained — PENDING eval + gatekeeper (not active)' : `LoRA training failed (exit ${code})`,
                proof: { ran: 'lora-train.py', result: code === 0 ? 'pass' : 'fail', detail: code === 0 ? 'adapter merged, awaiting eval' : `exit ${code}: ${String(output).slice(-160)}` },
              });
            } catch (_) {}
            resolve();
          });
  ...
  // ── Phase 5.5: Curator skill maintenance ─────────────────────────────────
  try {
    state.currentPhase = 'curator';
    saveState();
    log('Phase 5.5/6: Running skill curator...');
    const Curator = require('./curator');
    if (Curator.shouldRunNow && Curator.shouldRunNow()) {
      const curResult = await Curator.runCurator({ dryRun: false, consolidate: false });
      const archived = curResult.archived || 0;
      const stale = curResult.stale || 0;
      log(`  ✓ Curator ran: ${archived} archived, ${stale} marked stale`);
      results.phases.curator = { ok: true, archived, stale };
    } else {
      log('  ○ Curator skipped (not due for a run)');
      results.phases.curator = { ok: true, skipped: true };
    }
  } catch (e) {
    log(`  ✗ Curator failed: ${e.message}`);
    results.phases.curator = { ok: false, error: e.message };
  }
