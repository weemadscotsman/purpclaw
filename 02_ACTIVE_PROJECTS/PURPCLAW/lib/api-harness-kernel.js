...
// ── Co-Work alert (fire-and-forget POST to cowork overlay) ───────────────────
// Disabled by COWORK_ALERT_ENABLED=false
function sendCoworkAlert(msg, type = 'info') {
  ...
  const body = JSON.stringify({ msg, type });
  const req = http.request({ hostname: host, port, path: '/push', method: 'POST',
...
let mcpClient = null;
try { mcpClient = require('./mcp'); } catch {}
...
class ApiHarnessKernel extends EventEmitter {
  ...
  loadArchive() {
    ...
    // Load MCP servers from ~/.purpclaw/mcp.json at boot so the agent
    // loop has access to mcp__<server>__<tool> tools for every request.
    // This is the missing wire — lib/mcp.js existed but was never called.
    if (mcpClient) {
      mcpClient.loadServers()
        .then((stats) => {
          this.mcpServers = mcpClient.listServers();
          this.mcpTools   = mcpClient.listTools();
          // Push into the central tool registry so the LLM sees them.
          try {
            const reg = require('./tools');
            if (typeof reg.__registerMcpTools === 'function') {
              reg.__registerMcpTools(this.mcpTools, (server, tool, args) =>
                mcpClient.callMcpTool(server, tool, args));
              console.log(`[MCP] Loaded ${stats.count} server(s), ${stats.tools} tool(s); registered with agent loop`);
            }
          } catch (e) {
            console.log(`[MCP] tool-registry push failed: ${e.message}`);
          }
        })
        .catch((e) => {
          console.log(`[MCP] loadServers failed: ${e.message}`);
        });
    }
  }
  ...
  createJob(input = {}) {
    ...
    // Operator-initiated unless the job came from an autonomous/self-triggered
    // source (idle engine, autodream, evolution, cron, reasoning tick). A human
    // driving the chat/CLI is implicit approval — see governance.checkWorkflow.
    const AUTONOMOUS_SOURCES = /(idle|autodream|autonomous|evolution|self-?evolve|cron|schedul|reasoning-tick|background)/i;
    const jobSource = String(input.source || input.client || 'unified-api');
    const operatorInitiated = input.operatorInitiated === true || !AUTONOMOUS_SOURCES.test(jobSource);
    ...
    job.governance = gov;
    if (gov.autoApproved) {
      // Audit trail: operator-initiated action auto-approved (gated, not gutted).
      try {
        const approval = governance.requestApproval(this.rootDir, id, goal, contract, gov);
        governance.setApprovalStatus(this.rootDir, approval.id, 'approved');
      } catch (_) { /* audit log best-effort */ }
      this.addEvent(job, 'auto-approved', 'governance', `operator-initiated (${jobSource}); risks [${(gov.risks || []).join(', ') || 'none'}] auto-approved`, { governance: gov });
    }
    if (!gov.allowed) {
      const approval = governance.requestApproval(this.rootDir, id, goal, contract, gov);
  ...
  finishJob(job) {
    ...
    // Co-Work alert: notify the overlay of job completion
  ...
  addEvent(job, type, stage, message, detail) {
    ...
    // Unified job-chain: mirror every kernel event into the one chain so any
    // job reads start→finish and a failure pinpoints the EXACT hop that broke.
    // Best-effort — chain logging never breaks the job.
    try {
      const chain = require('./job-chain');
      const t = String(type || '');
      const area = stage || 'kernel';
      if (t === 'failed' || t === 'blocked') chain.fail(job.id, { area, detail: message });
      else if (t === 'completed') chain.done(job.id, { area, detail: message });
      else if (t === 'accepted') chain.start(job.id, { area, detail: message });
      else chain.step(job.id, {
        stage: t === 'delegated' ? 'delegated' : t === 'started' ? 'executing' : 'executing',
        area, detail: `${t}${message ? ': ' + message : ''}`,
      });
    } catch { /* chain optional — never break the job */ }
  }
