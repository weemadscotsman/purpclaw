// LEGACY (role=legacy in registry/workflow-authority.json). Event/message-passing
// workflow engine, unwired (0 consumers), superseded by lib/workflow-manager.js as
// the canonical workflow MUTATION authority. Retained read-only for its canonical
// message contract. Do NOT wire this as a second workflow authority.
'use strict';const path=require('path'),crypto=require('crypto');const{DatabaseSync}=require('node:sqlite'),SCHEMA=require('./schema-validator');const DB=process.env.PURPCLAW_SESSION_DB||path.join(path.resolve(__dirname,'..'),'.purpclaw','state.db'),db=(require('fs').mkdirSync(path.dirname(DB),{recursive:true}),new DatabaseSync(DB)),definitions=new Map();
