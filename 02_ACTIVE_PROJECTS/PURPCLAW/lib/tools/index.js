...
// Windows filesystem adapter for native path handling
const { windowsLs } = (() => { try { return require('./windows-fs-adapter'); } catch { return {}; } })();
...
registry.register({
  ...
  inputSchema: {
    type: 'object',
    properties: {
      command: { type: 'string', description: 'Shell command to run (aliases accepted: cmd, script)' },
  ...
  execute: async (args = {}) => {
    // Accept the aliases models actually emit. Observed live: MiniMax writes
    // {"tool":"shell","args":{"cmd":"..."}} and the destructure left `command`
    // undefined, which threw "Cannot read properties of undefined (reading
    // 'trim')" — an internal crash, not a usable error. The model then retried
    // in a loop (one prompt burned 10 failing shell calls). Same pattern the
    // `write` tool already handles with path||file.
    const command = args.command ?? args.cmd ?? args.script ?? args.input;
    const { timeout = 30_000, cwd } = args;
    if (typeof command !== 'string' || !command.trim()) {
      return { ok: false, error: 'shell requires a non-empty "command" string (aliases: cmd, script). Example: {"tool":"shell","args":{"command":"dir"}}' };
    }
    const safeCwd = cwd ? path.resolve(cwd) : process.cwd();

    // ── Windows path-listing shortcut: intercept ls/dir/cd and use native windowsLs
    if (windowsLs && process.platform === 'win32') {
      const trimmed = command.trim();
      // Strip surrounding quotes from the entire command (common in MSYS path handling)
      const unquoted = trimmed.replace(/^(['"])(.*)\1$/, '$2');
      // Match: ls [path], dir [path], cd [path]
      const lsMatch = unquoted.match(/^(ls|dir|cd)\s+(.+)$/i);
      if (lsMatch) {
        let targetPath = lsMatch[2].trim().replace(/^(['"])(.*)\1$/, '$2');
        try {
          const result = await windowsLs(targetPath);
          return { code: result.ok ? 0 : 1, stdout: result.stdout, stderr: result.stderr, cwd: result.cwd };
        } catch {
          // Fall through to normal shell execution
        }
      }
      // Also handle bare `ls path` or `dir path`
      const bareMatch = unquoted.match(/^(ls|dir)\s+(.+)$/i);
      if (bareMatch) {
        let targetPath = bareMatch[2].trim().replace(/^(['"])(.*)\1$/, '$2');
        try {
          const result = await windowsLs(targetPath);
          return { code: result.ok ? 0 : 1, stdout: result.stdout, stderr: result.stderr, cwd: result.cwd };
        } catch {
...
try {
  ...
  // Spec: hide huge tool counts from default startup. Behind PURPCLAW_VERBOSE=1.
  if (process.env.PURPCLAW_VERBOSE === '1') {
