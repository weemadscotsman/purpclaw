...
function createJobContract(command, parsed = {}, options = {}) {
  // Accept both legacy string command and full PurpClawTask schema fields (Stage 1 parity).
  // When a harness-normalised task arrives, prefer its explicit fields over keyword classification.
  const text = String(command || parsed.raw || parsed.target || '').trim();
  const classification = classifyJob(text);

  // Task-schema overrides: if caller passed explicit intent/agents/gates, use those
  const hasTaskSchema = Boolean(parsed.goal || parsed.routeIntent || parsed.harnessMode);
  const typeKey = hasTaskSchema
    ? ({ codex: 'code', claude: 'architecture', hermes: 'operations', minimax: 'graphics' })[parsed.harnessMode]
    : classification.type;
  const typeDef = JOB_TYPES[typeKey] || JOB_TYPES.code;

  const routeIntent = parsed.routeIntent
    || (hasTaskSchema && parsed.harnessMode === 'codex' ? 'build' : null)
    || (hasTaskSchema && parsed.harnessMode === 'claude' ? 'architect' : null)
    || (hasTaskSchema && parsed.harnessMode === 'hermes' ? 'infrastructure' : null)
    || (hasTaskSchema && parsed.harnessMode === 'minimax' ? 'design' : null)
    || classification.routeIntent;

  const gates = [...new Set([...(typeDef.gates || DEFAULT_GATES), ...(options.extraGates || [])])];

  // Merge: task-schema preferredAgents wins if provided, else type-default
  const preferredAgents = (parsed.preferredAgents && parsed.preferredAgents.length > 0)
    ? parsed.preferredAgents
    : typeDef.agents;

  // Carry task-schema context through to the contract so the engine can forward it
  const taskContext = hasTaskSchema ? {
    harnessMode:    parsed.harnessMode    || null,
    targetPath:     parsed.targetPath     || null,
    knownFiles:      parsed.knownFiles     || null,
    constraints:    parsed.constraints    || null,
    requiredOutput: parsed.requiredOutput || null,
    verificationRules: parsed.verificationRules || null,
  } : null;

  return {
    id: `job-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    source: options.source || (hasTaskSchema ? 'harness' : 'orchestrator'),
    command: text,
    type: typeKey,
    confidence: hasTaskSchema ? 'schema-driven' : classification.confidence,
    routeIntent,
