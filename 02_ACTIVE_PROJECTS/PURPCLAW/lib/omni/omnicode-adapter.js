...
function createOmnicodeClient(options) {
  ...
  // ── Pipeline spine: every OMNI action becomes a job + proof row, visible on
  // the board cross-process. "No OMNI result is true unless the proof ledger can
  // point to evidence." These are read-only scans — safe, no mutation. ──
  return _spineWrap(__client);
}

const _OMNI_LANES = {
  truthSnapshot:   'truth.scan',
  routeMap:        'repo.routes',
  testMap:         'repo.tests',
  configMap:       'repo.config',
  blastRadius:     'code.blast',
  deadCodeScan:    'code.deadscan',
  blindspotReport: 'code.blindspot',
  spaghettiReport: 'code.spaghetti',
  cockpitStatus:   'omni.status',
};

function _spineWrap(client) {
  let withPipeline = null;
  try { ({ withPipeline } = require('../with-pipeline')); } catch (_) { return client; }
  if (!withPipeline) return client;
  const proj = (p) => String(p || 'repo').split(/[\\/]/).filter(Boolean).pop() || 'repo';
  const out = {};
  for (const [name, fn] of Object.entries(client)) {
    const lane = _OMNI_LANES[name];
    if (typeof fn !== 'function' || !lane) { out[name] = fn; continue; }
    out[name] = async (...args) => {
      let res;
      await withPipeline(
        { pipeline: lane, project: proj(args[0]), lane: 'OMNI', trigger: 'omni', risk: 'low', inputs: { action: name, repo: args[0] } },
        async (ctx) => {
          ctx.step(name);
          res = await fn(...args);
          const failed = !!(res && res.ok === false);
          ctx.output(`omni:${lane}`, { kind: 'omni-result' });
          return {
            status: failed ? 'failed' : 'complete',
            claim: `${lane} on ${proj(args[0])}`,
            proof: { ran: `omnicode.${name}`, result: failed ? 'fail' : 'pass', detail: failed ? String(res.error || 'omni error').slice(0, 120) : `${name} ok` },
          };
        }
      );
      return res;
