...
class SoulRegistry {
  ...
  summon(problem, opts = {}) {
    ...
    if (chairOverride) {
      ...
      // Canonical chair per meeting type — beats keyword scoring. Falls through
      // to keyword match if the canonical chair isn't in the council.
      const CANONICAL_CHAIRS = {
        engineering: 'hermes',
        security: 'smith',
        creative: 'goose',
        funding: 'finance',
      ...
      const canonicalId = CANONICAL_CHAIRS[meetingType.toLowerCase()] || null;
      if (canonicalId) {
        const inCouncil = council.find(c => c.id === canonicalId);
        const inScored = scored.find(s => s.id === canonicalId);
        // Council members lack a `.soul` reference, so prefer scored (full shape).
        // If only the council entry exists, fall back to scored anyway to read soul.*
        const canonical = inScored || inCouncil;
        if (canonical && canonical.soul) {
          chair = canonical;
        } else if (inCouncil) {
          // Synthesize a minimal shape from the council entry so the rest works.
          const soulFallback = this.get(inCouncil.id);
          chair = { id: inCouncil.id, soul: soulFallback, total: inCouncil.score || 0, reasons: ['canonical chair'] };
        }
      }
      if (!chair) {
  ...
  _scoreSoulForProblem(soul, problem) {
    ...
    const DIVISION_DEFAULT_SKILLS = {
      ...
      MEDIA_OPERATIONS: ['media', 'video', 'audio', 'content', 'podcast', 'show', 'production', 'writing', 'creative', 'brand'],
