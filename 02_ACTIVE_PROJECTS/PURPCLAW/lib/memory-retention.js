...
async function flushQueue() {
  while (queue.length) {
    // Peek — do NOT remove until the write is confirmed. Memory is permanent
    // and not optional: a spine blip must never silently drop a write.
    const record = queue[0];
    const st = loadState();
    if (record.key && st.keys[record.key]) { queue.shift(); continue; }
    ...
    if (result.ok) {
      queue.shift(); // confirmed persisted — safe to drop from the retry queue
      if (record.key) {
        st.keys[record.key] = {
      ...
      // Spine unreachable — keep the record queued and retry with backoff.
      // The journal (appendJournal at enqueue) is the durable on-disk backstop;
      // this keeps it in flight until the write actually lands.
      record._retries = (record._retries || 0) + 1;
      if (record._retries > 240) { queue.shift(); } // ~2h of retries; journal still holds it
      else {
        flushing = false;
        const delay = Math.min(30000, 500 * record._retries);
        setTimeout(flushQueue, delay).unref?.();
        return;
      }
    }
  }
