...
async function reportEvent(evt) {
  try {
    return await httpPost(PORTS.diagnostics, '/diagnostics/event', {
      ...
      description: evt.event || evt.description || 'unknown',
      severity : evt.severity || 'INFO',
      metadata : { data: evt.data || {}, ts: new Date().toISOString() },
    });
  } catch { return null; }
}
...
async function diagnose(context = {}) {
  ...
  try {
    ...
    const [findings, vote, causalGraph] = await Promise.all([
      httpPost(PORTS.diagnostics, '/diagnostics/diagnose', context).catch(() => null),
      httpGet(PORTS.diagnostics, '/diagnostics/vote').catch(() => null),
      httpGet(PORTS.diagnostics, '/diagnostics/causal-graph').catch(() => null),
    ]);
    return { findings, vote, causalGraph };
  } catch { return null; }
}
...
async function retractFact(predicate, terms) {
  try {
    return { ok: false, reason: 'rules retract is not exposed by cognitive_spine.py', predicate, terms };
  } catch { return null; }
}
...
async function queryNeuro(factType = 'pattern_detected', limit = 10) {
  const payload = { fact_type: factType, limit };
  try {
    return await httpPost(PORTS.neuro, '/neuro-symbolic/query', payload);
  } catch (firstError) {
    await new Promise(resolve => setTimeout(resolve, 150));
    try {
      return await httpPost(PORTS.neuro, '/neuro-symbolic/query', payload);
...
// ── MEMORY LAYER 2-7 BRIDGE (single-port collapse over 7880) ──────────────────
// The spine exposes /memory/* routes that hit memory_matrix_v2.py. These
// wrappers give the agent loop a uniform API for layer 2-7 operations.

/**
 * Layer 3 — Temporal Projection: "what was I working on at T?"
 */
async function temporalProject(query, targetTime = null, limit = 5) {
  try {
    return await httpPost(PORTS.spine, '/memory/project', {
...
/**
 * Layer 3 — Timeline for an entity across all stored memory.
 */
async function getTimeline(entity, limit = 20) {
  try {
    return await httpGet(PORTS.spine, `/memory/timeline/${encodeURIComponent(entity)}?limit=${limit}`);
  } catch { return null; }
}

/**
 * Layer 4 — Counterfactual: "what if I had forgotten X?"
 */
async function whatIfForgotten(memoryId, query = '') {
  try {
    return await httpPost(PORTS.spine, '/memory/what_if/forgotten', {
      memory_id: memoryId,
      query: String(query || '').substring(0, 200),
    });
  } catch { return null; }
...
/**
 * Layer 4 — Counterfactual: "what if I had noticed X?"
 */
async function whatIfNoticed(entity, startTime, endTime, query = '') {
  try {
    return await httpPost(PORTS.spine, '/memory/what_if/noticed', {
      entity,
...
/**
 * Layer 4 — List all counterfactual branches.
 */
async function getCounterfactualBranches() {
  try {
    return await httpGet(PORTS.spine, '/memory/counterfactual/branches');
  } catch { return null; }
}

/**
 * Layer 2 — Lifted symbolic facts (high-level knowledge extracted from memories).
 */
async function getLiftedFacts() {
  try {
    return await httpGet(PORTS.spine, '/memory/lifted');
  } catch { return null; }
}

/**
 * Layer 2 — Ground symbolic query back into memory.
 */
async function groundSymbolic(query, limit = 5) {
  try {
    return await httpPost(PORTS.spine, '/memory/ground', { query, limit });
  } catch { return null; }
}

/**
 * Layer 6 — Modal: set an agent's belief with confidence.
 */
async function setBelief(agentName, prop, confidence = 0.5) {
  try {
    return await httpPost(PORTS.spine, '/modal/agent/doxastic/belief', {
      agent_id: agentName || 'PURPCLAW_CORE',
      prop,
      confidence,
    });
...
/**
 * Layer 6 — Modal: permit an action for an agent (deontic).
 */
async function permitAction(agentName, action) {
  try {
    return await httpPost(PORTS.spine, '/modal/agent/deontic/permit', {
      agent_id: agentName || 'PURPCLAW_CORE',
      action,
    });
  } catch { return null; }
...
/**
 * Layer 6 — Modal: epistemic know/learn.
 */
async function learn(agentName, prop, value = true) {
  try {
    return await httpPost(PORTS.spine, '/modal/agent/epistemic/know', {
      agent_id: agentName || 'PURPCLAW_CORE',
      prop,
      value,
    });
...
/**
 * Layer 5 — Symbolic rules: assert a structured fact (predicate + terms).
 */
async function assertFactTyped(agentName, predicate, terms, provenance = 'agent_loop') {
  try {
    return await assertFact(predicate, [agentName, ...terms], provenance);
  } catch { return null; }
}

/**
 * Diagnostics: deep diagnosis on a subsystem event.
 */
async function runDiagnosis(agent = null) {
  try {
    return await httpPost(PORTS.diagnostics, '/diagnostics/diagnose', agent ? { agent } : {});
  } catch { return null; }
}

/**
 * Layer 1 — AutoDream: trigger a dream cycle (background consolidation).
 */
async function runDreamCycle() {
  try {
    return await httpPost(PORTS.spine, '/autodream/dream', {});
  } catch { return null; }
}

/**
 * Layer 1 — AutoDream status snapshot.
 */
async function getDreamStatus() {
  try {
    return await httpGet(PORTS.spine, '/autodream/status');
  } catch { return null; }
}
...
/**
 * Full layer-2-through-7 status snapshot. The agent loop can ask the
 * spine for everything in one round trip before a decision.
 */
async function getCognitiveSnapshot() {
  const [health, lifted, branches, dream, rulesStats, modalStats] = await Promise.all([
    isOnline('spine').then(async (ok) => ok
      ? await httpGet(PORTS.spine, '/health').catch(() => null)
      : null),
    getLiftedFacts().catch(() => null),
    getCounterfactualBranches().catch(() => null),
    getDreamStatus().catch(() => null),
    httpGet(PORTS.spine, '/rules/stats').catch(() => null),
    httpGet(PORTS.spine, '/modal/engine/stats').catch(() => null),
  ]);
  return { health, lifted, branches, dream, rulesStats, modalStats };
}
...
module.exports = {
  ...
  // Memory layer 2 (neuro-symbolic + symbolic lift/ground)
  getLiftedFacts,
  groundSymbolic,

  // Memory layer 3 (temporal projection)
  temporalProject,
  getTimeline,

  // Memory layer 4 (counterfactual)
  whatIfForgotten,
  whatIfNoticed,
  getCounterfactualBranches,

  // AutoDream (layer 1 background consolidation)
  runDreamCycle,
  getDreamStatus,
