...
class Studio {
  ...
  updateWorld(delta) {
    // Extract private handoff fields (underscore-prefixed) — these are not world
    // state keys, just side-band signals between caller and this method.
    const incidentLabel = (delta && delta._incidentLabel) || null;
    // If caller provided a label but no active_incidents key in the delta, still
    // append the label so director incidents visibly land in the world list.
    let appendedLabel = false;
    if (incidentLabel && (!('active_incidents' in delta))) {
      if (!this.world.state.active_incidents.includes(incidentLabel)) {
        this.world.state.active_incidents.push(incidentLabel);
        appendedLabel = true;
      }
    }
    for (const [key, val] of Object.entries(delta)) {
      if (key.startsWith('_')) continue; // skip private handoff fields
      if (key === 'active_incidents') {
        if (typeof val === 'string' && val.startsWith('+push:')) {
          // Legacy: explicit incident string from github_outage
        ...
        } else if (val !== null && val !== undefined && incidentLabel && !appendedLabel) {
          // Any other truthy value (e.g. true / severity flag from a director incident)
          // means: append the labeled incident. Caller passes the label via _incidentLabel.
          if (!this.world.state.active_incidents.includes(incidentLabel)) {
            this.world.state.active_incidents.push(incidentLabel);
          }
        }
  ...
  inject(incidentId) {
    ...
    // Tag the delta with the human label so updateWorld knows what string to push
    // into active_incidents for incidents that don't use the legacy +push: prefix.
    const taggedDelta = { ...incident.world_delta };
    if (!taggedDelta.active_incidents || !String(taggedDelta.active_incidents).startsWith('+push:')) {
      // Synthesize an active_incidents entry with the label (severity-stamped)
      taggedDelta._incidentLabel = `[${incident.severity}] ${incident.label}`;
    }
    this.updateWorld(taggedDelta);
    this._updateWorldFromSession();
    timelineRecord({
