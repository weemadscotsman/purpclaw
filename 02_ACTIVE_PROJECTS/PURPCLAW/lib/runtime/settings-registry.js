...
const CATALOG = [
  ...
  // ── ui / visuals ──
  { key: 'ui.mode', label: 'UI mode', category: 'ui', type: 'enum', scope: 'user', path: 'ui.mode', default: 'hybrid', options: ['classic', 'hybrid', 'immersive', 'low-power', 'full-chaos'], help: 'Classic tabs / scene+drawers / full cockpit / low-power / all engines.' },
...
function listCustomPresets() {
  ...
  return Object.entries(cfg.customPresets || {}).map(([id, preset]) => ({
    id,
    label: preset.label || id,
    createdAt: preset.createdAt || null,
    updatedAt: preset.updatedAt || null,
    keys: Object.keys(preset.values || {}),
  }));
}

function saveCustomPreset(input = {}) {
  const label = String(input.label || input.name || '').trim();
  if (!label) return { ok: false, error: 'preset label required' };
  const id = String(input.id || label.toLowerCase().replace(/[^a-z0-9_-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 48) || 'custom').trim();
  const values = input.values && typeof input.values === 'object' ? input.values : exportAll();
  const cleaned = {};
  ...
  if (!Object.keys(cleaned).length) return { ok: false, error: 'no non-secret settings to save' };
  const cfg = readUserSettings();
  cfg.customPresets = cfg.customPresets || {};
  const existing = cfg.customPresets[id] || {};
  cfg.customPresets[id] = {
    id,
    label,
    values: cleaned,
  ...
  writeJson(USER_SETTINGS_PATH, cfg);
  return { ok: true, preset: cfg.customPresets[id] };
}

function deleteCustomPreset(id) {
  const key = String(id || '').trim();
  if (!key) return { ok: false, error: 'preset id required' };
  const cfg = readUserSettings();
  if (!cfg.customPresets || !cfg.customPresets[key]) return { ok: false, error: `unknown custom preset: ${key}` };
  delete cfg.customPresets[key];
  writeJson(USER_SETTINGS_PATH, cfg);
...
function applyPreset(name) {
  let p = PRESETS[name];
  if (!p) {
    const custom = readUserSettings().customPresets || {};
    p = custom[name]?.values;
  }
...
module.exports = { CATALOG, get, set, list, search, categories, exportAll, importAll, applyPreset, listCustomPresets, saveCustomPreset, deleteCustomPreset, PRESETS };
