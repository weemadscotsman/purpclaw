 ...
 *   1. If no explicit model, classify the prompt → pick a lane (routing-decisions).
 ...
 *
 * ROUTING CHAIN: This file now reads LANES and fallbacks from routing-decisions.js
 * (the single canonical source). No other routing logic lives here.
 */

const { runAgent } = require('./agent-loop');

// routing-decisions.js is the single source of truth for all lane definitions.
let RD = null;
try { RD = require('./routing-decisions'); } catch (_) { RD = null; }
let registry = null;
...
async function* runAgentRouted({ prompt, history = [], model, provider, lane, autoRoute = true, opts = {} }) {
  ...
  // ── Route: use canonical routing-decisions.js ────────────────────────────
  let routed = null;
  if (RD && (autoRoute || lane || model)) {
    try {
      routed = RD.resolve({ message: prompt, lane, model, provider, agent: opts.role });
    } catch (_) { routed = null; }
  } else if (model || provider) {
    // Fallback: explicit model/provider without routing-decisions
    routed = { provider: provider || 'nvidia', model, lane: 'custom', agent: null, label: model, fallbacks: [] };
  }

  // Build the per-hop provider list using routing-decisions' providerForHop.
  // Hop 0 uses the resolved primary provider; hops 1+ use nvidia (NIM).
  const chain = routed ? RD.getChain(routed) : [model].filter(Boolean);
  const primaryProvider = routed ? routed.provider : (provider || 'nvidia');
  function providerForHop(ci) {
    return RD.providerForHop(ci, primaryProvider, chain[ci]);
  }
