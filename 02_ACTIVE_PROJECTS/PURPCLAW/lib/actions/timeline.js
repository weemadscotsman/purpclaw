...
async function run(action, options = {}) {
  ...
  const cap = (caps.capabilities || caps)['timeline'] || {};
  ...
  try {
    ...
    if (action === 'record') {
      ...
      result = timeline.record({ kind: kind || 'surface.event', ...(payload || {}), refs: { ...(payload?.refs || {}), run_id: runId || null } });
    } else if (action === 'recent') {
      result = timeline.recent(options.n || 20);
    } else if (action === 'patterns') {
      result = timeline.patterns(options.n || 20);
    } else if (action === 'weave') {
      if (!MEMORY) return { ok: false, action, surface, error: 'memory-client not loaded' };
      result = await MEMORY.ingest(options);
    } else if (action === 'recall') {
      if (!MEMORY) return { ok: false, action, surface, error: 'memory-client not loaded' };
      result = await MEMORY.recall(options);
    }
