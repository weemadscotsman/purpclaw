...
// Load provider status state machine
let PROVIDER_STATES;
let getProviderStatus;
try {
  const ps = require('../core/provider-status');
  PROVIDER_STATES = ps.PROVIDER_STATES || ps.STATES;
  getProviderStatus = ps.getProviderStatus;
  if (!PROVIDER_STATES) throw new Error('provider state constants unavailable');
} catch {
  // fallback if core not yet built
  PROVIDER_STATES = {
    MISSING: 'missing', CONFIGURED: 'configured', VERIFIED: 'verified',
    AUTH_FAILED: 'auth_failed', LOCAL_UNAVAIL: 'local_unavailable', AVAILABLE: 'available',
  };
...
const PROVIDERS = [
  ...
  { id: 'glm',         name: 'Z.AI / GLM',         keyEnv: 'GLM_API_KEY',          baseUrl: 'https://api.z.ai/api/paas/v4' },
  ...
  { id: 'nvidia',      name: 'NVIDIA NIM',         keyEnv: 'NVIDIA_API_KEY',       baseUrl: 'https://integrate.api.nvidia.com/v1', free: true },
  { id: 'huggingface', name: 'Hugging Face',       keyEnv: 'HUGGINGFACE_API_KEY',  baseUrl: 'https://router.huggingface.co/v1', free: true },
  { id: 'cloudflare',  name: 'Cloudflare Workers AI', keyEnv: 'CF_API_TOKEN',       baseUrl: 'https://api.cloudflare.com', free: true },
  { id: 'cohere',      name: 'Cohere',             keyEnv: 'COHERE_API_KEY',       baseUrl: 'https://api.cohere.com/compatibility/v1' },
  { id: 'github-models', name: 'GitHub Models',    keyEnv: 'GITHUB_TOKEN',         baseUrl: 'https://models.inference.ai.azure.com', free: true },
  { id: 'codex',       name: 'OpenAI API (Codex)', keyEnv: 'OPENAI_API_KEY',       baseUrl: 'https://api.openai.com/v1' },
  { id: 'codex-oauth', name: 'Codex native login', keyEnv: 'CODEX_OAUTH_TOKEN',    baseUrl: 'https://api.openai.com/v1' },
  ...
  { id: 'atomic-chat', name: 'Atomic Chat',        keyEnv: 'ATOMIC_CHAT_API_KEY',  baseUrl: null },
...
const DEFAULT_MODELS = {
  ...
  kimi: 'kimi-k2-5', glm: 'glm-4.6', together: 'meta-llama/llama-3-70b', mistral: 'mistral-small',
  minimax: 'MiniMax-M3', nvidia: 'minimaxai/minimax-m3', huggingface: 'meta-llama/Llama-3.1-8B-Instruct',
  cloudflare: '@cf/meta/llama-3.1-8b-instruct', cohere: 'command-r-plus',
  'github-models': 'gpt-4o-mini', codex: 'gpt-5-codex', 'codex-oauth': 'gpt-5-codex',
  ollama: 'qwen2.5:3b', lmstudio: 'local-model', 'atomic-chat': 'atomic-chat-default',
  custom: 'default',
};
...
function setEnvValue(name, value) {
  ...
  const re = new RegExp(`^\\s*#?\\s*${escaped}=.*$`, 'm');
...
async function showStatus(foundKeys = null) {
  const keys = foundKeys || scanForKeys();

  // Load state machine roles
  const ROLES = {
    ...
    'flash-nim':     'fast worker',
    'ollama':        'local inference (free)',
    'lmstudio':      'local inference (free)',
    'openai':        'general chat',
    'anthropic':     'general chat',
    'gemini':        'general chat',
    'kimi':          'general chat',
  ...
  console.log('  ' + 'provider'.padEnd(18) + 'state'.padEnd(14) + 'role');
  console.log('  ' + '─'.repeat(72));

  for (const p of PROVIDERS) {
    const hasKey = keys[p.id];
    const isLocal = p.local || false;

    // Determine state using state machine
    let state, icon;
    if (hasKey) {
      state = PROVIDER_STATES.CONFIGURED;
      icon = '○';
    } else if (isLocal) {
      // Check if local service is actually running (best-effort)
      try {
        require('child_process').execSync(
          `curl -s --max-time 1 ${p.baseUrl.replace('/v1', '/api/tags')} > nul 2>&1`,
          { stdio: 'ignore', windowsHide: true }
        );
        state = PROVIDER_STATES.VERIFIED;
        icon = '●';
      } catch {
        state = PROVIDER_STATES.LOCAL_UNAVAIL;
        icon = '△';
      }
    } else if (p.free) {
      ...
      state = PROVIDER_STATES.MISSING;
      icon = '✗';
    }

    const role = ROLES[p.id] || '';
    const stateStr = String(state).padEnd(14);
    let notes = '';
    if (hasKey) notes = `key: ${hasKey.key}`;
    else if (isLocal) notes = state === PROVIDER_STATES.VERIFIED ? 'running' : 'not running';
    else if (p.free) notes = p.local ? 'install locally' : 'free tier';
    else notes = `set ${p.keyEnv || 'N/A'}`;

    console.log(`  ${icon} ${p.id.padEnd(16)} ${stateStr} ${role || notes}`);
  }
  console.log('');
  const ready = PROVIDERS.filter(p => keys[p.id] || (p.local || p.free));
  console.log(`  ${Object.keys(keys).length} of ${PROVIDERS.length} providers configured`);
  console.log('');
}

// ── LEGACY showStatusSync (sync, for --list flag) ─────────────────────
function showStatusSync(foundKeys = null) {
  const keys = foundKeys || scanForKeys();
  console.log('\n  🟣 PurpClaw Provider Status\n');
...
async function wizard() {
  ...
  // Runtime truth is shared by every surface and service. The JSON file below
  // is user-facing metadata; .env drives CLI, TUI, web, API, tower and orchestration.
  setEnvValue('LLM_PROVIDER', provider.id);
  setEnvValue('LLM_MODEL', model);
  ...
  try {
    ...
    const config = {
      ...
      version: '0.9.0',
      surfaces: ['cli', 'tui', 'web', 'api', 'tower', 'orchestrator'],
    };
