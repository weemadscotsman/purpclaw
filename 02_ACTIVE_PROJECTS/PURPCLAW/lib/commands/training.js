...
async function run(args, ctx) {
  ...
  // ── Extended status: buffer size, last export, total tokens ───────────────
  if (sub === '--status' || (args[0] || '').toLowerCase() === '--status') {
    banner(ctx);
    const s = buffer.stats;
    const statsPath = buffer.statsFile;
    const enabled = !process.env.PURPCLAW_TRAINING_DISABLED || process.env.PURPCLAW_TRAINING_DISABLED !== '1';
    const days = buffer.listDays().length;
    const allRecords = buffer.readAll();
    let totalTokens = 0;
    for (const r of allRecords) {
      const in_  = ((r.input  || '').length);
      const out_ = ((r.output || '').length);
      totalTokens += Math.ceil(in_ / 4) + Math.ceil(out_ / 4);
    }
    ...
    console.log(`  ${col(C.cyan, 'buffer size:')}   ${col(C.white, days + ' daily files')}`);
    console.log(`  ${col(C.cyan, 'total sessions:')} ${col(C.white, String(s.totalRecorded || allRecords.length))}`);
    console.log(`  ${col(C.cyan, 'total tokens:')}  ${col(C.white, totalTokens.toLocaleString() + ' (approx)')}`);
    console.log(`  ${col(C.cyan, 'total success:')} ${col(C.green, String(s.totalSuccess || 0))}`);
    console.log(`  ${col(C.cyan, 'total failed:')}  ${col(C.red,   String(s.totalFailed  || 0))}`);
    console.log(`  ${col(C.cyan, 'total exported:')} ${col(C.white, String(s.totalExported || 0))}`);
    console.log(`  ${col(C.cyan, 'last export:')}   ${s.lastExportTs ? col(C.gray, s.lastExportTs) : col(C.gray, 'never')}`);
    console.log(`  ${col(C.cyan, 'last record:')}   ${s.lastRecordTs ? col(C.gray, s.lastRecordTs)  : col(C.gray, 'never')}`);
    console.log('');
    return;
  }
  ...
  if (sub === 'export') {
    ...
    // ── purpclaw train --from-sessions --days=N ──────────────────────────────
    const fromSessions = rest.includes('--from-sessions');
    const daysArg = rest.find(a => a.startsWith('--days='));
    const daysN = daysArg ? parseInt(daysArg.split('=')[1], 10) : 7;

    if (fromSessions) {
      const since = new Date();
      since.setDate(since.getDate() - daysN);
      const sinceStr = since.toISOString().slice(0, 10); // YYYY-MM-DD
      const untilStr = new Date().toISOString().slice(0, 10);

      const records = buffer.readAll({ since: sinceStr, until: untilStr });
      const valid = records.filter(r => r.output);
      let totalTokens = 0;
      for (const r of valid) {
        totalTokens += Math.ceil(((r.input  || '').length) / 4) + Math.ceil(((r.output || '').length) / 4);
      }

      console.log(`  ${col(C.cyan, 'from-sessions:')}  last ${daysN} day(s)  ${col(C.gray, '(' + sinceStr + ' → ' + untilStr + ')')}`);
      console.log(`  ${col(C.cyan, 'sessions:')}        ${col(C.white, valid.length)} with output`);
      console.log(`  ${col(C.cyan, 'total tokens:')}     ${col(C.white, totalTokens.toLocaleString() + ' (approx)')}`);
      console.log(`  ${col(C.cyan, 'format:')}           ${col(C.white, 'chatml')}`);

      if (valid.length === 0) {
        console.log(`\n  ${col(C.yellow, 'No sessions with output in the last ' + daysN + ' day(s).')}\n`);
        return;
      }
      ...
      if (valid.length > 0) {
        const first = valid[0];
        const inputPreview = ((first.input  || '').slice(0, 120) + (first.input  && first.input.length  > 120 ? '…' : ''));
        const outputPreview = ((first.output || '').slice(0, 120) + (first.output && first.output.length > 120 ? '…' : ''));
        console.log(`\n  ${col(C.gray, 'preview (first session):')}`);
        console.log(`    ${col(C.magenta, 'input:')}  ${inputPreview}`);
        console.log(`    ${col(C.green,   'output:')} ${outputPreview}`);
      }

      // Export to ChatML
      const result = buffer.export({ format: 'chatml', since: sinceStr, until: untilStr });
      if (result.error) {
        console.error(col(C.red, `\n  ✗ export failed: ${result.error}\n`));
        process.exitCode = 1;
        return;
      }
      console.log(`\n  ${col(C.green, '✓')}  exported ${result.count} sessions  ${col(C.gray, '→ ' + result.file)}`);
      console.log(`  ${col(C.green, '✓')}  ${result.count} sessions × ~${Math.round(totalTokens / valid.length)} tokens/session  ${col(C.gray, '≈ ' + totalTokens.toLocaleString() + ' total tokens')}\n`);
      return;
    }
...
`], { encoding: 'utf-8', timeout: 30_000 });
  ...
  console.log(`    purpclaw training --status          buffer size, last export, total tokens`);
  console.log(`    purpclaw training status          reward stats, by-route, by-skill breakdown`);
  console.log(`    purpclaw training export <jsonl|json|sharegpt|chatml> [--since=YYYY-MM-DD] [--until=YYYY-MM-DD]`);
  console.log(`    purpclaw training --from-sessions --days=N   export last N days as ChatML`);
