...
const COMMANDS = {
  ...
  chaos:     { run: async (args) => {
    ...
    if (sub === 'random') return JSON.stringify(sn.smith.randomAttack(), null, 2);
    if (sub === 'ledger') return JSON.stringify(sn.neo.ledger(), null, 2);
    if (sub === 'round') {
      const r = sn.smith.randomAttack();
      if (!r.ok) return 'attack failed';
      const def = sn.neo.stabilize({ content: r.corrupted.content || '' });
      return '⚔️ chaos round:\n  smith → ' + r.attack.technique + ' (' + r.attack.severity + ')\n  neo → ' + (def.stabilized ? '🛡️ stabilized' : '⚠️ detected (no auto-fix)');
    }
    if (sub === 'campaign') {
      const pack = rest[0] || 'output';
      const r = cc.runCampaign(pack);
      return r.message + '\n  detection: ' + r.campaign.results.detectionRate + '% · repair: ' + r.campaign.results.repairRate + '% · ' + r.campaign.results.avgResponseMs + 'ms avg';
    }
    if (sub === 'memory') {
      const r = mc.check();
      return '🧠 memory scan: ' + r.facts_scanned + ' facts · ' + r.findings.length + ' findings · ' + r.summary +
        (r.findings.length > 0 ? '\n' + r.findings.slice(0, 5).map(f => '  ' + f.severity + ': ' + f.type + ' — ' + f.reason).join('\n') : '');
    }
    if (sub === 'status') {
      const s = cc.status();
      return '🛡️ reliability: ' + s.totals.attacks + ' attacks · ' + s.totals.detected + ' detected · ' + s.totals.repaired + ' repaired';
    }
    if (sub === 'reset') return cc.reset().message;
    return 'usage: chaos inject <tech> | random | round | campaign <pack> | memory | status | ledger | reset\n  packs: output, memory, agent, provider';
  }, desc: 'Chaos engine: inject|random|round|campaign <pack>|memory|status|ledger|reset' },
  help:       { run: async () => {
