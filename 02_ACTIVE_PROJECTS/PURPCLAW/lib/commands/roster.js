...
async function run(args, ctx) {
  ...
  let canonical = null;
  try { canonical = require('../agent-registry').readRegistry(); } catch {}
  if (canonical && Array.isArray(canonical.agents)) {
    const agents = canonical.agents;
    // FIX 2026-07-08: "profile" used to mean `agent_profiles.json`, which is
    // archived and gives a misleading 35-missing count. The real persona
    // surface is agents/<key>.md. Check filesystem presence directly so the
    // roster reflects what actually exists on disk.
    const fs = require('fs');
    const path = require('path');
    const PURP_DIR_LOCAL = path.resolve(__dirname, '..', '..');
    const personaExists = (key) => fs.existsSync(path.join(PURP_DIR_LOCAL, 'agents', key + '.md'));

    const personaBacked = agents.filter(a => a.type === 'persona' && personaExists(a.key));
    const towerBacked = agents.filter(a => (a.sources || []).includes('agent_tower.js'));
    const missingPersona = towerBacked.filter(a => !personaExists(a.key));
    const personaOrphans = personaBacked.filter(a =>
      !(a.sources || []).includes('agent_tower.js')
    );

    if (wantJson) {
      console.log(JSON.stringify({
        total: canonical.total,
        by_type: canonical.by_type || {},
        by_division: canonical.by_division || {},
        source_counts: canonical.source_counts || {},
        source_audit: canonical.source_audit || {},
        missing_persona: missingPersona.map(a => a.key || a.name),
        persona_orphans: personaOrphans.map(a => a.key || a.name),
        agents,
      }, null, 2));
      return;
    }

    sectionHead('  AGENT ROSTER - CANONICAL REGISTRY');
    console.log(col(C.gray, `\n  Canonical total:       ${col(C.cyan, canonical.total)}`));
    console.log(col(C.gray, `  Source counts:         ${JSON.stringify(canonical.source_counts || {})}`));
    console.log(col(C.gray, `  By type:               ${JSON.stringify(canonical.by_type || {})}`));
    console.log(col(C.gray, `  Agent-like files seen: ${canonical.source_audit ? canonical.source_audit.files_seen : 'unknown'}`));
    console.log(col(C.gray, `  Tower without persona: ${col(missingPersona.length ? C.yellow : C.green, missingPersona.length)}`));
    console.log(col(C.gray, `  Persona without tower: ${col(personaOrphans.length ? C.yellow : C.green, personaOrphans.length)}\n`));

    if (wantMissing) {
      for (const a of missingPersona) console.log(`  ${col(C.cyan, String(a.key || a.name).padEnd(18))} ${col(C.gray, `${a.division || ''} ${a.role || ''}`)}`);
      return;
    }
    if (wantOrphans) {
      for (const a of personaOrphans) console.log(`  ${col(C.cyan, String(a.key || a.name).padEnd(24))} ${col(C.gray, a.file || '')}`);
      return;
    }

    for (const [division, count] of Object.entries(canonical.by_division || {}).sort((a, b) => b[1] - a[1])) {
      console.log(`  ${col(C.cyan, division.padEnd(18))} ${count}`);
    }
    console.log('');
    return;
  }
