...
const { whoamiFull } = require('../whoami');
...
async function tourIntro() {
  ...
  // Live counts — no hardcoded marketing numbers
  const liveStats = (() => {
    try {
      const tower = require('../../agent_tower');
      const TOOLS = require('../../lib/tools');
      const tools = TOOLS.list().length;
      const agents = tower.AGENT_TOWER ? Object.keys(tower.AGENT_TOWER.registry).length : 0;
      const providerKeys = ['MINIMAX_API_KEY','OPENROUTER_API_KEY','DEEPSEEK_API_KEY','NVIDIA_API_KEY','ANTHROPIC_API_KEY','OPENAI_API_KEY','KIMI_API_KEY','OLLAMA_HOST','GITHUB_MODELS_API_KEY'];
      const providers = providerKeys.filter(k => process.env[k] && process.env[k].length > 0).length;
      return `  ${providers} providers. ${tools} tools. ${agents} agents. Microservices running locally. 3 surfaces.\n`;
    } catch { return '  (live count unavailable — see /api/whoami)\n'; }
  })();
...
async function tourSurfaces() {
  ...
  speak('PurpClaw has three surfaces. CLI for quick one-shot commands. TUI for a full-screen terminal dashboard with live stats. And a WebUI mission control dashboard at port 3030.');
  ...
  console.log('  🖥 WebUI:  http://localhost:3030               — mission control\n');
...
async function tourProviders() {
  ...
  speak(`You have ${ready} providers ready. PurpClaw's provider stack is configurable — drop an API key in .env and it's picked up automatically. Switch between them without changing your workflow.`);
...
async function tourAgents() {
  ...
  // Live count from the tower registry — combines hardcoded animals (35)
  // + persona .md files loaded at boot (see agent_tower.js persona merge).
  const agentCount = (() => {
    try {
      const tower = require('../../agent_tower');
      return tower.AGENT_TOWER ? Object.keys(tower.AGENT_TOWER.registry).length : 0;
    } catch { return 0; }
  })();
  speak(`PurpClaw has ${agentCount} registered agents. Each has defined skills, goals, and protocols.`);
  console.log(`  🐝 ${agentCount} agents across divisions:\n`);
...
async function tourQuickstart() {
  ...
  speak('You are ready to use PurpClaw. Here is what to do next. Type purpclaw ask followed by your question. Try switching providers with the slash command. Launch the full screen TUI with purpclaw tui ask. Or open the WebUI in your browser at localhost port 3030.');
...
async function run(args, ctx) {
  ...
  async function tourProviders() {
    ...
    const found = setup.scanForKeys();
    const ready = Object.keys(found).length;
    speak(`${user.name}, you have ${ready} providers ready. PurpClaw's provider stack is configurable — drop an API key in .env and it's picked up automatically. Switch between them without changing your workflow.`);
...
async function tourAgents() {
  section('4. AGENTS — THE SWARM');
  // Live count from the tower registry — replaces the hardcoded "152".
  const agentCount = (() => {
    try {
      const tower = require('../../agent_tower');
      return tower.AGENT_TOWER ? Object.keys(tower.AGENT_TOWER.registry).length : 0;
  ...
  speak(`${user.name}, PurpClaw has ${agentCount} registered agents. Each has defined skills, goals, and protocols. You can spawn them from the chat with a slash command.`);
  console.log(`  🐝 ${agentCount} agents across divisions\n`);
