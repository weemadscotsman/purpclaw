...
const announce = {
  ...
  // ── Instruction Guard — enforcement ladder ───────────────────────────
  // Levels: nudge → warn → pause → block
  // Companion is the FACE of the guard; real enforcement is in the policy layer.
  guard: {
    // Nudge: gentle reminder that behaviour is drifting
    nudge:     (msg, payload = {}) => emit({ namespace: 'guard', action: 'nudge',     source: 'guard', payload: { message: msg, ...payload } }),
    // Warn: visible caution about an action in progress
    warn:      (msg, payload = {}) => emit({ namespace: 'guard', action: 'warn',      source: 'guard', payload: { message: msg, ...payload } }),
    // Pause: require user confirmation before continuing
    pause:     (msg, payload = {}) => emit({ namespace: 'guard', action: 'pause',     source: 'guard', payload: { message: msg, ...payload } }),
    // Block: stop the operation — violates explicit instruction
    block:     (msg, payload = {}) => emit({ namespace: 'guard', action: 'block',     source: 'guard', payload: { message: msg, ...payload } }),
    // Specific guard event types
    instruction_drift:    (from, to, payload = {}) => emit({ namespace: 'guard', action: 'instruction_drift',    source: 'guard', payload: { from, to, ...payload } }),
    scope_creep:          (added, payload = {})    => emit({ namespace: 'guard', action: 'scope_creep',          source: 'guard', payload: { added, ...payload } }),
    unauthorized_edit:    (path, payload = {})     => emit({ namespace: 'guard', action: 'unauthorized_edit',    source: 'guard', payload: { path, ...payload } }),
    destructive_action:    (action, payload = {})  => emit({ namespace: 'guard', action: 'destructive_action',    source: 'guard', payload: { action, ...payload } }),
    fake_success_claim:   (claim, payload = {})    => emit({ namespace: 'guard', action: 'fake_success_claim',   source: 'guard', payload: { claim, ...payload } }),
    missing_verification: (step, payload = {})    => emit({ namespace: 'guard', action: 'missing_verification', source: 'guard', payload: { step, ...payload } }),
    tool_loop:            (tool, count, payload = {}) => emit({ namespace: 'guard', action: 'tool_loop',   source: 'guard', payload: { tool, count, ...payload } }),
    provider_failure:     (provider, error, payload = {}) => emit({ namespace: 'guard', action: 'provider_failure', source: 'guard', payload: { provider, error, ...payload } }),
    task_mismatch:        (asked, attempted, payload = {}) => emit({ namespace: 'guard', action: 'task_mismatch', source: 'guard', payload: { asked, attempted, ...payload } }),
  },
