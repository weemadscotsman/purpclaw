...
class TrainingBuffer {
  ...
  summary() {
    ...
    for (const r of records) {
      const reward = Number.isFinite(Number(r.reward))
        ? Number(r.reward)
        : (r.job?.state === 'completed' ? 1 : (r.job?.state === 'failed' || r.job?.state === 'blocked' ? 0 : 0.5));
      const route = r.job?.route || 'unknown';
      const ok = reward >= 1.0;
      const fail = reward === 0.0;
    ...
    const rewardOf = (r) => Number.isFinite(Number(r.reward))
      ? Number(r.reward)
      : (r.job?.state === 'completed' ? 1 : (r.job?.state === 'failed' || r.job?.state === 'blocked' ? 0 : 0.5));
    return {
      total: records.length,
      success: records.filter(r => rewardOf(r) >= 1.0).length,
      failed: records.filter(r => rewardOf(r) === 0.0).length,
      partial: records.filter(r => rewardOf(r) > 0 && rewardOf(r) < 1.0).length,
