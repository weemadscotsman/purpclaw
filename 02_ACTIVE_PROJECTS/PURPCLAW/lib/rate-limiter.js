...
async function rateLimited(opts) {
  ...
  // Cap-watcher: TOCTOU-safe — capture state snapshot in each tick and guard
  // against the watcher firing when workers are still active (capHit trips mid-batch).
  const capWatcher = setInterval(() => {
    if (state.capHit && !state.active) {
      markRemaining();
      clearInterval(capWatcher);
    }
    if (!state.active && nextOrderIndex >= order.length) {
      clearInterval(capWatcher);
    }
  }, 200);

  // pump() can hang if runOne() throws before setting state.running = false.
  // Safety abort: if pump() doesn't finish in 30s, force state.running = false
  // and let the drain loop collect whatever workers completed.
  await Promise.race([
    pump(),
    sleep(30_000).then(() => { state.running = false; }),
  ]);
