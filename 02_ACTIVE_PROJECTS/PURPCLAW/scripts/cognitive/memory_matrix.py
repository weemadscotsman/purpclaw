...
class LongTermMemory:
    ...
    def __init__(self, storage_path: str = None):
        ...
        # Vectorised similarity cache. dequantize() does a 384x384 matmul plus a
        # .tolist() per call, and cosine_similarity() called it twice per atom —
        # ~15.5k matmuls and list round-trips to answer one search over 7.7k
        # atoms (measured 5.56s). Each atom is dequantized ONCE into a float32
        # matrix and searches become a single dot product.
        # ~12MB at 7.7k atoms (n x 384 x 4 bytes). Rebuilt when atoms change.
        self._vec_matrix = None      # np.ndarray (n, dims) float32
        self._vec_ids = None         # list[str] aligned to rows
        self._vec_norms = None       # np.ndarray (n,) float32
        self._vec_stamp = None       # change probe
    ...
    def _dequant_rows(self, items):
        """Dequantize (id, atom) pairs into aligned id/vector lists."""
        ids, rows = [], []
        for atom_id, atom in items:
            try:
    ...
    def _ensure_vec_cache(self, atoms_list):
        """Build/refresh the dequantized embedding matrix.

        Appends incrementally when atoms were only added (the common case — the
        stack ingests continuously). A full rebuild on every change made the
        cache useless: ~1.5s per search because something had always ingested
        since the last one.
        """
        stamp = (len(atoms_list),
    ...
    def search_similar(self, query_embedding: bytes, limit: int = 5,
        ...
        Vectorised: one dot product over a cached float32 matrix instead of a
        per-atom dequantize+cosine loop. Same cosine values, same ordering and
        same emotional_filter semantics, without 15.5k redundant matmuls.
        """
        with self.lock:
        ...
        self._ensure_vec_cache(atoms_list)
        matrix, ids, norms = self._vec_matrix, self._vec_ids, self._vec_norms
        if matrix is None or not ids:
            return []

        try:
            query = np.asarray(QuantizedMemory.dequantize(query_embedding), dtype=np.float32)
        except Exception:
            return []
