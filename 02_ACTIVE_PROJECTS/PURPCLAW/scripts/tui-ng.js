...
let state = {
  ...
  turns: 0,
  services: {},
  agents: { active: 0, registered: 152 },
  mcpTools: 0,
  lastPoll: 0,
};
...
async function poll() {
  ...
  try {
    const r = await fetch('http://127.0.0.1:7790/tower/status'); if (r.ok) {
      ...
      state.agents.registered = j.totalRegistered || 0;  // whoami will populate if available
  ...
  // v2.1 — Pulse: live stack heartbeat (notifications feed)
  try {
    const r = await fetch(API + '/api/pulse/notifications?limit=5');
    if (r.ok) { const j = await r.json(); state.pulse = j.notifications || []; }
  } catch {}

  // v2.1 — Whoami: live tool/agent/provider counts (replaces hardcoded numbers)
  try {
    const r = await fetch(API + '/api/whoami');
    if (r.ok) {
      const w = await r.json();
      state.whoami = w;
      // Surface the breakdown into the state shape the UI already uses
      if (w.systems && w.systems.tools) {
        state.toolCount = w.systems.tools.total || 0;
        state.toolsBreakdown = w.systems.tools.breakdown || {};
      }
      if (w.systems && w.systems.agents) {
        state.agents.registered = w.systems.agents.count || state.agents.registered;
      }
      if (w.systems && w.systems.providers) {
        state.providerCount = w.systems.providers.count || 0;
        state.providersPresent = w.systems.providers.present || [];
      }
    }
  } catch {}
...
inputBox.on('submit', async (text) => {
  ...
  if (cmd === '/agents') {
    chatLog.add('{yellow-fg}registered: ' + state.agents.registered + ' agents across 9 divisions ● active: ' + state.agents.active + '{/}');
    inputBox.clearValue(); inputBox.focus(); return;
  }
