...
function startBackend() {
  ...
  // spine.py uses package-relative imports — it must run as a module from the
  // repo root, not as a bare script path.
  const repoRoot = path.join(ROOT, '..', '..');
  backend = spawn(PYTHON_BIN, ['-m', 'services.cognitive.spine', '--port', String(BACKEND_PORT)], {
    cwd: repoRoot,
    windowsHide: true,
    env: {
      ...process.env,
