...
require('./lib/runtime/telemetry-console').installConsoleTelemetry('purpclaw-pool');
...
// ── Persistence toggle ────────────────────────────────────────────────────────
// Set PURPCLAW_POOL_PERSIST=0 to run pool fully in-memory.
// All writes become no-ops, reads return empty. Pool stays functional
// as an in-process skill/agent index without touching disk.
const PERSIST = process.env.PURPCLAW_POOL_PERSIST !== '0';
...
function loadIndex() {
  if (!PERSIST) {
    // In-memory mode: start with empty indexes, skip all disk I/O
    console.log('[POOL] PERSIST=0 — running in-memory, indexes start empty');
    rebuildIndex();
    return;
  }
...
const ROUTES = {
  ...
  'GET /pool/recent': (query) => {
    ...
    if (!PERSIST || !fs.existsSync(QUERY_LOG_FILE)) return { entries: [], count: 0 };
  ...
  'POST /pool/memory/append': (query, params, body) => {
    ...
    if (!PERSIST) return { ok: false, mode: 'in-memory', note: 'PURPCLAW_POOL_PERSIST=0 — write skipped' };
