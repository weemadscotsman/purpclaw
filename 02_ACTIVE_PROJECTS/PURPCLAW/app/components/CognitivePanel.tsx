...
export function CognitivePanel() {
  ...
  const runRulesQuery = useCallback(async () => {
    ...
    try {
      const res = await fetch(proxyUrl(7880, '/rules/query'), {
        method: 'POST',
      ...
      setRulesResult({ error: 'Cognitive spine rules endpoint offline' });
    }
  }, [rulesQuery]);

  const runRulesInfer = useCallback(async () => {
    try {
      const res = await fetch(proxyUrl(7880, '/rules/infer'));
      const proxied = await res.json();
      setRulesResult(proxied.data ?? proxied);
    } catch (e) {
      setRulesResult({ error: 'Cognitive spine rules endpoint offline' });
  ...
  const runMemoryRecall = useCallback(async () => {
    ...
    try {
      const res = await fetch(proxyUrl(7880, '/memory/recall'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    try {
      const [diagRes, voteRes] = await Promise.all([
        fetch(proxyUrl(7880, '/diagnostics/diagnose'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ source: 'cognitive-panel' }),
        }),
        fetch(proxyUrl(7880, '/diagnostics/vote')),
      ]);
      const diagProxy = await diagRes.json();
      const voteProxy = await voteRes.json();
      const diagData = diagProxy.data ?? diagProxy;
      const rawVoteData = voteProxy.data ?? voteProxy;
      const voteData = {
        tally: rawVoteData.tally || rawVoteData.vote_tally || {},
        leading: rawVoteData.leading || rawVoteData.leading_cause || null,
      };
      setDiagResult({ ...diagData, votes: voteData });
    } catch (e) {
      setDiagResult({ error: 'Cognitive spine diagnostics offline' });
    } finally {
  ...
  const fetchCausalGraph = useCallback(async () => {
    try {
      const res = await fetch(proxyUrl(7880, '/diagnostics/causal-graph/dot'));
      const proxied = await res.json();
      const data = proxied.data ?? proxied;
      setCausalDot(typeof data === 'string' ? data : (data.dot || '// Causal graph unavailable'));
      setShowCausal(true);
    } catch (e) {
