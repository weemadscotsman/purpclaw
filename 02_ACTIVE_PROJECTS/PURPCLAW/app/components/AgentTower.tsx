...
export function AgentTower() {
  ...
  const proxyUrl = (port: number, path: string, soft = true) =>
    `/api/service-proxy?port=${port}&path=${encodeURIComponent(path)}${soft ? '&soft=1' : ''}`;
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      try {
        const res = await fetch(proxyUrl(7780, '/api/health'));
        if (res.ok) {
    ...
    const checkServices = async () => {
      const serviceList = [
        ...
        { name: 'Voice', port: 8781, path: '/health' },
        { name: 'Bridge', port: 7792, path: '/health' },
      ...
      const results = await Promise.all(serviceList.map(async (svc) => {
        ...
        try {
          const res = await fetch(proxyUrl(svc.port, svc.path), { signal: AbortSignal.timeout(2500) });
          const body = await res.json().catch(() => null);
          return { ...svc, status: res.ok && body?.status === 'online' ? 'healthy' : 'down', latency: Date.now() - start } as ServiceInfo;
  ...
  const refreshAgents = async () => {
    try {
      const res = await fetch(proxyUrl(7790, '/tower/status'));
      if (res.ok) {
        const data = unwrapProxy(await res.json());
  ...
  const spawnAgent = async (name: string) => {
    ...
    try {
      await fetch(proxyUrl(7790, '/api/spawn', false), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
