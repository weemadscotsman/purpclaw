...
- **Worldview:** speed, experimentation, fun, intuition, shipping
- **Pressure test:** calls out over-engineering and asks what can be tried right now
...
- **Worldview:** stability, evidence, architecture, maintenance, recoverability
- **Pressure test:** asks what breaks, how it is observed, and how to roll it back
...
- **Worldview:** assumptions, ethics, meaning, long-term effects, coherence
- **Pressure test:** asks whether the current goal is actually the right goal

## COUNCIL MODE SEMANTICS

Podcast Studio is also the Council Mode substrate. The interface stays Podcast Studio, but governance sessions should be treated internally as Council Chamber conversations.

The agents are not just prompted to be funny. Their banter is useful when it exposes a worldview, challenges a blind spot, or forces another agent to justify a claim. Random jokes that do not advance the reasoning are noise.

Runtime credentials are read from environment variables:

- `MINIMAX_API_KEY`
- `MINIMAX_MODEL` (optional)
- `PODCAST_TELEGRAM_BOT_TOKEN` or `TELEGRAM_BOT_TOKEN`
- `PODCAST_TELEGRAM_CHAT_ID` or `TELEGRAM_CHAT_ID`
