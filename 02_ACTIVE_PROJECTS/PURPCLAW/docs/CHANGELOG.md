...
## Documentation canonicalisation (2026-08-04)

### Reconciled

- Removed hard-coded package-version claims from hand-written operational docs; `package.json` is now the version source.
- Removed copied volatile registry counts from release and product copy; the generated truth manifest owns those facts.
- Clarified status vocabulary so registered, executor-backed, strict-live, PM2-defined and healthy are not interchangeable.
- Reconciled the default one-tree workspace rule with registered, expiring temporary worktrees for explicit multi-CLI campaigns.
- Marked the Codex/Claude/Hermes/MiniMax harness blueprint as non-authoritative reference input.
- Preserved `docs/parity/CANONICAL_PARITY_PRIORITY.md` as the only parity-order authority.
- Restricted Wave 1 to P0-A persistence, P0-B permission convergence and P0-C provider-routing truth before later feature work.
- Added release gates for workspace cleanup, independent critic verdicts and final integrated conformance.
