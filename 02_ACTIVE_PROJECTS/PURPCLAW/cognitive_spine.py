...
# Make `scripts/` importable so `import mem_guard` resolves. The watchdog
# is the only thing standing between a leak in any of the heavy cognitive
# subsystems (memory_matrix, faiss index, embeddings_index) and a 5 GB
# zombie that PM2 max_memory_restart doesn't see (this is a child process
# of cognitive_gateway.js, not a directly managed PM2 service).
...
try:
    ...
    mem_guard.install(
        label="cognitive-spine",
        limit_mb=_SPINE_MEM_LIMIT_MB,
        interval_s=_SPINE_MEM_INTERVAL_S,
    )
except Exception as _mem_exc:
    # Loud-not-silent: the watchdog is the only thing protecting the host
    # from a runaway python child. If it can't arm, say so.
    sys.stderr.write(f"[cognitive-spine] FATAL: mem_guard unavailable: {_mem_exc}\n")
    sys.stderr.write("[cognitive-spine] FATAL: running WITHOUT memory watchdog — fix scripts/ path or install psutil\n")
    sys.stderr.flush()
...
class ReuseThreadingServer(ThreadingTCPServer):
    ...
    def process_request(self, request, client_address):
        ...
        try:
            request.settimeout(float(os.environ.get("COGNITIVE_SOCKET_TIMEOUT_S", "3")))
...
class SpineHandler(BaseHTTPRequestHandler):
    ...
    # Loading the memory matrix takes a long time (thousands of atoms, embeddings).
    # The server now binds and answers BEFORE that finishes, so callers get a fast,
    # honest 503 instead of ECONNREFUSED against a socket that does not exist yet.
    def _warming(self, path):
        ...
        if path in ("/health", "/cognitive/health"):
            self.send_json({
                "status": "warming",
                "service": "cognitive_spine",
                "detail": "memory matrix still loading; not ready to serve",
                "ready": False,
            }, status=503)
            return True
        self.send_json({
            "ok": False,
            "error": "cognitive_backend_warming",
            "detail": "memory matrix still loading; retry shortly",
            "ready": False,
        }, status=503)
        return True
    ...
    def spine_health_cached(self):
        ...
        if cached is not None:
            return {
                ...
                "service": "cognitive_spine",
                "port": PORT,
                "uptime": cached.get("uptime", 0),
                "services": {
                    k: (
                        {"status": v.get("status", "unknown"), "service": v.get("service", k)}
                        if isinstance(v, dict) else {"status": "unknown", "service": k}
                    )
                    for k, v in (cached.get("services") or {}).items()
                },
            }
...
def main():
    ...
    # Bind and serve BEFORE building CognitiveState. Constructing the memory
    # matrix loads and embeds thousands of atoms; doing that first meant the
    # socket did not exist for the whole warm-up, so every caller got
    # ECONNREFUSED and the gateway reported cognitive_backend_unavailable while
    # the process sat there apparently healthy. Requests during warm-up now get
    # a 503 "warming" from _warming() instead.
    with ReuseThreadingServer(("127.0.0.1", PORT), SpineHandler) as server:
        server_thread = _th.Thread(target=server.serve_forever, name="spine-http", daemon=True)
        server_thread.start()
        print("[CognitiveSpine] listening on 127.0.0.1:" + str(PORT) + " (warming)", flush=True)

        t0 = time.time()
        STATE = CognitiveState()
        print("[CognitiveSpine] ready in %.1fs" % (time.time() - t0), flush=True)
