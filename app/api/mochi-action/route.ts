...
const AUTODREAM_URL = 'http://127.0.0.1:7880/autodream';
...
async function syncPoolMemory(row: Record<string, unknown>) {
  try {
    const mod: any = await import('../../../lib/canonical-memory-sync.js');
    const syncRecord = mod.syncRecord || mod.default?.syncRecord;
    if (!syncRecord) return null;
    return await syncRecord(process.cwd(), 'pool-memory', row);
  } catch {
    return null;
  }
}
...
export async function POST(request: NextRequest) {
  ...
  if (action === 'feed') {
    ...
    try {
      fallback = await call(`${POOL_URL}/pool/memory/append`, 'POST', fallbackMemory);
      if (fallback?.ok) await syncPoolMemory(fallbackMemory);
    } catch (e: any) {
  ...
  if (action === 'sleep') {
    ...
    const resolvedDreamMsg = dreamResult?.dedup?.skipped
      ? `below threshold - dream skipped (${dreamResult.dedup.skipped})`
      : dreamResult?.rules?.rulesExtracted !== undefined
        ? `dream cycle complete - ${dreamResult?.dedup?.merged ?? 0} merged, ${dreamResult.rules.rulesExtracted ?? 0} rules extracted`
        : dreamMsg;
    ...
    const finalMsg = mem?.ok && resolvedDreamMsg
      ? `${resolvedDreamMsg} - ${failCount} failure patterns reviewed`
      : msg;
