...
const ARCH_LINKS = [
  ...
  // Voice chain
  {
    source: "svc:voice-bridge",
    target: "core",
    color: "#f472b6",
    opacity: 0.5,
  },
  {
    source: "svc:voice-coordinator",
    target: "svc:api",
    color: "#f472b6",
    opacity: 0.5,
  },
  {
    source: "svc:voice-ingress",
    target: "svc:orchestrator",
    color: "#f472b6",
    opacity: 0.5,
  },
  // Dark / optional
  {
    source: "svc:reasoning",
    target: "svc:eventbus",
    color: "#a78bfa",
    opacity: 0.3,
  },
...
// Bolt ⚡: Memoize unique agents list by reference to prevent redundant O(N) mapping.
const uniqueAgentsCache = new WeakMap<
  MissionData["agents"],
  MissionData["agents"]
>();
...
export function LiveSystemMap({ data }: { data: MissionData }) {
  ...
  const { nodes, links } = useMemo(() => {
    ...
    const addNode = (n: GraphNode) => {
      ...
      nodes.push({
        ...n,
        kind: n.kind,
        color: n.color ?? cfg.color,
        size: n.size ?? cfg.size,
    ...
    for (const conn of data.observedConnections ?? []) {
      ...
      const topicColor = conn.topic.startsWith("orchestrator.")
        ? "#f97316"
        : conn.topic.startsWith("agent.") || conn.topic.startsWith("tower.")
          ? "#34d399"
          : conn.topic.startsWith("kernel.") ||
              conn.topic.startsWith("harness.")
            ? "#a78bfa"
            : "#38bdf8";
    ...
    const divisions = [
      ...new Set(agents.map((a) => a.division || "UNASSIGNED")),
    ];
    for (const d of divisions) {
      ...
      addNode({
        ...
        label: d,
        kind: "division",
        color: KIND_CONFIG.division.color,
        active: working > 0,
        size: KIND_CONFIG.division.size,
      });
    ...
    for (const a of agents) {
      ...
      addLink(
        id,
        `div:${a.division || "UNASSIGNED"}`,
        working ? "#34d399" : "#38bdf8",
        working ? 0.5 : 0.15,
      );
    }
    ...
    for (const w of data.pipeline?.active ?? []) {
      ...
      addNode({
        ...
        label: w.intent || w.id,
        kind: "workflow",
        color: KIND_CONFIG.workflow.color,
        active: true,
        size: KIND_CONFIG.workflow.size,
      });
  ...
  const counts = useMemo(() => {
    ...
    return {
      ...
      divisions: new Set(agents.map((a) => a.division || "UNASSIGNED")).size,
  ...
  const handleNodeHover = useCallback(
    (node: any | null, prevNode: any | null) => {
      if (node) {
  ...
  return (
    ...
    >
      ...
      <ForceGraph3D
        ...
        linkOpacity={useCallback(
          (link: any) => (link.observed ? 0.9 : (link.opacity ?? 0.4)),
          [],
        )}
        linkDirectionalParticles={useCallback(
          (link: any) =>
            link.observed ? Math.min(6, 2 + (link.weight ?? 1)) : 0,
          [],
        )}
        linkDirectionalParticleSpeed={useCallback(() => 0.005, [])}
        linkDirectionalParticleWidth={useCallback(
          (link: any) =>
            link.observed ? Math.min(3, 1 + (link.weight ?? 1) * 0.5) : 0,
          [],
        )}
        linkDirectionalParticleColor={useCallback(
          (link: any) => link.color ?? "#f97316",
          [],
        )}
      ...
      <div className="absolute left-3 top-3 z-20 max-w-xs border border-white/10 bg-black/70 px-3 py-2 font-mono text-[9px] leading-relaxed">
        <div className="mb-1 text-[8px] uppercase tracking-[0.28em] text-cyan-300/60">
          Live System Map
        </div>
        <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-white/45">
          <span className="text-emerald-300">
            {counts.services}/{counts.totalServices}
          </span>
        ...
        <div className="mt-1.5 space-y-0.5 text-white/40">
          ...
          <div className="flex items-center gap-1.5">
            <span className="inline-block h-0.5 w-4 bg-cyan-400/40" />
            arch contract
          </div>
        </div>
      </div>
          ...
          return (
            <div className="absolute right-3 top-3 z-20 border border-white/10 bg-black/70 px-3 py-2 font-mono text-[9px]">
              ...
              <div className="mt-0.5 text-white/30">
                {node.kind} · {node.active ? "active" : "inactive"}
              </div>
              {node.sub && (
                <div className="mt-0.5 text-white/20">{node.sub}</div>
              )}
              <div className="mt-1 border-t border-white/10 pt-1 text-white/20">
