...
export function OverviewPanel({ data }: { data: MissionData }) {
  ...
  const criticalLanes = useMemo(() => (rival?.lanes || []).filter((lane: any) => lane.priority === 1).slice(0, 5), [rival?.lanes]);
  const epsHistory = useEPSHistory(60);
  const chartData = useMemo(() => epsHistory.map((p, i) => ({ i, eps: p.eps })), [epsHistory]);
  ...
  const startResearchRoom = async () => {
    ...
    setResearchMessage('');
    try {
      const res = await fetch('/api/research/group', {
        method: 'POST',
      ...
      if (!res.ok || payload.ok === false) throw new Error(payload.error || 'research room failed to start');
