...
import { Onboarding, ONBOARD_KEY } from './Onboarding';
...
export function MissionControl({ data }: { data: MissionData }) {
  ...
  const [showOnboarding, setShowOnboarding] = useState(() => {
    try {
      return typeof window !== 'undefined' && !localStorage.getItem(ONBOARD_KEY);
    } catch {
      return false;
    }
  });
  ...
  return (
    <div className="h-screen bg-[#030508] text-white relative overflow-hidden flex flex-col">
      ...
      <header className="relative z-10 flex shrink-0 items-center justify-between gap-3 px-3 md:px-4 h-14 border-b border-cyan-500/10 bg-black/50 backdrop-blur-2xl">
        ...
        {/* Center â€” system vitals */}
        <div className="hidden lg:flex items-center gap-4">
          <VitalBadge label="Services" value={`${serviceCountLabel(data.services).online}/${serviceCountLabel(data.services).total}`} color="#22c55e" />
          <div className="w-px h-5 bg-white/10" />
          <VitalBadge label="Agents" value={uniqueAgentCount} sub="/R" color="#00d4ff" />
          <div className="w-px h-5 bg-white/10" />
...
function DrawerOverlay({ tab, data, onClose, iframeRef }: { tab: TabId; data: MissionData; onClose: () => void; iframeRef?: React.RefObject<HTMLIFrameElement | null> }) {
  ...
  return (
    <>
      ...
      {/* Drawer panel */}
      <div
        className={`absolute left-20 top-0 bottom-0 z-50 flex flex-col border-r border-white/8 shadow-2xl transition-all duration-500 ${
          isDream 
            ? 'w-96 bg-[#040a10]/80 backdrop-blur-md' 
            : 'w-[min(88vw,900px)] bg-[#040a10]/96 backdrop-blur-2xl'
        }`}
...
function PanelContent({ tab, data, iframeRef }: { tab: TabId; data: MissionData; iframeRef?: React.RefObject<HTMLIFrameElement | null> }) {
  const content = (() => {
    switch (tab) {
      ...
      case 'timeline': return <EventTimelinePanel />;
      case 'sampler': return <SamplerPanel />;
      case 'gatekeeper': return <GatekeeperPanel />;
...
function routingValue(data: MissionData): string {
  const primary = data.llmStatus?.provider?.provider;
  const swarm = data.llmStatus?.swarm?.provider;
...
function DelegationRoutingLens({ data }: { data: MissionData }) {
  ...
  const primary = llm?.provider;
  const swarm = llm?.swarm;
