...
const TABS: Tab[] = [
  {
    ...
    stage: "start",
    purpose:
      "Start here. See the whole system, then launch or inspect the latest job.",
  },
  {
    id: "command",
    label: "Control Room",
    ...
    stage: "start",
    purpose:
      "Talk to the stack directly: chat, API command, kernel job, mission, agent, or research room.",
  },
  {
    id: "harness",
    ...
    stage: "build",
    purpose:
      "Run and inspect autonomous harness missions with verification gates.",
  },
  {
    id: "agents",
    ...
    stage: "build",
    purpose:
      "See which specialist agents exist, what they are doing, and where work is stuck.",
  },
  {
    id: "tower",
    label: "Tower State",
    ...
    stage: "build",
    purpose:
      "Manage spawned agents and the tower runtime that executes direct assignments.",
  },
  {
    id: "swarm",
    ...
    stage: "build",
    purpose:
      "Inspect swarm delegation: who got the work, what happened, and what failed.",
  },
  {
    id: "pipeline",
    label: "Workflow Flow",
    icon: "WF",
    stage: "observe",
    purpose: "Follow workflow state from queued to active to archived.",
  },
  {
    id: "timeline",
    label: "Event Lens",
    icon: "EL",
    stage: "observe",
    purpose: "Read the event timeline when you need exact runtime history.",
  },
  {
    id: "sampler",
    label: "Live Metrics",
    ...
    stage: "observe",
    purpose:
      "Sampler-style live dashboards from shell metrics (config/samplers.yml).",
  },
  {
    id: "logs",
    label: "Raw Signals",
    icon: "LG",
    stage: "observe",
    purpose: "Drop to raw signal logs when the pretty panels are not enough.",
  },
  {
    id: "dream",
    label: "Dream Swarm",
    icon: "DR",
    stage: "observe",
    purpose: "Live altered-states WebGL swarm telemetry visualizer.",
  },
  {
    id: "gatekeeper",
    label: "Risk Gate",
    icon: "GK",
    stage: "control",
    purpose: "Check safety gates, approvals, and blocked risky operations.",
  },
  {
    id: "abliterator",
    label: "Abliterator",
    icon: "AB",
    stage: "control",
    purpose: "OBLITERATUS refusal weight excision and red-team sandbox.",
  },
  {
    id: "cognitive",
    ...
    icon: "CG",
    stage: "control",
    purpose: "Use memory, rules, diagnostics, and reasoning lenses.",
  },
  {
    id: "evolution",
    ...
    stage: "control",
    purpose:
      "Track the human-steers, harness-builds, loop-improves learning cycle.",
  },
  {
    id: "graph",
    label: "System Map",
    ...
    stage: "control",
    purpose:
      "Map system relationships across services, agents, workflows, and events.",
  },
  {
    id: "mochi",
    label: "Asher",
    ...
    stage: "start",
    purpose:
      "Talk to your thringlet companion. Live narrator over every event, job, and agent action.",
  },
];
...
function serviceCountLabel(services: MissionData["services"]) {
  ...
  const all = services || [];
  const online = all.filter((service) =>
    serviceReachable(service.status),
  ).length;
...
type SurfaceCapability = {
  ...
  cli: string[];
  tui: string[];
  web: { route: string; mode: string; api: string };
};
...
function HeaderMochi({
  ...
  useEffect(() => {
    fetch("/api/mochi")
      ...
      .then((m) => {
        ...
        const eye = m.eye ?? "·";
        const fn =
          MOCHI_FACES[m.species as string] ?? ((e: string) => `(${e}${e})`);
        setFace(fn(eye));
        setName(m.name ?? "");
      })
      .catch(() => {});
  ...
  return (
    ...
    >
      ...
      <span
        className={`text-[9px] font-mono text-white/30 ${compact ? "hidden" : "hidden md:inline"}`}
      >
        {name}
      </span>
      <span className="text-[8px] font-mono text-fuchsia-300/60 hidden md:inline">
        {isOpen ? "▾" : "▴"}
      </span>
    </button>
  );
...
function OperatorSessionSelector() {
      ...
      .then((d) => {
        if (d && d.success) {
          ...
          window.dispatchEvent(
            new CustomEvent("operator-changed", { detail: d.name }),
          );
        }
      })
      .catch(() => {});
  ...
  return (
    ...
    >
      ...
      <span
        className="text-[9px] font-mono font-black text-cyan-300"
...
export function MissionControl({ data }: { data: MissionData }) {
  const [openDrawer, setOpenDrawer] = useState<TabId | null>(null);
  ...
  useEffect(() => {
    ...
    if (agentName) {
      const regAgent = data.agents.find(
    ...
    if (
      typeLower === "agent.spawned" ||
      typeLower === "agent.spawn" ||
      typeLower === "agent_spawned" ||
      typeLower === "agent"
    ) {
      eventType = "spawn";
      ...
      typeLower === "agent.completed" ||
      typeLower === "agent.complete" ||
      typeLower === "agent_complete" ||
      (typeLower === "system" && msgLower.includes("completed"))
    ) {
      eventType = "complete";
    } else if (
      ...
      typeLower === "chorus" ||
      (typeLower === "system" && latestLog.source !== "TOWER")
    ) {
      eventType = "chorus";
      ...
      if (
        msgLower.includes('"speaking":false') ||
        msgLower.includes("speaking: false")
      ) {
  ...
  return (
    <div className="h-full min-h-0 bg-[#030508] text-white relative overflow-hidden flex flex-col">
      ...
      <header className="relative z-10 flex shrink-0 items-center justify-between gap-3 px-3 md:px-4 h-10 border-b border-cyan-500/10 bg-black/35 backdrop-blur-xl">
        ...
        <div className="flex min-w-0 items-center gap-3 md:gap-4">
          <div className="flex min-w-0 items-center gap-3">
            <span className="hidden sm:inline truncate text-[10px] font-black tracking-[0.22em] text-white/80 font-mono uppercase">
              Mission Workspace
            </span>
            <div className="hidden sm:block w-px h-4 bg-cyan-500/30" />
            <span className="hidden lg:inline text-[9px] tracking-[0.22em] text-cyan-400/55 font-mono uppercase">
              Live command lens
            </span>
          </div>
          <div className="hidden sm:flex items-center gap-1.5 ml-2">
            ...
            />
            <span className="text-[9px] text-emerald-400/50 font-mono tracking-widest">
              LIVE
            </span>
          </div>
        </div>

        {/* Center --” system vitals (authoritative totals from /api/manifest) */}
        <div className="hidden lg:flex items-center gap-4">
          {/* online (live health) / total (manifest source of truth) */}
          <VitalBadge
            label="Services"
            value={`${serviceCountLabel(data.services).online}/${data.manifest?.services ?? serviceCountLabel(data.services).total}`}
            color="#22c55e"
          />
          ...
          <VitalBadge
            label="Agents"
            value={`${uniqueAgentCount}/${data.manifest?.agents ?? uniqueAgentCount}`}
            sub={data.manifest ? `${data.manifest.divisions.length}div` : "/R"}
            color="#00d4ff"
          />
          <div className="w-px h-5 bg-white/10" />
          <VitalBadge
            label="Tools"
            value={data.manifest?.tools ?? 0}
            color="#f59e0b"
          />
          <div className="w-px h-5 bg-white/10" />
        ...
        <div className="flex shrink-0 items-center gap-2 md:gap-4">
          <div className="hidden xl:flex items-center gap-3">
            {[
              ...
              <div key={label} className="flex items-center gap-1.5">
                <div
                  className={`w-1.5 h-1.5 rounded-full ${ok ? "bg-emerald-400" : "bg-rose-500"}`}
                  style={{
                    boxShadow: ok
          ...
          <button
            onClick={() =>
              window.open(
                `${window.location.origin}/mission?chrome=minimal`,
                "_blank",
      ...
      {/* Chat fills the whole remaining space */}
      <main className="relative z-10 flex-1 min-h-0 overflow-hidden pl-20">
        <ErrorBoundary>
      ...
      <MochiFloat
        data={data}
        open={mochiFloatOpen}
        onClose={() => setMochiFloatOpen(false)}
        onExpand={openMochiFullView}
      />

      {/* First-run onboarding / how-it-works (reopen from header ?) */}
      {showOnboarding && (
        <Onboarding data={data} onClose={() => setShowOnboarding(false)} />
      )}
    </div>
  );
}
  ...
  return (
    <div
      ...
      style={{
        ...
        top: pos.y,
        background:
          "linear-gradient(135deg, rgba(10,6,20,0.97) 0%, rgba(30,8,50,0.95) 100%)",
        border: "1px solid rgba(217,70,239,0.25)",
        boxShadow:
          "0 8px 40px rgba(0,0,0,0.6), 0 0 24px rgba(217,70,239,0.12), inset 0 1px 0 rgba(255,255,255,0.05)",
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          <span
            className="text-sm font-mono text-fuchsia-300"
            style={{ textShadow: "0 0 10px rgba(217,70,239,0.6)" }}
          >
            {"<✦~✦>"}
          </span>
          <div>
            <div className="text-[11px] font-bold text-white/85">Asher</div>
            <div className="text-[8px] font-mono text-fuchsia-300/40">
              dragon · narrator
            </div>
          </div>
        </div>
  ...
  switch (tab) {
    case "overview":
      return [
        {
          label: "Services",
          value: `${serviceCounts.online}/${serviceCounts.total}`,
          ok: serviceCounts.online === serviceCounts.total,
        },
        { label: "Agents", value: `${agents.length} registered`, ok: true },
        {
          label: "Last job",
          value: latest?.state || "none",
          ok: latest?.state !== "failed",
        },
      ];
    case "agents":
      return [
    ...
    case "tower":
      return [
        {
          label: "Tower",
          value: data.towerConnected ? "online" : "offline",
          ok: data.towerConnected,
        },
    ...
    case "swarm":
      return [
        {
          label: "Missions",
          value: `${data.kernelJobs?.filter((j) => j.route === "swarm-coordinator").length || 0}`,
          ok: true,
        },
    ...
    case "pipeline":
      return [
        {
          label: "Active",
          value: `${data.pipeline?.active?.length || 0}`,
          ok: true,
        },
        {
          label: "Queued",
          value: `${data.pipeline?.queue?.items?.length || 0}`,
          ok: true,
        },
        {
          label: "Done",
          value: `${data.pipeline?.completed?.length || 0}`,
          ok: true,
        },
      ];
    case "timeline":
      return [
        ...
        {
          label: "Latest",
          value: (data.logs[0] as any)?.type || "none",
          ok: true,
        },
      ];
    case "harness":
      return [
        {
          label: "Bench",
          value: `${Math.round(((data.harnessBenchmarks as any)?.summary?.completionRate || 0) * 100)}% pass`,
          ok: true,
        },
        {
          label: "Scored",
          value: `${data.agentScores?.meta?.totalTasksRecorded || 0} outcomes`,
          ok: true,
        },
      ];
    case "gatekeeper":
      return [
        {
          label: "Gatekeeper",
          value: data.apiConnected ? "active" : "offline",
          ok: data.apiConnected,
        },
        {
          label: "Repair",
          value: (data.omnicodeStatus as any)?.destructiveRepairBlocked
            ? "blocked"
            : "open",
          ok: true,
        },
    ...
    case "cognitive":
      return [
        {
          label: "Memory",
          value: data.apiConnected ? "live" : "offline",
          ok: data.apiConnected,
        },
      ];
    case "logs":
      return [
        { label: "Signals", value: `${data.logs.length}`, ok: true },
        {
          label: "Errors",
          value: `${data.agents.filter((a) => a.status === "error").length}`,
          ok: data.agents.filter((a) => a.status === "error").length === 0,
        },
      ];
    case "mochi":
      return [
        {
          label: "Pet",
          value: data.apiConnected ? "online" : "offline",
          ok: data.apiConnected,
        },
    ...
    default:
      return [{ label: tab, value: "see panel", ok: true }];
  }
}

function FloatingTabRail({
  openDrawer,
  onToggle,
  data,
  onMochiToggle,
        ...
        {stages.map((stage) => {
          ...
          return (
            <React.Fragment key={stage}>
              <div
                className={`text-center text-[7px] uppercase tracking-[0.18em] font-mono px-1 py-0.5 ${stageColors[stage]} opacity-55`}
              >
              ...
              {tabs.map((tab) => {
                ...
                const preview =
                  hovered === tab.id ? tabPreviewData(tab.id, data) : null;
                return (
                  <div
                    key={tab.id}
                    className="relative"
                  ...
                  >
                    ...
                    {preview && (
                      ...
                      >
                        <div className="text-[8px] uppercase tracking-[0.24em] font-mono text-white/35 mb-2">
                          {tab.label}
                        </div>
                        <div className="text-[10px] font-mono text-white/40 mb-2 leading-4">
                          {tab.purpose}
                        </div>
                        <div className="space-y-1.5">
                          {preview.map((item, i) => (
                            ...
                            >
                              ...
                              <span
                                className={`text-[10px] font-mono font-bold ${item.ok !== false ? "text-white/75" : "text-amber-300"}`}
                              >
                                {item.value}
                              </span>
                            </div>
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          animation: "slideInLeft 200ms cubic-bezier(0.16,1,0.3,1) forwards",
        }}
      >
        {/* Drawer header */}
        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-white/8 bg-black/40 px-5 py-3">
          <div className="flex items-center gap-3">
            <span className="rounded border border-cyan-300/20 bg-black/40 px-2 py-1 text-[9px] font-mono text-cyan-100/65">
              {TAB_CODES[tab]}
            </span>
...
function FlowRibbon({ data }: { data: MissionData }) {
  const latest = data.kernelJobs[0];
  const activeAgents = getUniqueAgents(data.agents).filter(
  ...
  const steps = [
    {
      label: "Hello",
      detail: data.apiConnected ? "API listening" : "API offline",
      active: data.apiConnected,
    },
    {
      label: "Kernel",
      detail: latest?.id || "waiting",
      active: Boolean(latest),
    },
    {
      label: "Job",
      detail: latest?.state || "none",
      active: Boolean(latest && latest.state !== "failed"),
    },
    {
      label: "Swarm",
      detail: latest?.linkedMissionId || `${activeFlows} workflows`,
      active: Boolean(latest?.linkedMissionId || activeFlows),
    },
    {
      label: "Agents",
      detail: `${activeAgents} working`,
    ...
    },
    {
      ...
      detail: latest?.finishedAt
        ? "closed"
        : latest?.state === "delegated"
          ? "delegated"
          : "pending",
      active: Boolean(latest?.finishedAt || latest?.state === "delegated"),
    },
  ];
  ...
  return (
    <section className="relative z-10 shrink-0 border-b border-cyan-300/10 bg-black/35 px-3 py-2 backdrop-blur-xl">
      <div className="flex items-center gap-2 overflow-x-auto">
        <div className="mr-1 shrink-0 text-[8px] font-mono uppercase tracking-[0.24em] text-cyan-200/45">
          flow
        </div>
        {steps.map((step, index) => (
          <React.Fragment key={step.label}>
            <div
              className={`min-w-[118px] rounded-lg border px-2.5 py-1.5 ${step.active ? "border-cyan-300/25 bg-cyan-300/10" : "border-white/10 bg-white/[0.025]"}`}
            >
              <div className="flex items-center gap-2">
                <span
                  className={`h-1.5 w-1.5 rounded-full ${step.active ? "bg-cyan-300 shadow-[0_0_10px_rgba(103,232,249,0.75)]" : "bg-white/20"}`}
                />
                <span className="text-[9px] font-bold uppercase tracking-[0.16em] text-white/60">
                  {step.label}
                </span>
              </div>
              <div className="mt-1 truncate font-mono text-[9px] text-white/30">
                {step.detail}
              </div>
            </div>
            {index < steps.length - 1 && (
              <div className="h-px w-5 shrink-0 bg-cyan-300/20" />
            )}
          </React.Fragment>
        ))}
      </div>
      ...
      <div className="flex min-w-max items-stretch gap-3">
        {stages.map((stage) => {
          ...
          return (
            <div
              key={stage}
              className="flex items-center gap-1 rounded-xl border border-white/5 bg-white/[0.018] px-2 py-1"
            >
              <div className="mr-1 w-14 shrink-0 text-[8px] font-mono uppercase tracking-[0.18em] text-white/28">
                {TAB_STAGE_LABELS[stage]}
              </div>
              {tabs.map((tab) => (
                <button
                ...
                >
                  <span className="rounded border border-white/10 bg-black/30 px-1.5 py-0.5 text-[8px] font-mono text-white/45">
                    {TAB_CODES[tab.id] || tab.icon}
                  </span>
                  {tab.label}
                </button>
  ...
  const stageSteps = [
    {
      stage: "start",
      label: "Start",
      body: "Ask, launch, or inspect the latest job.",
    },
    {
      stage: "build",
      label: "Build",
      body: "Agents, harness, and swarm do the work.",
    },
    {
      stage: "observe",
      label: "Observe",
      body: "Follow workflows, events, and raw signals.",
    },
    {
      stage: "control",
      label: "Control",
      body: "Review safety, memory, maps, and learning.",
    },
  ] as const;
  return (
    <section className="relative z-10 shrink-0 border-b border-cyan-300/10 bg-[#050b10]/86 px-3 py-2 backdrop-blur-xl">
      <div className="grid gap-2 xl:grid-cols-[280px_1fr]">
        <div className="rounded-xl border border-cyan-300/12 bg-cyan-300/[0.035] px-3 py-2">
          <div className="text-[8px] font-mono uppercase tracking-[0.24em] text-cyan-200/45">
            {TAB_STAGE_LABELS[active.stage]} lens
          </div>
          <div className="mt-1 flex items-center gap-2">
            <span className="rounded border border-cyan-300/20 bg-black/40 px-2 py-0.5 text-[9px] font-mono text-cyan-100/65">
              {TAB_CODES[active.id]}
            </span>
            <span className="text-sm font-black tracking-[0.08em] text-white/82">
              {active.label}
            </span>
          </div>
          <div className="mt-1 text-[11px] leading-4 text-white/42">
            {active.purpose}
          </div>
        </div>
        <div className="grid gap-1 sm:grid-cols-4">
          {stageSteps.map((step, index) => {
            ...
            return (
              <div
                key={step.stage}
                className={`rounded-xl border px-3 py-2 ${isActive ? "border-cyan-300/24 bg-cyan-300/10" : "border-white/7 bg-white/[0.02]"}`}
              >
                <div className="flex items-center gap-2">
                  <span
                    className={`flex h-5 w-5 items-center justify-center rounded-full border text-[9px] font-mono ${isActive ? "border-cyan-200/40 bg-cyan-200/15 text-cyan-100" : "border-white/10 bg-black/25 text-white/32"}`}
                  >
                    {index + 1}
                  </span>
                  <span
                    className={`text-[10px] font-bold uppercase tracking-[0.14em] ${isActive ? "text-cyan-100/75" : "text-white/35"}`}
                  >
                    {step.label}
                  </span>
                </div>
                <div className="mt-1 line-clamp-2 text-[10px] leading-4 text-white/32">
                  {step.body}
                </div>
              </div>
            );
...
function DreamControlPanel({
  ...
  const substances = [
    {
      label: "LSD (Intelligence Division)",
      eventType: "spawn",
      extra: { agentName: "Intelligence Swarm", division: "Intelligence" },
      color: "text-purple-400 border-purple-500/20 bg-purple-500/5",
    },
    {
      label: "DMT (Security Division)",
      eventType: "spawn",
      extra: { agentName: "Security Core", division: "Security" },
      color: "text-rose-400 border-rose-500/20 bg-rose-500/5",
    },
    {
      label: "Psilocybin (Engineering)",
      eventType: "spawn",
      extra: { agentName: "Engineering Loop", division: "Engineering" },
      color: "text-emerald-400 border-emerald-500/20 bg-emerald-500/5",
    },
    {
      label: "Mescaline (Infrastructure)",
      eventType: "spawn",
      extra: { agentName: "Infrastructure Node", division: "Infrastructure" },
      color: "text-amber-400 border-amber-500/20 bg-amber-500/5",
    },
    {
      label: "Ketamine (Error Wash-out)",
      eventType: "error",
      extra: { agentName: "Wash-out" },
  ...
  const signals = [
    ...
    {
      label: "🔊 Chladni eigenmode (Voice Speaking)",
      eventType: "voice-speaking-start",
    },
    { label: "🔇 AI Silent (Voice Stop)", eventType: "voice-speaking-stop" },
    {
      label: "🎭 Pulse Bloom (Chorus Broadcast)",
      eventType: "chorus",
      extra: { agentName: "Companion" },
    },
  ...
  return (
    <div className="h-full overflow-y-auto p-5 space-y-5">
      <div>
        <span className="text-[9px] uppercase tracking-[0.25em] text-cyan-300/45 font-mono font-bold">
          swarm visual cortex
        </span>
        <h3 className="text-base font-black uppercase tracking-wider text-white/85 mt-0.5">
          ENTHEA Control Bridge
        </h3>
        <p className="text-xs text-white/40 leading-relaxed mt-2 font-mono">
          Interactive cockpit for the altered-states visual synthesizer.
          Triggers manual telemetry overrides or simulates swarm stress
          configurations.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div className="rounded-xl border border-white/5 bg-black/25 p-4 space-y-3">
          <span className="text-[9px] uppercase tracking-wider text-white/30 font-mono font-bold block">
            Substance Emulation Overrides
          </span>
...
function PanelContent({
  ...
  const content = (() => {
    switch (tab) {
      case "overview":
        return <CommandDeckOverview data={data} />;
      case "evolution":
        return <SelfEvolutionLens data={data} />;
      case "graph":
        return <LiveSystemMap data={data} />;
      case "harness":
        return <AutonomousHarnessPanel data={data} />;
      case "agents":
        return <AgentRosterPanel data={data} />;
      case "tower":
        return <TowerPanel data={data} />;
      case "swarm":
        return <SwarmPanel data={data} />;
      case "pipeline":
        return <PipelinePanel data={data} />;
      case "timeline":
        return <EventTimelinePanel data={data} />;
      case "sampler":
        return <SamplerPanel />;
      case "gatekeeper":
        return <GatekeeperPanel data={data} />;
      case "cognitive":
        return <CognitivePanel />;
      case "command":
        return <CommandPanel data={data} />;
      case "logs":
        return <LogStreamPanel data={data} />;
      case "mochi":
        return <MochiNarrator data={data} />;
      case "dream":
        return <DreamControlPanel iframeRef={iframeRef || { current: null }} />;
      case "abliterator":
        return <AbliteratorPanel />;
      default:
...
// Bolt ⚡: Memoize unique agents list by reference. In MissionControl,
// multiple separate sub-components (CommandDeckOverview, ProjectKnowledgeGraph, FlowRibbon, etc.)
// call this function every render cycle on the same `data.agents` array.
// A WeakMap prevents redundant O(N) iterations.
const agentsCache = new WeakMap<MissionData["agents"], MissionData["agents"]>();

function getUniqueAgents(agents: MissionData["agents"]) {
  if (agentsCache.has(agents)) return agentsCache.get(agents)!;
...
function TabVisualizer({ tab, data }: { tab: TabId; data: MissionData }) {
  ...
  const serviceCounts = serviceCountLabel(data.services);
  const workflows =
    (data.pipeline?.active?.length || 0) + (data.pipeline?.queue?.depth || 0);
  ...
  const intensity = Math.min(
    1,
    (active + workflows + recentLogs.length / 8 + serviceCounts.online / 2) /
      18,
  );
  const volumeNodes = [
    ...agents
    ...
    ...(data.pipeline?.active || [])
      ...
      .map((workflow, i) => ({
        kind: "job",
        label: workflow.intent || workflow.id,
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: `radial-gradient(circle at 50% 42%, ${colors[0]}33, transparent 36%), radial-gradient(circle at 24% 72%, ${colors[1]}22, transparent 28%)`,
        }}
      />
      <div
        ...
        style={{
          background:
            "repeating-linear-gradient(90deg, transparent 0 18px, rgba(255,255,255,0.55) 19px 20px)",
        }}
      />
      <div className="relative z-10 flex h-full flex-col p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-[9px] uppercase tracking-[0.3em] text-white/35 font-mono">
              live visualizer
            </div>
            <div className="mt-1 text-xl font-black uppercase tracking-[0.18em] text-white/90">
              {tab}
            </div>
            <div
              className="mt-0.5 text-[9px] font-mono text-white/30"
              title="when this data was last fetched from the API"
            >
              ↻{" "}
              {data.fetchedAt
                ...
                : "live"}{" "}
              ·{" "}
              {data.apiConnected ? data.source || "unified_api" : "api offline"}
            </div>
          </div>
          <div className="flex flex-col items-end gap-1">
            ...
            <div
              className="text-[8px] font-mono text-white/25"
              title="active = kernel jobs in flight right now"
            >
              active: {active} · jobs:{" "}
              {
                data.kernelJobs.filter(
                  (j) =>
                    j.state &&
                    !["completed", "failed", "done", "aborted"].includes(
                      String(j.state).toLowerCase(),
                    ),
                ).length
              }{" "}
              · events: {data.logs.length}
              {data.logs[0]?.timestamp
                ? ` · latest ${data.logs[0].timestamp}`
                : ""}
            </div>
          </div>
        </div>
          ...
          <div className="absolute inset-x-3 bottom-2 flex h-20 items-end gap-1">
            {(() => {
              ...
              for (const log of data.logs) {
                ...
                const idx = Math.min(
                  BUCKETS - 1,
                  Math.floor((WINDOW_MS - age) / bucketMs),
                );
                heights[idx] += 1;
              }
              ...
              return heights.map((count, i) => {
                ...
                const h =
                  count === 0 ? 4 : Math.min(100, 8 + (count / maxH) * 88);
...
function VisualizerStat({
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-white/[0.035] px-2 py-2 text-center">
      ...
      <div className="mt-1 text-[8px] uppercase tracking-wider text-white/30 font-mono">
        {label}
      </div>
    </div>
  );
...
function MiniProof({
  ...
  return (
    <div className={`rounded-lg border px-2 py-1.5 ${toneClass}`}>
      <div className="truncate text-[8px] uppercase tracking-[0.18em] text-white/35 font-mono">
        {label}
      </div>
...
function ProjectKnowledgeGraph({ data }: { data: MissionData }) {
  ...
  const agents = getUniqueAgents(data.agents);
  const divisions = [
    ...new Set(agents.map((agent) => agent.division || "UNASSIGNED")),
  ];
  const workflows = [
    ...(data.pipeline?.active || []),
    ...(data.pipeline?.completed || []).slice(0, 8),
  ];
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at 50% 48%, rgba(34,211,238,0.22), transparent 34%), radial-gradient(circle at 18% 80%, rgba(167,139,250,0.18), transparent 28%)",
        }}
      />
      <div className="absolute left-5 top-5 z-20">
        <div className="text-[9px] uppercase tracking-[0.35em] text-cyan-300/45 font-mono">
          relative mapped knowledge graph
        </div>
        <div className="mt-1 text-2xl font-black uppercase tracking-[0.18em] text-white/90">
          Project Graph
        </div>
      ...
      <div className="absolute right-5 top-5 z-20 max-w-xs rounded-2xl border border-white/10 bg-black/55 p-4 backdrop-blur-xl">
        {selected ? (
          ...
          <div className="text-xs text-white/40 font-mono">
            Hover or click graph nodes to inspect project relationships.
          </div>
        )}
      </div>
      <div className="absolute inset-0 [perspective:1150px]">
        <div
          ...
          style={{
            transform: `rotateX(${pointer.y * -18}deg) rotateY(${pointer.x * 26}deg) translateZ(0px)`,
          }}
        >
          {[0, 1, 2, 3].map((layer) => (
            <div
          ...
          {nodes.map((node) => (
            <button
              ...
              style={{
                ...
                backgroundColor:
                  node.kind === "core" ? `${node.color}22` : node.color,
      ...
      <div className="absolute bottom-4 left-4 right-4 z-20 grid grid-cols-2 md:grid-cols-5 gap-2">
        ...
        <VisualizerStat
          label="services"
          value={`${serviceCountLabel(services).online}/${serviceCountLabel(services).total}`}
          color="#34d399"
        />
        <VisualizerStat
          label="flows"
        ...
        <VisualizerStat
          label="companions"
          value={COMPANION_SPECIES.length}
          color="#f472b6"
        />
      </div>
    </div>
...
function CommandComposerDock({ data }: { data: MissionData }) {
  ...
  const [capabilities, setCapabilities] = useState<SurfaceCapability[]>([]);
  const [selectedCapabilityId, setSelectedCapabilityId] =
    useState<string>("mission");
  const [actionStatus, setActionStatus] = useState<string | null>(null);
  const latestResearchJob =
    data.researchStatus?.latest ||
    data.kernelJobs.find((job) => job.route === "deep-research-group") ||
    null;

  const agentOptions = data.agents
    .filter(
      (agent, index, arr) =>
        arr.findIndex((a) => a.name === agent.name) === index,
    )
    .sort((a, b) => a.name.localeCompare(b.name));

  const modeConfig: Record<
    CommandMode,
    { label: string; endpoint: string; hint: string; action: string }
  > = {
    chat: {
      label: "Chat",
      endpoint: ":7780/api/chat",
      hint: "Ask the stack without forcing a full mission",
      action: "Send Chat",
    },
    tower: {
      label: "Agent",
      endpoint: ":7790/api/spawn",
      hint: "Assign one selected tower agent directly",
      action: "Send Agent",
    },
    kernel: {
      label: "Swarm",
      endpoint: ":7780/api/kernel/jobs",
      hint: "Start a canonical API job and hand it to the swarm coordinator",
      action: "Send Swarm",
    },
    orchestrate: {
      label: "Mission",
      endpoint: ":7784/api/orchestrate",
      hint: "Plan, route, execute, and verify a full mission",
      action: "Launch",
    },
    research: {
      label: "Research",
      endpoint: ":7780/api/research/group",
      hint: "Run source-backed OpenRouter deep research through the kernel",
      action: "Research",
    },
    groupchat: {
      label: "Group Chat",
      endpoint: ":7780/api/research/group",
      hint: "Ask the OpenRouter model room and archive the group answer as a kernel job",
      action: "Ask Group",
    },
    api: {
      label: "Raw API",
      endpoint: ":7780/api/command",
      hint: "Send a low-level command through the gateway",
      action: "Send API",
    },
  };
  ...
  const modeForCapability = (capability: SurfaceCapability): CommandMode => {
    if (capability.id === "chat") return "chat";
    if (capability.id === "agent") return "tower";
    if (capability.id === "swarm") return "kernel";
    if (capability.id === "research") return "research";
    if (capability.id === "mission") return "orchestrate";
    return capability.category === "execute" ? "orchestrate" : "chat";
  };

  const applyCapability = (capability: SurfaceCapability) => {
    setSelectedCapabilityId(capability.id);
    if (capability.id === "providers" || capability.id === "settings") {
      window.location.href = capability.web.route;
      return;
    }
    const nextMode = modeForCapability(capability);
    setMode(nextMode);
    setText(`${capability.label}: ${capability.reason}`);
  };

  const dispatchCatalogAction = async () => {
    const capability = capabilities.find(
      (item) => item.id === selectedCapabilityId,
    );
    const command = text.trim();
    if (!capability || !command || busy) return;
    setBusy(true);
    setActionStatus(`dispatching ${capability.id}`);
    try {
      const res = await fetch("/api/action", {
        ...
        body: JSON.stringify({
          ...
          delegate: delegateFromChat,
        }),
      });
      const payload = await res.json().catch(() => ({}));
      setActionStatus(
        res.ok
          ? `${capability.id} dispatched`
          : payload.error || `action failed ${res.status}`,
      );
      setHistory((prev) => [
        {
          mode: modeForCapability(capability),
          text: command,
          status: res.ok
            ? payload.result?.status
              ? `action ${payload.result.status}`
              : "action sent"
            : payload.error || `failed ${res.status}`,
          reply: payload.plan
            ? `${payload.plan.method} ${payload.plan.port ? `:${payload.plan.port}` : ""}${payload.plan.path}`
            : undefined,
        },
        ...prev.slice(0, 4),
      ]);
      ...
      setActionStatus(e?.message || "action failed");
      setHistory((prev) => [
        {
          mode: modeForCapability(capability),
          text: command,
  ...
  const fetchWorkflowDetail = async (workflowId: string) => {
    const paths = [serviceProxyUrl(7784, `/api/workflow/${workflowId}`)];
  ...
  const submit = async () => {
    ...
    if (mode === "tower" && !selectedAgent) {
      setHistory((prev) => [
        { mode, text: command, status: "select an agent" },
        ...prev.slice(0, 4),
      ]);
      return;
    }
    ...
    try {
      ...
      if (mode === "api") {
        ...
        url = serviceProxyUrl(7780, "/api/kernel/jobs");
        body = {
          goal: command,
        ...
        body = {
          ...
          task: command,
          options: { source: "mission-control-ui" },
        };
        ...
        body = {
          ...
          depth: 1,
          model_count: 8,
          source: "mission-control-group-chat",
          tags: ["group-chat"],
        };
      }
      ...
      setHistory((prev) => [
        {
          ...
          status: res.ok
            ? payload.job?.id
              ? `${payload.job.state || "queued"} ${String(payload.job.id).slice(-6)}`
              : workflowId
                ? `${workflow?.status || "delegated"} ${String(workflowId).slice(-6)}`
                : payload.providerStatus ||
                  payload.status ||
                  payload.message ||
                  (payload.success !== false ? "sent" : "accepted")
            : payload.error || `failed ${res.status}`,
          reply:
            payload.reply ||
            payload.result ||
            ...
            payload.synthesis ||
            (payload.job
              ? `${payload.job.route} via Unified API kernel`
              : undefined) ||
            (payload.memberCount
              ? `${payload.successCount}/${payload.memberCount} research models answered`
              : undefined),
          workflowId,
      ...
      setHistory((prev) => [
        { mode, text: command, status: e?.message || "connection failed" },
        ...prev.slice(0, 4),
      ]);
    } finally {
      setBusy(false);
    ...
    <section className="relative z-10 shrink-0 border-b border-cyan-300/10 bg-black/45 backdrop-blur-2xl px-3 py-2">
      <div className="mb-2 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <div className="text-[8px] uppercase tracking-[0.26em] text-cyan-300/45 font-mono whitespace-nowrap">
            mission command spine
          </div>
          <div className="hidden md:block h-px w-16 bg-cyan-300/20" />
          <div className="truncate text-[9px] font-mono text-white/30">
            {modeConfig[mode].hint}
          </div>
        </div>
        <button
          onClick={() => setCollapsed(!collapsed)}
          className="rounded-lg border border-white/10 bg-white/[0.03] px-3 py-1 text-[9px] font-mono uppercase tracking-wider text-white/40 hover:text-cyan-200"
        >
          {collapsed ? "Open Dock" : "Compact"}
        </button>
      </div>
      <div
        className={`grid grid-cols-12 gap-2 items-stretch ${collapsed ? "hidden" : ""}`}
      >
        <div className="col-span-12 xl:col-span-2 rounded-xl border border-white/10 bg-white/[0.03] p-2">
          <div className="text-[8px] uppercase tracking-[0.22em] text-white/32 font-mono">
            selected route
          </div>
          <div className="mt-2 rounded-lg border border-cyan-300/20 bg-cyan-300/10 px-2 py-2">
            <div className="text-[10px] uppercase tracking-[0.18em] text-cyan-100 font-mono">
              {modeConfig[mode].label}
            </div>
            <div className="mt-1 truncate text-[9px] text-white/28 font-mono">
              {modeConfig[mode].endpoint}
            </div>
          </div>
        ...
        <div className="col-span-12 rounded-xl border border-fuchsia-300/10 bg-fuchsia-300/[0.035] p-2.5">
          <div className="mb-2 flex items-center justify-between gap-3">
            <div>
              <div className="text-[8px] uppercase tracking-[0.24em] text-fuchsia-200/45 font-mono">
                surface parity catalog
              </div>
              <div className="mt-1 text-[10px] text-white/35 font-mono">
                Same job classes across CLI, TUI, and web. Pick one to prefill
                the command route or jump to setup.
              </div>
            </div>
            <div className="hidden md:block text-[9px] text-white/25 font-mono">
              CLI: purpclaw capabilities
            </div>
          </div>
                ...
                <button
                  ...
                  className={`min-h-24 rounded-lg border p-2 text-left transition-colors ${
                    selectedCapabilityId === capability.id
                      ? "border-fuchsia-300/45 bg-fuchsia-300/12"
                      : "border-white/10 bg-black/30 hover:border-fuchsia-300/35 hover:bg-fuchsia-300/10"
                  }`}
                  title={`${capability.cli[0] || ""} / ${capability.web.route}`}
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="truncate text-[10px] font-black uppercase tracking-[0.12em] text-white/80">
                      {capability.label}
                    </span>
                    <span className="shrink-0 rounded border border-white/10 bg-white/[0.04] px-1.5 py-0.5 text-[8px] font-mono uppercase text-white/35">
                      {capability.category}
                    </span>
                  </div>
                  <div className="mt-1 line-clamp-2 text-[10px] leading-4 text-white/38">
                    {capability.reason}
                  </div>
                  <div className="mt-2 truncate text-[9px] font-mono text-cyan-200/60">
                    {capability.web.mode} {"->"} {capability.web.api}
                  </div>
                  <div className="mt-1 truncate text-[9px] font-mono text-amber-200/55">
                    setup: {capability.setup.join(", ")}
                  </div>
                </button>
              ))}
            {!capabilities.length && (
              <div className="rounded-lg border border-white/10 bg-black/30 p-3 text-[10px] text-white/35 font-mono">
        ...
        <div className="col-span-12 xl:col-span-8 rounded-xl border border-cyan-300/15 bg-[#031018]/80 p-2.5 shadow-[0_0_40px_rgba(34,211,238,0.08)]">
          <div className="flex items-center gap-3 mb-2">
            ...
            {mode === "chat" && (
              <label className="ml-auto flex items-center gap-2 text-[9px] uppercase tracking-wider text-white/35 font-mono">
                <input
                  type="checkbox"
                  checked={delegateFromChat}
                  onChange={(e) => setDelegateFromChat(e.target.checked)}
            ...
            {(mode === "research" || mode === "groupchat") && (
              <div className="ml-auto flex flex-wrap items-center justify-end gap-2 text-[9px] font-mono">
                <span
                  className={`rounded border px-2 py-1 ${data.researchStatus?.hasKey ? "border-emerald-300/25 bg-emerald-300/10 text-emerald-200" : "border-amber-300/25 bg-amber-300/10 text-amber-200"}`}
                >
                  OpenRouter{" "}
                  {data.researchStatus?.hasKey
                    ? data.researchStatus.keySource || "keyed"
                    : "missing key"}
                </span>
          ...
          {(mode === "research" || mode === "groupchat") &&
            latestResearchJob && (
              <div className="mb-2 grid gap-2 md:grid-cols-4">
                <MiniProof
                  label="latest"
                  value={latestResearchJob.state}
                  tone={
                    latestResearchJob.state === "failed"
                      ? "bad"
                      : latestResearchJob.state === "completed"
                        ? "good"
                        : "live"
                  }
                />
                <MiniProof
                  label="models"
                  value={
                    latestResearchJob.researchRun
                      ? `${latestResearchJob.researchRun.successCount || 0}/${latestResearchJob.researchRun.memberCount || 0}`
                      : "queued"
                  }
                  tone="live"
                />
                <MiniProof
                  label="free"
                  value={latestResearchJob.researchRun?.freeModelCount ?? "n/a"}
                  tone="neutral"
                />
                <MiniProof
                  label="sources"
                  value={latestResearchJob.researchRun?.sourceCount ?? "n/a"}
                  tone="neutral"
                />
              </div>
            )}
          <div className="flex flex-col md:flex-row gap-2">
            ...
            <button
              onClick={dispatchCatalogAction}
              disabled={busy || !text.trim() || !selectedCapabilityId}
              className="w-full md:w-28 rounded-lg border border-fuchsia-300/30 bg-fuchsia-300/12 text-[10px] font-black uppercase tracking-[0.16em] text-fuchsia-200 disabled:opacity-35 hover:bg-fuchsia-300/20"
              title="Dispatch through /api/action using the selected capability"
            >
              Action
            </button>
          </div>
          {actionStatus && (
            <div className="mt-2 rounded-lg border border-fuchsia-300/15 bg-fuchsia-300/[0.04] px-2 py-1 text-[9px] font-mono text-fuchsia-100/65">
              {actionStatus}
            </div>
          )}
          <div className="mt-2 flex flex-wrap gap-2">
            {[
              {
                mode: "kernel" as CommandMode,
                text: "Start a canonical API harness job that audits provider routing, tool access, job status, and verification gaps.",
              },
              {
                mode: "research" as CommandMode,
                text: "Run deep research on the best self-hosted AI agent stack patterns and summarize implementation risks.",
              },
              {
                mode: "groupchat" as CommandMode,
                text: "Ask the model room to compare PURPCLAW against Odysseus and surface the strongest next build move.",
              },
              {
                mode: "orchestrate" as CommandMode,
                text: "Analyze this codebase with No Spaghett, then route cleanup work to the right agents.",
              },
              {
                mode: "chat" as CommandMode,
                text: "Explain current mission state and blockers. Do not edit.",
              },
            ].map((chip) => (
              <button
                key={chip.text}
                ...
                className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1 text-[9px] font-mono text-white/35 hover:text-cyan-200 hover:border-cyan-300/30"
              >
                {chip.text}
              </button>
            ))}
        ...
        <div className="col-span-12 xl:col-span-2 rounded-xl border border-white/10 bg-white/[0.03] p-2.5 overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-[9px] uppercase tracking-[0.22em] text-white/35 font-mono">
              dispatch trace
            </span>
          ...
          <div className="mt-2 max-h-32 space-y-1.5 overflow-y-auto pr-1">
            {history.length === 0 ? (
              <div className="text-[10px] text-white/25 font-mono">
                No dispatches from this console yet.
              </div>
            ) : (
              history.map((item, i) => (
                <div
                ...
                >
                  ...
                  {item.workflowId && (
                    <div className="mt-1 text-[9px] font-mono text-emerald-300/70">
                      workflow {item.workflowId}
                    </div>
                  )}
                  {item.workflow?.delegationLabel && (
                    <div className="mt-1 text-[9px] font-mono text-amber-200/70">
                      delegated to {item.workflow.delegationLabel}
                    </div>
                  )}
                      ...
                      {item.workflow.trace
                        ...
                        .map((step, stepIndex) => (
                          <div
                            ...
                            className="grid grid-cols-[54px_1fr] gap-2 text-[9px] font-mono"
                          >
                            <span
                              className={
                                step.status === "failed"
                  ...
                  {item.reply && (
                    <div className="mt-1 line-clamp-3 text-[10px] leading-4 text-cyan-100/70">
                      {item.reply}
                    </div>
                  )}
                </div>
  ...
  return (
    <div className="h-full overflow-y-auto p-3 grid grid-cols-12 auto-rows-auto gap-3">
      ...
      <section className="col-span-12 xl:col-span-7 min-h-[clamp(440px,58vh,720px)] rounded-2xl border border-cyan-300/10 bg-black/35 relative overflow-hidden command-panel">
        <SectionHeader
          eyebrow="mission lens"
          title="Swarm Constellation"
      ...
      <section className="col-span-12 xl:col-span-5 min-h-[clamp(440px,58vh,720px)] grid grid-rows-[minmax(0,1fr)_minmax(120px,0.5fr)_minmax(170px,0.55fr)] gap-3">
        <div className="rounded-2xl border border-fuchsia-300/10 bg-black/35 relative overflow-hidden command-panel">
          <SectionHeader
            eyebrow="routing & delegation"
            title="Delegation Lens"
            value={routingValue(data)}
          />
          <DelegationRoutingLens data={data} />
        </div>
        <div
          className="rounded-2xl border border-fuchsia-400/20 bg-black/40 relative overflow-hidden command-panel"
          style={{ boxShadow: "0 0 20px rgba(217,70,239,0.06)" }}
        >
          <MochiWidget />
        </div>
        <div className="rounded-2xl border border-emerald-300/10 bg-black/35 relative overflow-hidden command-panel">
          <SectionHeader
            ...
            title="Endpoint Ribbon"
            value={
              serviceCounts.online === serviceCounts.total
                ? "clean"
                : "degraded"
            }
          />
      ...
      <section className="col-span-12 xl:col-span-7 min-h-[210px] rounded-2xl border border-white/10 bg-black/35 relative overflow-hidden command-panel">
        <SectionHeader
          eyebrow="live feed"
          title="Signal Rail"
          value={data.eventBusConnected ? "receiving" : "offline"}
        />
...
function CommandCoreHero({ data }: { data: MissionData }) {
  ...
  const readiness = Math.round(
    ((serviceCounts.online / Math.max(serviceCounts.total, 1)) * 0.55 +
      (requiredLinks.filter((link) => link.ok).length / requiredLinks.length) *
        0.35 +
  ...
  const faultCount = uniqueAgents.filter(
    (agent) => agent.status === "error",
  ).length;
  const routeTone = faultCount
    ? "#fb7185"
    : readiness > 75
      ? "#34d399"
      : "#fbbf24";
    ...
    <section className="purpclaw-core col-span-12 min-h-[260px] overflow-hidden rounded-2xl border border-cyan-200/15 bg-black/45 relative">
      ...
      <div className="relative z-10 grid h-full grid-cols-12 gap-4 p-4">
        <div className="col-span-12 xl:col-span-5 flex min-h-[220px] flex-col justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded border border-cyan-200/20 bg-cyan-200/10 px-2 py-1 text-[9px] font-mono uppercase tracking-[0.22em] text-cyan-100/70">
                mission runtime
              </span>
              <span className="rounded border border-white/10 bg-white/[0.03] px-2 py-1 text-[9px] font-mono uppercase tracking-[0.2em] text-white/35">
                build green
              </span>
              <span
                className="rounded border px-2 py-1 text-[9px] font-mono uppercase tracking-[0.2em]"
                style={{ borderColor: `${routeTone}44`, color: routeTone }}
              >
                readiness {readiness}%
              </span>
            </div>
            ...
            <div className="mt-3 max-w-2xl text-sm leading-6 text-white/55">
              One mission runtime with a command spine, delegated execution,
              validation gates, and separate visual lenses for every layer of
              the process.
            </div>
          </div>
          ...
          <div className="mt-5 grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            <CoreReadout
              label="faults"
              value={faultCount}
              color={faultCount ? "#fb7185" : "#34d399"}
            />
          </div>
        </div>
        ...
        <div className="col-span-12 xl:col-span-4 relative min-h-[220px] rounded-xl border border-white/10 bg-black/30 overflow-hidden">
          ...
          <div
            className="absolute left-1/2 top-1/2 h-24 w-24 -translate-x-1/2 -translate-y-1/2 rounded-full border border-emerald-200/20"
            style={{ boxShadow: `0 0 42px ${routeTone}55` }}
          />
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center">
              <div
                ...
                style={{
                  color: routeTone,
                  textShadow: `0 0 28px ${routeTone}88`,
                }}
              >
                {readiness}
              </div>
              <div className="mt-2 text-[9px] uppercase tracking-[0.32em] text-white/35 font-mono">
                core sync
              </div>
            </div>
          </div>
          {requiredLinks.map((link, i) => {
            ...
            return (
              <div
                key={link.label}
                className="absolute -translate-x-1/2 -translate-y-1/2 rounded-lg border bg-black/75 px-2 py-1 text-[9px] font-mono"
                style={{
                  left: `${x}%`,
                  top: `${y}%`,
        ...
        <div className="col-span-12 xl:col-span-3 flex min-h-[220px] flex-col gap-3">
          <div className="rounded-xl border border-white/10 bg-white/[0.035] p-3">
            <div className="text-[9px] uppercase tracking-[0.24em] text-white/35 font-mono">
              routing stack
            </div>
            <div className="mt-3 space-y-2">
              {requiredLinks.map((link) => (
                ...
                >
                  ...
                  <span
                    className={link.ok ? "text-emerald-300" : "text-rose-300"}
                  >
                    {link.ok ? "linked" : "offline"}
                  </span>
                </div>
              ))}
            </div>
            ...
            <div className="mt-3 space-y-1.5 overflow-hidden">
              {recent.length ? (
                recent.slice(0, 5).map((log) => (
                  <div
                    key={log.id}
                    className="grid grid-cols-[56px_1fr] gap-2 text-[10px] font-mono"
                  >
                ...
                <div className="flex h-24 items-center justify-center text-[10px] uppercase tracking-[0.2em] text-white/25 font-mono">
                  awaiting telemetry
                </div>
              )}
            </div>
...
function CoreReadout({
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-3">
      ...
      <div className="mt-2 text-[9px] uppercase tracking-[0.18em] text-white/35 font-mono">
        {label}
      </div>
    </div>
  );
...
function HoloMetric({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.035] px-4 py-3">
      ...
      />
      <div className="text-[9px] uppercase tracking-[0.22em] text-white/35 font-mono">
        {label}
      </div>
      <div className="mt-1 flex items-end justify-between gap-3">
        <div
          className="text-3xl font-black font-mono leading-none"
          style={{ color, textShadow: `0 0 22px ${color}66` }}
        >
          {value}
        </div>
      ...
      <div className="mt-2 text-[10px] text-white/30 font-mono uppercase tracking-wider">
        {sub}
      </div>
    </div>
  );
...
function SelfEvolutionDiagram({
  ...
  const llmCost = data.llmLedger?.totalCost
    ? `$${data.llmLedger.totalCost.toFixed(4)}`
    : "$0";
  const benchRate = Math.round(
    ((data.harnessBenchmarks as any)?.summary?.completionRate || 0) * 100,
  );
  ...
  return (
    <div
      ...
      style={{ minHeight: compact ? 280 : 600 }}
    >
      {/* Live loop counters */}
        ...
        {[
          {
            ...
            value: scoredTasks,
            color: "#22d3ee",
            sub: "real task results",
          },
          {
            label: "LLM Calls",
            value: llmCalls,
            color: "#a78bfa",
            sub: `${llmCost} estimated`,
          },
          {
            label: "Research Ticks",
            value: tickCount,
            color: "#34d399",
            sub: isRunning
              ? "🔄 running now"
              : lastTick
                ? `last: ${lastTick.status}`
                : "waiting",
          },
          {
            label: "Bench Pass",
            value: `${benchRate}%`,
            color: benchRate >= 90 ? "#34d399" : "#fbbf24",
            sub: "canonical goals",
          },
        ].map((m) => (
          <div
      ...
      <div className="rounded-xl border border-white/8 bg-black/40 p-4">
        <div className="text-[8px] uppercase tracking-[0.28em] font-mono text-white/30 mb-3">
          Agent Leaderboard — Real Outcomes
        </div>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
          {leaderboard.slice(0, 10).map((entry: any, i: number) => (
            <div
              key={entry.agent}
              className={`rounded-lg border px-3 py-2 ${i === 0 ? "border-cyan-300/30 bg-cyan-300/8" : "border-white/8 bg-white/[0.02]"}`}
            >
              <div className="flex items-center justify-between">
              ...
              <div className="mt-1 text-[8px] font-mono text-white/30">
                {entry.totalTasks || 0} tasks · {entry.score || 0}pts
              </div>
            </div>
          ))}
          {leaderboard.length === 0 && (
            <div className="col-span-5 text-[10px] font-mono text-white/25">
              No scored outcomes yet — run a swarm mission to start
            </div>
          )}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-white/8 bg-black/40 p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="text-[8px] uppercase tracking-[0.28em] font-mono text-white/30">
            Auto-Research History — OpenRouter → Memory Matrix
          </div>
          {isRunning && (
        ...
        {recentTicks.length === 0 ? (
          <div className="text-[10px] font-mono text-white/25">
            No research ticks yet — first tick fires 90s after boot, then every
            30 min
          </div>
        ) : (
          <div className="space-y-2">
            {recentTicks
              ...
              .map((tick: any, i: number) => (
                <div
                  key={i}
                  className={`rounded-lg border px-3 py-2 ${tick.status === "ingested" ? "border-emerald-300/20 bg-emerald-300/5" : tick.status === "error" ? "border-rose-300/20 bg-rose-300/5" : "border-white/8 bg-white/[0.02]"}`}
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <span
                        className={`text-[8px] font-mono uppercase px-1.5 py-0.5 rounded border ${tick.status === "ingested" ? "border-emerald-300/25 text-emerald-300" : tick.status === "error" ? "border-rose-300/25 text-rose-300" : "border-white/15 text-white/35"}`}
                      >
                        {tick.status}
                      </span>
                      <span className="text-[9px] font-mono text-white/50 truncate max-w-xs">
                        {tick.topic?.slice(0, 60)}
                      </span>
                    </div>
                  ...
                  {tick.synthesis && (
                    <div className="mt-1.5 text-[9px] font-mono text-white/35 line-clamp-2">
                      {tick.synthesis.slice(0, 180)}
                    </div>
                  )}
                </div>
              ))}
...
function SectionHeader({
  ...
  return (
    <div className="absolute top-0 left-0 right-0 z-10 flex items-center justify-between px-4 py-3 border-b border-white/5 bg-black/25 backdrop-blur-xl">
      <div>
        <div className="text-[8px] uppercase tracking-[0.3em] text-cyan-300/40 font-mono">
          {eyebrow}
        </div>
        <div className="text-sm font-bold tracking-[0.16em] uppercase text-white/80">
          {title}
        </div>
      </div>
...
function AgentConstellation({ data }: { data: MissionData }) {
  const [selected, setSelected] = useState<
    MissionData["agents"][number] | null
  >(null);
    ...
    <div className="absolute inset-x-0 bottom-0 top-14 overflow-y-auto overflow-x-visible p-4">
      <div className="mb-3 flex items-center gap-2 rounded-lg border border-cyan-300/15 bg-cyan-300/[0.04] px-3 py-1.5">
        ...
        <span className="text-[10px] font-mono text-cyan-200/70">
          Click any agent to view & edit its role, division, tier and skills —
          saved live to the tower.
        </span>
      </div>
      <div className="grid [grid-template-columns:repeat(auto-fit,minmax(190px,1fr))] gap-3">
        <div className="relative min-h-[168px] overflow-hidden rounded-xl border border-cyan-300/20 bg-cyan-300/10 p-4 shadow-[0_0_38px_rgba(34,211,238,0.12)]">
          <div
            ...
            style={{
              background:
                "radial-gradient(circle at 50% 45%, rgba(34,211,238,0.35), transparent 62%)",
            }}
          />
          <div className="relative flex h-full flex-col justify-between">
            <div>
              <div className="text-[10px] text-cyan-200/60 font-mono tracking-[0.25em]">
                CORE
              </div>
              <div className="mt-1 text-4xl font-black text-white">
                {data.eventBusConnected ? "ON" : "OFF"}
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-lg border border-white/10 bg-black/30 px-2 py-2 text-center">
                <div className="text-xl font-black font-mono text-cyan-200">
                  {agents.length}
                </div>
                <div className="text-[8px] uppercase tracking-wider text-white/35 font-mono">
                  nodes
                </div>
              </div>
              <div className="rounded-lg border border-white/10 bg-black/30 px-2 py-2 text-center">
        ...
        {divisions.map(([division, list], divisionIndex) => {
          ...
          return (
            <div
              key={division}
              className="relative min-h-[168px] overflow-visible rounded-xl border bg-white/[0.025] p-3 transition-opacity"
              style={{ borderColor: `${cardTone}33`, opacity: dim ? 0.55 : 1 }}
            >
              <div
                className="absolute inset-0 opacity-25"
                style={{
                  background: `radial-gradient(circle at 50% 45%, ${color}22, transparent 68%)`,
                }}
              />
              <div className="relative flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <div
                    className="truncate text-[9px] font-mono uppercase tracking-[0.18em]"
                    style={{ color }}
                  >
                    {division}
                  </div>
                  <div className="text-[9px] font-mono text-white/30">
                    {active} active / {list.length} agents
                  </div>
                </div>
                <div
                  className="flex h-8 w-8 items-center justify-center rounded-full border text-[11px] font-black font-mono"
              ...
              <div className="relative mt-3 h-[clamp(90px,13vh,128px)] rounded-lg border border-white/5 bg-black/25 overflow-visible">
                ...
                {orbit.map((agent, i) => {
                  const angle =
                    (Math.PI * 2 * i) / Math.max(orbit.length, 1) - Math.PI / 2;
                  ...
                  return (
                    <button
                      ...
                      style={{
                        left: `${x}%`,
                        top: `${y}%`,
                        borderColor: `${agentColor}77`,
                        boxShadow: `0 0 16px ${agentColor}33`,
                      }}
                    >
                      ...
                      <span className="pointer-events-none absolute left-1/2 top-full z-50 mt-2 hidden w-56 -translate-x-1/2 rounded-lg border border-cyan-300/20 bg-black/95 p-2 text-left shadow-[0_0_28px_rgba(34,211,238,0.18)] group-hover:block">
                        <span className="block text-[10px] font-mono uppercase tracking-wider text-cyan-200">
                          {agent.name}
                        </span>
                        <span className="block text-[10px] text-white/50">
                          {agent.role || "agent"} / {agent.status}
                        </span>
                        <span className="mt-1 block truncate text-[9px] text-white/35 font-mono">
                          {agent.task || "Idle. No current task reported."}
                        </span>
                      </span>
                    </button>
                  );
              ...
              <div className="relative mt-3 h-1.5 overflow-hidden rounded-full bg-white/10">
                <div
                  ...
                  style={{
                    width: `${Math.max(6, (active / Math.max(list.length, 1)) * 100)}%`,
                    backgroundColor: cardTone,
                    boxShadow: `0 0 12px ${cardTone}`,
                  }}
                />
              </div>
            </div>
...
function routingValue(data: MissionData): string {
  ...
  const primary =
    data.llmStatus?.provider?.provider || lm?.PRIMARY_CHAT?.provider;
...
function DelegationRoutingLens({ data }: { data: MissionData }) {
  ...
  const fallback =
    llm?.fallback || (lm?.FALLBACK ? { enabled: true, ...lm.FALLBACK } : null);
  ...
  const activeJobs = (data.pipeline?.active || []).filter(
    (w) => Array.isArray(w.plan) && w.plan.length,
  );
  ...
  return (
    <div className="absolute inset-x-0 bottom-0 top-14 overflow-y-auto px-4 pb-4 pt-3 space-y-3">
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.025] p-3">
        <div className="flex items-center justify-between">
          <div className="text-[8px] uppercase tracking-[0.24em] font-mono text-white/35">
            model routing
          </div>
          {minimaxReserved && (
            <span className="rounded border border-cyan-300/25 bg-cyan-300/10 px-1.5 py-0.5 text-[8px] font-mono uppercase tracking-wider text-cyan-200/70">
              minimax reserved
            </span>
          )}
        </div>
          ...
          {hops.map((hop, i) => (
            <React.Fragment key={hop.role}>
              <div
                ...
                style={{
                  borderColor: `${hop.tone}${hop.on ? "44" : "1a"}`,
                  background: hop.on ? `${hop.tone}0d` : "transparent",
                }}
              >
                <div
                  className="text-[8px] font-mono uppercase tracking-[0.18em]"
                  style={{ color: hop.on ? hop.tone : "#64748b" }}
                >
                  {hop.role}
                </div>
                <div className="mt-1 truncate text-[11px] font-black font-mono text-white/85">
                  {hop.label}
                </div>
                {hop.model && (
                  <div className="truncate text-[9px] font-mono text-white/35">
                    {hop.model}
                  </div>
                )}
              </div>
              {i < hops.length - 1 && (
                <div className="flex shrink-0 items-center text-white/30 font-mono text-xs">
                  →
                </div>
              )}
            </React.Fragment>
        ...
        {llm?.local?.online && (
          <div className="mt-2 flex items-center gap-1.5 text-[9px] font-mono text-emerald-300/70">
            ...
            {llm.local.models?.[0] ? ` · ${llm.local.models[0]}` : ""}
          </div>
        )}
      </div>
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.025] p-3">
        ...
        {activeJobs.length ? (
          <div className="space-y-2">
            {activeJobs.slice(0, 4).map((job) => {
              ...
              return (
                <div
                  key={job.id}
                  className="rounded-lg border border-white/8 bg-black/30 px-2.5 py-2"
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="truncate text-[10px] font-mono text-white/70">
                      {job.intent || job.id}
                    </span>
                    <span className="shrink-0 text-[8px] font-mono uppercase text-cyan-300/60">
                      {stage?.stage || job.status}
                    </span>
                  </div>
                  <div className="mt-1 flex flex-wrap items-center gap-1 text-[9px] font-mono">
                    <span className="text-emerald-300/80">
                      {stage?.leader || "unassigned"}
                    </span>
                    {stage?.members?.length ? (
                      <span className="text-white/30">
                        + {stage.members.slice(0, 4).join(", ")}
                      </span>
                    ) : null}
                  </div>
                </div>
            ...
            {lanes.slice(0, 6).map((lane) => (
              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  <span
                    className={
                      lane.status === "result-posted" || lane.status === "done"
                        ? "text-emerald-300/70"
                        : "text-white/35"
                    }
                  >
          ...
          <div className="flex h-16 items-center justify-center text-[10px] font-mono uppercase tracking-[0.2em] text-white/25">
            no work in flight
          </div>
        )}
      </div>
      ...
      {services.map((svc) => {
        ...
        return (
          <div
            ...
            className="group flex-1 min-w-0 flex flex-col items-center gap-2"
          >
            <div
              className="w-full rounded-t-md border border-white/10 bg-white/[0.03] relative overflow-hidden"
              style={{ height }}
            >
              <div
                className="absolute inset-x-0 bottom-0 h-full opacity-80"
                style={{
                  background: `linear-gradient(180deg, transparent, ${color}55)`,
                }}
              />
            </div>
            <div
...
function ActivityHeatmap({ logs }: { logs: MissionData["logs"] }) {
  const cells = Array.from({ length: 80 }).map(
    (_, i) => logs[i % Math.max(logs.length, 1)],
  );
  return (
    <div className="absolute inset-x-4 bottom-4 top-16 grid [grid-template-columns:repeat(16,minmax(0,1fr))] gap-1">
      {cells.map((log, i) => {
        const color = log?.type?.toLowerCase().includes("error")
          ? "#fb7185"
          : log?.type?.toLowerCase().includes("agent")
            ? "#22d3ee"
            : log?.type?.toLowerCase().includes("system")
              ? "#a78bfa"
              : "#34d399";
        ...
        return (
          <div
            ...
            style={{
              ...
              boxShadow: active && i < 24 ? `0 0 14px ${color}44` : "none",
            }}
          />
        );
      })}
...
function SignalRail({ logs }: { logs: MissionData["logs"] }) {
  ...
  return (
    <div className="absolute inset-x-4 bottom-4 top-16 flex flex-col gap-2 overflow-hidden">
      {recent.length === 0 ? (
        <div className="flex h-full items-center justify-center text-[11px] text-white/25 font-mono uppercase tracking-[0.25em]">
          waiting for signal
        </div>
      ) : (
        recent.map((log, i) => {
          ...
          return (
            <div
              key={log.id || i}
              className="grid grid-cols-[80px_110px_1fr] items-center gap-3 rounded-xl border border-white/10 bg-white/[0.035] px-3 py-2"
            >
              <div className="text-[10px] text-white/25 font-mono">
                {log.timestamp}
              </div>
              ...
              <div className="truncate text-[11px] text-white/60 font-mono">
                {log.message}
              </div>
            </div>
          );
...
function AgentRosterPanel({ data }: { data: MissionData }) {
  ...
  const filtered =
    filter === "all"
      ? data.agents
      : data.agents.filter((a) => a.division === filter);

  return (
    <div className="h-full flex flex-col p-4 gap-3">
      <div className="flex items-center gap-3">
        <h2 className="text-[11px] uppercase tracking-[0.2em] text-white/40 font-mono">
          Agent Roster
        </h2>
        <div className="flex gap-1">
          {[
            ...
            ...divisions.map((d) => ({
              v: d,
              l: d.charAt(0).toUpperCase() + d.slice(1),
            })),
          ].map((opt) => (
            <button
              key={opt.v}
        ...
        <div className="grid grid-cols-3 xl:grid-cols-6 2xl:grid-cols-8 gap-2">
          {filtered.map((agent, i) => (
            <button
              key={i}
              onClick={() => setSelected(agent)}
              className="relative text-left rounded-xl border border-white/10 bg-white/[0.03] p-3 hover:bg-white/[0.06] hover:border-cyan-300/30 transition-all group"
            >
              ...
              {agent.task && (
                <div className="text-[9px] text-cyan-400/50 mt-1 truncate font-mono">
                  {agent.task}
                </div>
              )}
              <div className="pointer-events-none absolute left-1/2 top-full z-30 mt-2 hidden w-72 -translate-x-1/2 rounded-xl border border-cyan-300/20 bg-black/95 p-3 shadow-[0_0_35px_rgba(34,211,238,0.18)] group-hover:block">
                ...
                <div className="mt-1 text-[11px] text-white/55">
                  {agent.role || "agent"} / {agent.division}
                </div>
                ...
                <div className="mt-2 text-[9px] uppercase tracking-wider text-white/25">
...
function AgentDetailModal({
  ...
  agent: MissionData["agents"][number];
  onClose: () => void;
}) {
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch(
        serviceProxyUrl(7790, `/api/agents/${encodeURIComponent(agent.name)}`),
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="w-[560px] max-w-[calc(100vw-32px)] rounded-2xl border border-cyan-300/20 bg-[#05080d] p-5 shadow-[0_0_80px_rgba(34,211,238,0.18)]">
        <div className="flex items-start gap-4">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-cyan-300/30 bg-cyan-300/10 text-3xl">
            {agent.emoji}
          </div>
          <div className="min-w-0 flex-1">
            <div className="text-[10px] uppercase tracking-[0.28em] text-cyan-300/50 font-mono">
              agent detail
            </div>
          ...
          <button
            onClick={onClose}
            className="rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2 text-xs text-white/45 hover:text-white"
          >
            Close
          </button>
        </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <label className="space-y-1">
            <span className="text-[9px] uppercase tracking-wider text-white/35 font-mono">
              role
            </span>
            <input
              value={role}
              onChange={(e) => setRole(e.target.value)}
              className="w-full rounded-lg border border-white/10 bg-black/50 px-3 py-2 text-sm text-white/75 outline-none focus:border-cyan-300/40"
            />
          </label>
          <label className="space-y-1">
            <span className="text-[9px] uppercase tracking-wider text-white/35 font-mono">
        ...
        <div className="mt-5 flex items-center justify-between">
          ...
          <button
            onClick={save}
            className="rounded-lg border border-emerald-300/30 bg-emerald-300/12 px-5 py-2 text-xs font-black uppercase tracking-[0.18em] text-emerald-200 hover:bg-emerald-300/20"
          >
            Save Agent
          </button>
        </div>
...
function LogStreamPanel({ data }: { data: MissionData }) {
  ...
  const filtered =
    level === "all"
      ? data.logs
      : data.logs.filter((l) => l.type.toLowerCase() === level.toLowerCase());
    ...
    <div className="h-full flex flex-col p-4 gap-3">
      ...
      <div className="flex-1 overflow-y-auto font-mono text-[10px] space-y-0.5">
        {filtered.slice(0, 200).map((log, i) => (
          <div
            key={i}
            className="flex items-center gap-3 px-3 py-1 rounded hover:bg-white/5 transition-colors group"
          >
            <span className="text-white/15 shrink-0 w-20">
              {log.timestamp?.split("T")[1]?.slice(0, 8) || "--”"}
            </span>
            <TypeBadge type={log.type} />
            <span className="text-white/40 shrink-0 w-16 truncate">
              {log.source}
            </span>
            <span className="text-white/60 flex-1 truncate">{log.message}</span>
            {log.agentName && (
              <span className="text-cyan-400/30 shrink-0">{log.agentName}</span>
            )}
          </div>
        ))}
      </div>
...
function TypeBadge({ type }: { type: string }) {
  ...
  return (
    <span
      className={`px-1.5 py-0.5 rounded text-[8px] font-mono uppercase shrink-0 ${cfg.bg} ${cfg.text}`}
    >
      {type}
    </span>
  );
...
function MochiStatBar({
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span className="text-[8px] font-mono uppercase tracking-wider text-white/30 w-8">
        {label}
      </span>
      ...
      <span className="text-[8px] font-mono text-white/20 w-5 text-right">
        {pct}
      </span>
    </div>
  );
...
function MochiWidget() {
  ...
  return (
    <div className="h-full flex flex-col gap-2 p-3">
      {/* Header */}
      <div className="flex items-center justify-between">
        <span className="text-[8px] uppercase tracking-[0.22em] font-mono text-white/30">
          companion
        </span>
        <a
          href="/mochi"
          className="text-[8px] font-mono text-fuchsia-300/50 hover:text-fuchsia-200 transition-colors"
        >
          full view â†’
        </a>
      </div>
      ...
      <div className="flex flex-col items-center gap-1 py-1">
        <div
          className="text-xl font-mono text-fuchsia-300 tracking-widest"
          style={{ textShadow: "0 0 12px rgba(217,70,239,0.6)" }}
        >
          {face}
        </div>
        <div className="flex items-center gap-1.5">
      ...
      {pool && (
        <div className="mt-auto pt-1 border-t border-white/5 grid grid-cols-2 gap-x-2 gap-y-0.5">
          <span className="text-[8px] font-mono text-white/20">
            {pool.skillsCount ?? 0} skills
          </span>
          <span className="text-[8px] font-mono text-white/20">
            {pool.agentsCount ?? 0} agents
          </span>
          <span className="text-[8px] font-mono text-white/20">
            {pool.memories ?? 0} memories
          </span>
          <span className="text-[8px] font-mono text-white/20">
            {interactions} chats
          </span>
  ...
  return (
    <div className="h-full flex flex-col gap-2 p-3 overflow-y-auto">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[8px] font-mono text-cyan-300/60">
          top active
        </span>
      </div>
      <div className="space-y-1.5 mt-1">
        {leaderboard.length === 0 ? (
          ...
          leaderboard.map((item, idx) => (
            <div
              key={item.agent}
              className="flex items-center justify-between rounded border border-white/5 bg-black/15 px-2 py-1 text-[10px] font-mono"
            >
              <div className="flex items-center gap-1.5">
  ...
  return (
    <div className="h-full flex flex-col gap-3 p-3">
      <div className="flex items-center justify-between gap-3">
        <div>
          <div className="text-[8px] uppercase tracking-[0.22em] font-mono text-white/30">
            self-improvement benchmark
          </div>
          <div className="mt-1 text-sm font-black tracking-[0.12em] uppercase text-white/80">
            Canonical Harness Goals
          </div>
        </div>
        <span
          className={`rounded border px-2 py-1 text-[9px] uppercase tracking-[0.18em] font-mono ${
            completionRate >= 90
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
        <BenchmarkMetric
          label="Goals"
          value={`${summary?.passedGoals || 0}/${summary?.totalGoals || 0}`}
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-2">
        <div className="rounded-lg border border-white/10 bg-black/25 p-3">
          <div className="text-[9px] uppercase tracking-[0.18em] font-mono text-violet-200/55">
            learning writes
          </div>
          <div className="mt-2 text-2xl font-black text-violet-200">
            {summary?.memoryLessons || 0}
          </div>
          <div className="mt-1 text-[10px] leading-4 text-white/40">
            Memory lessons captured by benchmark missions.
          </div>
        </div>
        <div className="rounded-lg border border-white/10 bg-black/25 p-3">
          <div className="text-[9px] uppercase tracking-[0.18em] font-mono text-cyan-200/55">
            score updates
          </div>
          <div className="mt-2 text-2xl font-black text-cyan-200">
            {summary?.agentScoreRecords || 0}
          </div>
          <div className="mt-1 text-[10px] leading-4 text-white/40">
            Routing data fed back into agent selection.
          </div>
        </div>
        <div className="rounded-lg border border-white/10 bg-black/25 p-3">
          <div className="text-[9px] uppercase tracking-[0.18em] font-mono text-emerald-200/55">
            trend
          </div>
          ...
          <div className="mt-1 text-[10px] leading-4 text-white/40">
            Repeated runs prove improvement or expose regression.
          </div>
        </div>
      </div>
    </div>
...
function BenchmarkMetric({
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-black/25 p-2 text-center">
      ...
      <div className="mt-1 text-[8px] uppercase tracking-[0.18em] text-white/28 font-mono">
        {label}
      </div>
    </div>
  );
    ...
    <div className="h-full flex flex-col gap-2 p-3 justify-between">
      <div>
        <div className="flex items-center justify-between">
          ...
          <span className="text-[8px] font-mono text-emerald-300/60">
            real spend
          </span>
        </div>
        <div className="grid grid-cols-2 gap-2 mt-2">
          <div className="rounded border border-white/5 bg-black/15 p-2 text-center">
            ...
            <div className="text-[8px] uppercase tracking-wider text-white/25 font-mono">
              calls
            </div>
          </div>
          <div className="rounded border border-white/5 bg-black/15 p-2 text-center">
      ...
      <div className="rounded-xl border border-emerald-300/10 bg-emerald-300/5 p-2.5 mt-2 flex items-center justify-between">
        <div>
          <div className="text-[8px] uppercase tracking-[0.16em] text-emerald-300/60 font-mono">
            estimated cost
          </div>
