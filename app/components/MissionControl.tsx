...
export function MissionControl({ data }: { data: MissionData }) {
  ...
  const toggleDrawer = (tab: TabId) => setOpenDrawer(prev => prev === tab ? null : tab);
    ...
    <div className="h-full min-h-0 bg-[#030508] text-white relative overflow-hidden flex flex-col">
      ...
      <header className="relative z-10 flex shrink-0 items-center justify-between gap-3 px-3 md:px-4 h-10 border-b border-cyan-500/10 bg-black/35 backdrop-blur-xl">
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          <VitalBadge label="Services" value={`${serviceCountLabel(data.services).online}/${data.manifest?.services ?? serviceCountLabel(data.services).total}`} color="#22c55e" />
          <div className="w-px h-5 bg-white/10" />
          {/* live active agents / full roster from the manifest */}
          <VitalBadge label="Agents" value={`${uniqueAgentCount}/${data.manifest?.agents ?? uniqueAgentCount}`} sub={data.manifest ? `${data.manifest.divisions.length}div` : '/R'} color="#00d4ff" />
          <div className="w-px h-5 bg-white/10" />
          <VitalBadge label="Tools" value={data.manifest?.tools ?? 0} color="#f59e0b" />
          <div className="w-px h-5 bg-white/10" />
      ...
      {openDrawer && (
        <DrawerOverlay tab={openDrawer} data={data} onClose={closeDrawer} iframeRef={iframeRef} />
      )}
                  ...
                  <div key={tab.id} className="relative" onMouseEnter={() => setHovered(tab.id)} onMouseLeave={() => setHovered(null)}>
                    ...
                    {preview && (
                      ...
                      >
                        ...
                        <div className="mt-2 text-[8px] font-mono text-white/20">click to open drawer</div>
                      </div>
                    )}
                  </div>
                );
...
function CommandComposerDock({ data }: { data: MissionData }) {
  ...
  const [selectedCapabilityId, setSelectedCapabilityId] = useState<string>('mission');
  ...
  const modeConfig: Record<CommandMode, { label: string; endpoint: string; hint: string; action: string }> = {
    ...
    kernel: { label: 'Swarm', endpoint: ':7780/api/kernel/jobs', hint: 'Start a canonical API job and hand it to the swarm coordinator', action: 'Send Swarm' },
  ...
  useEffect(() => {
    fetch('/api/capabilities', { cache: 'no-store' })
      .then(r => r.ok ? r.json() : null)
      .then(payload => {
        if (Array.isArray(payload?.capabilities)) setCapabilities(payload.capabilities);
      })
      .catch(() => {});
  }, []);
  ...
  const dispatchCatalogAction = async () => {
    ...
    try {
      const res = await fetch('/api/action', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ capability: capability.id, task: command, agent: selectedAgent || undefined, delegate: delegateFromChat }),
      });
      const payload = await res.json().catch(() => ({}));
      setActionStatus(res.ok ? `${capability.id} dispatched` : payload.error || `action failed ${res.status}`);
      setHistory(prev => [{
        mode: modeForCapability(capability),
        text: command,
        status: res.ok ? (payload.result?.status ? `action ${payload.result.status}` : 'action sent') : (payload.error || `failed ${res.status}`),
        reply: payload.plan ? `${payload.plan.method} ${payload.plan.port ? `:${payload.plan.port}` : ''}${payload.plan.path}` : undefined,
      }, ...prev.slice(0, 4)]);
      if (res.ok) setText('');
    } catch (e: any) {
      setActionStatus(e?.message || 'action failed');
      setHistory(prev => [{ mode: modeForCapability(capability), text: command, status: e?.message || 'action failed' }, ...prev.slice(0, 4)]);
    } finally {
      setBusy(false);
    }
  };
    ...
    <section className="relative z-10 shrink-0 border-b border-cyan-300/10 bg-black/45 backdrop-blur-2xl px-3 py-2">
      ...
      <div className={`grid grid-cols-12 gap-2 items-stretch ${collapsed ? 'hidden' : ''}`}>
        ...
        <div className="col-span-12 rounded-xl border border-fuchsia-300/10 bg-fuchsia-300/[0.035] p-2.5">
          <div className="mb-2 flex items-center justify-between gap-3">
            <div>
              <div className="text-[8px] uppercase tracking-[0.24em] text-fuchsia-200/45 font-mono">surface parity catalog</div>
              <div className="mt-1 text-[10px] text-white/35 font-mono">Same job classes across CLI, TUI, and web. Pick one to prefill the command route or jump to setup.</div>
            </div>
            <div className="hidden md:block text-[9px] text-white/25 font-mono">CLI: purpclaw capabilities</div>
          </div>
          <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
            {(capabilities.length ? capabilities : []).slice(0, 8).map(capability => (
              <button
                key={capability.id}
                type="button"
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="truncate text-[10px] font-black uppercase tracking-[0.12em] text-white/80">{capability.label}</span>
                  <span className="shrink-0 rounded border border-white/10 bg-white/[0.04] px-1.5 py-0.5 text-[8px] font-mono uppercase text-white/35">{capability.category}</span>
                </div>
                <div className="mt-1 line-clamp-2 text-[10px] leading-4 text-white/38">{capability.reason}</div>
                <div className="mt-2 truncate text-[9px] font-mono text-cyan-200/60">{capability.web.mode} {'->'} {capability.web.api}</div>
                <div className="mt-1 truncate text-[9px] font-mono text-amber-200/55">setup: {capability.setup.join(', ')}</div>
              </button>
            ))}
            {!capabilities.length && (
              <div className="rounded-lg border border-white/10 bg-black/30 p-3 text-[10px] text-white/35 font-mono">Capability catalog loading...</div>
            )}
          </div>
        </div>
        <div className="col-span-12 xl:col-span-8 rounded-xl border border-cyan-300/15 bg-[#031018]/80 p-2.5 shadow-[0_0_40px_rgba(34,211,238,0.08)]">
          ...
          <div className="flex flex-col md:flex-row gap-2">
            ...
            <button onClick={dispatchCatalogAction} disabled={busy || !text.trim() || !selectedCapabilityId}
          ...
          {actionStatus && <div className="mt-2 rounded-lg border border-fuchsia-300/15 bg-fuchsia-300/[0.04] px-2 py-1 text-[9px] font-mono text-fuchsia-100/65">{actionStatus}</div>}
      ...
      <div className="flex-1 overflow-y-auto font-mono text-[10px] space-y-0.5">
        {filtered.slice(0, 200).map((log, i) => (
          <div key={i} className="flex items-center gap-3 px-3 py-1 rounded hover:bg-white/5 transition-colors group">
            <span className="text-white/15 shrink-0 w-20">{log.timestamp?.split('T')[1]?.slice(0, 8) || '--”'}</span>
...
function VitalBadge({ label, value, sub, color }: { label: string; value: string | number; sub?: string; color: string }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-sm font-bold font-mono" style={{ color, textShadow: `0 0 12px ${color}60` }}>{value}<span className="text-[9px] text-white/30">{sub || ''}</span></span>
    </div>
  );
}
    ...
    <div className="h-full flex flex-col gap-2 p-3">
      ...
      <div className="flex flex-col items-center gap-1 py-1">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {mochi?.shiny && <span className="text-yellow-300 text-[10px]">¨</span>}
        </div>
