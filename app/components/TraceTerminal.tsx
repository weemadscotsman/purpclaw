...
export function TraceTerminal({ compact = false }: { compact?: boolean }) {
  ...
  const sources = useMemo(() => {
    return Array.from(
      new Set(events.map((event) => event.source).filter(Boolean)),
    ).sort();
  }, [events]);
  ...
  if (!open) {
    return (
      ...
      >
        ...
        TRACE
        {events.length > 0 && (
          <span className="text-white/40">{events.length}</span>
        )}
      </button>
    );
  }
  ...
  return (
    <section
      ...
      className={`${frame} flex flex-col overflow-hidden rounded-xl border border-cyan-300/25 bg-black/92 shadow-2xl backdrop-blur-xl`}
    >
      <div className="flex shrink-0 flex-wrap items-center gap-1.5 border-b border-white/10 px-2.5 py-1.5">
        ...
        <input
          ...
          className="ml-auto min-w-20 flex-1 rounded border border-white/10 bg-white/[0.04] px-2 py-0.5 text-[11px] text-white/75"
        />
        <select
          value={sourceFilter}
          onChange={(e) => setSourceFilter(e.target.value)}
          aria-label="Filter by source"
          className="rounded border border-white/10 bg-black px-1.5 py-0.5 text-[10px] font-mono text-white/65"
        >
        ...
        <button
          ...
          className="rounded border border-white/10 px-1.5 py-0.5 text-[10px] font-mono text-white/65"
        >
          {paused ? "▶" : "❙❙"}
        </button>
        <button
        ...
        <button
          ...
          onClick={() => setEvents([])}
          title="clear"
          className="rounded border border-white/10 px-1.5 py-0.5 text-[10px] font-mono text-white/45"
        >
          ✕
        </button>
        <button
          aria-label={expanded ? "shrink" : "fullscreen"}
          onClick={() => setExpanded((v) => !v)}
          title={expanded ? "shrink" : "fullscreen"}
          className="rounded border border-white/10 px-1.5 py-0.5 text-[10px] font-mono text-white/45"
        >
          {expanded ? "⊟" : "⛶"}
        </button>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2.5 py-1.5 font-mono text-[11px] leading-5">
        {visible.length === 0 && (
          <div className="text-white/25">
            No trace events yet. Actions appear here as they route. Click any
            line to copy.
          </div>
        )}
        {visible.map((event) => (
          <div
            ...
            onClick={() => {
              try {
                navigator.clipboard.writeText(lineFor(event));
                setCopied(event.id);
                setTimeout(
                  () => setCopied((c) => (c === event.id ? null : c)),
                  800,
                );
              } catch {}
            }}
