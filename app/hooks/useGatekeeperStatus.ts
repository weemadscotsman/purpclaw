...
export function useGatekeeperStatus() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const [statusRes, amendRes] = await Promise.all([
        fetch('http://localhost:7791/api/status', { signal: AbortSignal.timeout(2000) }),
        fetch('http://localhost:7791/api/propose-amendments', { method: 'POST', signal: AbortSignal.timeout(2000) }),
      ]);
  ...
  const acceptAmendment = useCallback(async (id: string) => {
    try {
      ...
      if (!proposal) return;
      const res = await fetch('http://localhost:7791/api/amend-patch', {
        method: 'POST',
