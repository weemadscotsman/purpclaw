...
export function useAgentTower(): UseAgentTowerReturn {
  ...
  const spawnAgent = useCallback(async (division: string, agentName?: string): Promise<Response> => {
    const res = await fetch(proxyUrl(7790, '/api/spawn'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ agentName: agentName || division, task: `Manual spawn: ${agentName || division}`, options: { division } }),
    });
  ...
  const spawnTeam = useCallback(async (teamName: string, leader: string, members: string[]): Promise<Response> => {
    const res = await fetch(proxyUrl(7790, '/api/team/spawn'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: teamName, leader, members }),
    });
    if (!res.ok) throw new Error(res.statusText);
    return res;
  }, []);
  ...
  const killAgent = useCallback(async (agentId: string): Promise<Response> => {
    const res = await fetch(proxyUrl(7790, `/api/agents/${encodeURIComponent(agentId)}`), {
      method: 'DELETE',
    });
    if (!res.ok) throw new Error(res.statusText);
    return res;
  ...
  const killTeam = useCallback(async (teamName: string): Promise<Response> => {
    const res = await fetch(proxyUrl(7790, `/api/teams/${encodeURIComponent(teamName)}`), {
      method: 'DELETE',
    });
    if (!res.ok) throw new Error(res.statusText);
    return res;
