 ...
 * service on :7798 via fetch + SSE.  Falls back to local file archive readout
 * when the service is offline (read-only).
 */
...
export default function HarnessPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`${HARNESS_BASE}/health`, { cache: 'no-store' });
        if (!cancelled) setServiceOnline(r.ok);
      } catch {
        if (!cancelled) setServiceOnline(false);
      }
    })();
    const t = setInterval(async () => {
      try { const r = await fetch(`${HARNESS_BASE}/health`, { cache: 'no-store' }); setServiceOnline(r.ok); }
  ...
  const refreshHistory = useCallback(async () => {
    ...
    try {
      const r = await fetch(`${HARNESS_BASE}/harness/jobs`, { cache: 'no-store' });
  ...
  const attachStream = useCallback((jobId: string) => {
    closeStream();
    const es = new EventSource(`${HARNESS_BASE}/harness/jobs/${jobId}/stream`);
  ...
  const handleRun = async () => {
    ...
    if (!serviceOnline) return;
    try {
      const r = await fetch(`${HARNESS_BASE}/harness/run`, {
        method: 'POST',
  ...
  const handleStop = async () => {
    if (!job) return;
    try { await fetch(`${HARNESS_BASE}/harness/jobs/${job.id}/stop`, { method: 'POST' }); }
    catch {}
  };
  ...
  const handleLoadHistory = async (id: string) => {
    closeStream();
    try {
      const r = await fetch(`${HARNESS_BASE}/harness/jobs/${id}`, { cache: 'no-store' });
      const data = await r.json();
