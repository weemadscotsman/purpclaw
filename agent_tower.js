...
require('./lib/runtime/telemetry-console').installConsoleTelemetry('purpclaw-tower');
...
// ── Merge persona agents from agents/*.md ───────────────────────────
// The 41 specialist .md files in agents/ (architect, code-reviewer,
// security-reviewer, tdd-guide, …) were authored but never loaded into
// the spawnable tower. Without this, `spawn(architect)` fails even
// though the persona file exists. Source-of-truth: lib/agent-personas.js.
//
// After this block: registry = 35 hardcoded animals + N personas where N
// is the number of unique .md files in agents/. Verified at boot, no
// duplication, no lies in the preprompt.
try {
  const { personaRegistryEntries } = require('./lib/agent-personas');
  const personaEntries = personaRegistryEntries();
  ...
  AGENT_TOWER.stats.totalRegistered = Object.keys(AGENT_TOWER.registry).length;
  AGENT_TOWER.stats.personasAdded = added;
...
async function spawnAgent(agentName, task, options = {}) {
  ...
  // ── Pipeline spine: each spawned agent is its own watchable/stoppable job. ──
  try {
    activeAgent.pipelineJobId = require('./lib/pipeline-registry').start({
      pipeline: `agent:${agentName.toLowerCase()}`,
      project: 'PURPCLAW',
      lane: agentInfo.name,
      trigger: options.source || options.workflowId ? 'workflow' : 'tower',
      risk: agentInfo.tier >= 3 ? 'medium' : 'low',
      inputs: { task: String(task).slice(0, 200) },
    }).job_id;
  ...
  if (agentLoopTools) {
    ...
    try {
      for await (const ev of runAgent({
        ...
        opts: { maxTokens: options?.maxTokens || 16384, temperature: 0.7, tools: AGENT_TOOLS, ...llmOverride },
      })) {
        if (ev.type === 'token') {
          agentState.text += ev.content || '';
        } else if (ev.type === 'tool-call') {
          agentState.toolCalls.push({ name: ev.tool, args: ev.args, result: undefined });
          try { if (activeAgent.pipelineJobId) require('./lib/pipeline-registry').tool(activeAgent.pipelineJobId, { tool: ev.tool, ok: true }); } catch (_) {}
      ...
      // The per-agent override provider (e.g. nvidia, whose keys can 403/expire)
      // failed. CRITICAL: retry the SAME tool-calling loop on the GLOBAL provider
      // (LLM_PROVIDER — a tool-capable provider like minimax) BEFORE degrading to
      // a tool-less one-shot. Otherwise the agent silently loses the ability to
      // use tools (write files, run shell) whenever the override provider is down,
      // which is exactly why showcase/build tasks produced text but no files.
      let recovered = false;
      if (overrideProvider || overrideModel) {
        try {
          console.log(`[TOWER] ${agentInfo.name}: override "${providerName}" failed (${String(e.message).slice(0, 60)}); retrying tool-loop on global provider...`);
          const retryState = { toolCalls: [], text: '' };
          for await (const ev of runAgent({
            prompt: fullPrompt,
            opts: { maxTokens: options?.maxTokens || 16384, temperature: 0.7, tools: AGENT_TOOLS },  // no override → global provider
          })) {
            if (ev.type === 'token') retryState.text += ev.content || '';
            else if (ev.type === 'tool-call') retryState.toolCalls.push({ name: ev.tool, args: ev.args, result: undefined });
            else if (ev.type === 'tool-result') {
              const pending = retryState.toolCalls.filter(tc => tc.result === undefined);
              const target = pending.length ? pending[0] : null;
              if (target) target.result = ev.result;
              else retryState.toolCalls.push({ name: ev.tool, args: ev.args, result: ev.result });
            }
          }
          const retrySummary = retryState.toolCalls.map(tc =>
            `[${tc.name}] ${JSON.stringify(tc.args || {}).substring(0, 100)} → ${String(tc.result ?? '').substring(0, 200)}`
          ).join('\n');
          result = { content: [retryState.text.trim(), retrySummary].filter(Boolean).join('\n\n') || 'Task completed.', toolCalls: retryState.toolCalls };
          totalTokens = retryState.toolCalls.length;
          recovered = true;
          console.log(`[TOWER] ${agentInfo.emoji} ${agentInfo.name} completed on global provider (${retryState.toolCalls.length} tool calls)`);
        } catch (e2) {
          console.log(`[TOWER] global-provider tool-loop also failed: ${e2.message}`);
        }
      }
      if (!recovered) {
        console.log(`[TOWER] Agent-loop fell back to one-shot (no tools): ${e.message}`);
  ...
  // Carry the real tool-call evidence back to the caller. It was computed at
  // result.toolCalls but previously dropped — spawnAgent returned only
  // { success, agent } with the string output, so the coordinator/orchestrator
  // saw 0 tool calls even though the tower log said "N tool calls". Now the
  // structured evidence rides home.
  ...
  // ── Pipeline spine: prove + remember (logFile = the agent's saved output, so
  // it's never a black-hole; proof carries the tool count + model used). ──
  try {
    if (activeAgent.pipelineJobId) {
      const reg = require('./lib/pipeline-registry');
      reg.output(activeAgent.pipelineJobId, logFile, { kind: 'agent-log' });
      reg.finish(activeAgent.pipelineJobId, {
        status: exitCode === 0 ? 'complete' : 'failed',
        claim: `${agentInfo.name} ${exitCode === 0 ? 'completed' : 'failed'}: ${String(task).slice(0, 60)}`,
        proof: { result: exitCode === 0 ? 'pass' : 'fail', detail: `${(activeAgent.toolCalls || []).length} tool calls via ${providerName}/${modelName}` },
  ...
  // (lib/memory-client.js → memory_matrix_v2.py. Do NOT build a parallel store.)
  try {
