...
class NeuroSymbolicBridge:
    ...
    def __init__(self, cozo_path: Optional[str] = None, use_knowledge_graph: bool = True, manage_memory: bool = True):
        ...
        # manage_memory=False means another owner (the Cognitive Spine's
        # MemoryMatrixV2) already loads and writes memory_archive.json.gz.
        # Creating a second engine here means TWO save loops racing on one
        # gzip archive — the source of the atomic-write .tmp contention that
        # deadlocked the spine. When disabled, the memory-lift features
        # degrade gracefully (every call site guards `if self.memory_matrix`).
        self.memory_matrix: Optional[MemoryMatrix] = None
        if MEMORY_MATRIX_AVAILABLE and manage_memory:
            try:
                self.memory_matrix = MemoryMatrix()
            except Exception as e:
                print(f"[NEURO-SYM] Memory Matrix init failed: {e}")
        elif not manage_memory:
            print("[NEURO-SYM] Memory Matrix managed externally (single-writer mode)")
...
def main():
    ...
    parser.add_argument('--port', type=int, default=7884, help='Port to listen on')
