        ...
        if (command) {
          ...
          if (agentMatch) {
            ...
            for (const agentName of autoSpawn.slice(0, 3)) {
              if (AgentTower && AgentTower.spawnAgent) {
                const result = AgentTower.spawnAgent(agentName, `Voice task: ${command.substring(0, 50)}`, {});
                if (result.success) {
  ...
  try {
      ...
      try {
        ...
        if (body.type === 'tool_call') { state.sammyCurrentTask = body.name; state.sammyStatus = 'working'; }
          ...
          if (AgentTower && AgentTower.spawnTeam) {
            const result = AgentTower.spawnTeam({ name: `Team-${teamId}`, leader, members, task });
      ...
      try {
        ...
        if (spawnAgents) {
          ...
          for (const agentName of matchedAgents.slice(0, 3)) {
            const result = AgentTower.spawnAgent(agentName, `Task from chat: ${message.substring(0, 50)}`, {});
            if (result.success) {
    ...
    if (pathname === '/api/tower/spawn' && method === 'POST') {
      try {
        ...
        const result = AgentTower.spawnAgent(agentName, task || 'No task specified', { teamId, parentId });
        if (result.success) {
    ...
    if (pathname === '/api/tower/team' && method === 'POST') {
      try {
        ...
        const result = AgentTower.spawnTeam({ name, leader, members, task: task || 'Team task', priority });
        if (result.success) {
