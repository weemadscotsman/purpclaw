...
async function fetchAll() {
  ...
  return { svcs, coreOk, coreTotal: core.length, orchStat, towerStat, poolStats, workflows, ctxStats };
}
...
function drawHeader(out, W, data) {
  ...
  const left  = ` ${col(C.purple + C.bold, 'PURPCLAW')}  ${col(C.gray, '·')}  ${statusDot} ${col(statusC, coreOk + '/' + coreTotal)} core  ${col(C.gray, '·')}  ${col(C.white, String(agents))} agents  ${col(C.gray, '·')}  ${col(C.white, String(skills))} skills`;
