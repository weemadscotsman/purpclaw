...
async function submitInput() {
  ...
  try {
    ...
    // Auto model routing — same brain as the web UI. Route to the best NIM lane
    // unless the session pinned a model.
    ...
    for await (const ev of runAgent({ prompt: text, history: state.history, model: _model, provider: _provider, opts: { maxTurns: 10 } })) {
      if (ev.type === 'token') {
