...
// The live tower roster. Preferred lists used to contain phantom agents
// ("numbers", "chart") that the tower rejects with "Unknown agent" — burning
// a dispatch + retry cycle on every mission that touched those domains.
const KNOWN_AGENTS = new Set([
  'duck', 'ghost', 'dragon', 'octopus', 'robot', 'mushroom', 'chonk', 'owl',
  'cactus', 'penguin', 'goose', 'turtle', 'axolotl', 'rabbit', 'void', 'wolf',
  'spider', 'raven', 'snake', 'bee', 'bunny', 'guardian', 'karen', 'lemur',
  'mantis', 'shark', 'gorilla', 'phoenix', 'fox', 'crow', 'scientist', 'hawk',
  'elephant', 'panda', 'parrot',
]);
...
function selectAgent(domain, agentScoreModule = null) {
  ...
  // Only ever pick agents that exist in the tower.
  const candidates = def.preferred.filter(a => KNOWN_AGENTS.has(a));
  if (!candidates.length) return 'robot';
...
function decomposeTask(rawTask, parsedIntent = null, agentScoreModule = null) {
  ...
  // Cohesion guard: a short task chained with "then" (create X, then run it,
  // then report) is ONE job for ONE tool-wielding agent. Splitting it across
  // agents inverts dependencies ("report the output" dispatched before the
  // file exists) and loses the shared context. Returning null routes it to
  // the coordinator's single-agent fallback.
  if (rawTask.length < 400 && /\bthen\b/i.test(rawTask)) return null;
