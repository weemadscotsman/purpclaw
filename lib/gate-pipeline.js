...
// ═══════════════════════════════════════════════════════════════════════════
// GATE 0: Regret Score (runs BEFORE all other gates)
// ═══════════════════════════════════════════════════════════════════════════
//
// The most powerful training signal isn't what was created.
// It's what survived.
//
//   Created 24 files, deleted 22 → Regret Score 91% → Training weight: VERY LOW
//   Created 8 files, kept 30 days      → Regret Score 3%  → Training weight: VERY HIGH
//
// Long-term survival is the ultimate truth. Git doesn't lie. Humans do.
function gateRegretScore(sessionData, context = {}) {
  const stats = state.gateStats.regretScore || { runs: 0, highRegret: 0, lowRegret: 0 };
  stats.runs++;

  const created = context.filesCreated || 0;
  const deleted = context.filesDeleted || 0;
  const modified = context.filesModified || 0;
  const stillPresent = context.filesStillPresent || 0;
  const survivalDays = context.survivalDays || 0;

  // Calculate regret score
  const totalTouched = created + modified;
  let regretScore = 0;
  let trainingWeight = 'normal';

  if (totalTouched > 0) {
    // Raw regret: what percentage of touched files were deleted?
    const rawRegret = deleted / Math.max(totalTouched, 1);

    // Survival bonus: files still present after N days reduce regret
    const survivalBonus = stillPresent / Math.max(totalTouched, 1);
    const survivalMultiplier = Math.min(survivalDays / 30, 1); // caps at 30 days

    // Final regret: raw regret minus survival bonus, scaled by survival time
    regretScore = Math.max(0, rawRegret - (survivalBonus * survivalMultiplier * 0.5));
    regretScore = Math.round(regretScore * 100);
  } else if (deleted > 0 && totalTouched === 0) {
    // Only deletions, no creations — pure cleanup session
    regretScore = 0;
    trainingWeight = 'cleanup';
  }

  // Assign training weight based on regret score
  if (regretScore >= 80) {
    trainingWeight = 'very-low';
    stats.highRegret++;
  } else if (regretScore >= 50) {
    trainingWeight = 'low';
  } else if (regretScore >= 25) {
    trainingWeight = 'reduced';
  } else if (regretScore <= 5 && totalTouched > 3 && survivalDays >= 7) {
    trainingWeight = 'very-high';
    stats.lowRegret++;
  } else if (regretScore <= 15 && totalTouched > 0) {
    trainingWeight = 'high';
  }

  const passed = regretScore < 80; // Only hard-fail at 80%+ regret

  state.gateStats.regretScore = stats;
  saveState();
  ...
  return {
    ...
    stillPresent,
    survivalDays,
    signal: regretScore <= 15 ? 'STRONG_KEEP' : regretScore >= 80 ? 'WEAK_DISCARD' : 'MODERATE',
  };
}
...
function runAllGates(sessionData = {}, context = {}) {
  ...
  // Run all gates (Gate 0: Regret Score first)
  results.gates.regretScore = gateRegretScore(sessionData, context);
  ...
  // Extract training weight from regret score
  results.trainingWeight = results.gates.regretScore.trainingWeight;
