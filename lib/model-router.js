...
// v2.1 — Model list matches operator directive (2026-06-24):
//   Main chat:   MiniMax platform subscription (MiniMax-M3)
//   Swarm:       moonshotai/kimi-k2.6 (NIM)
//   Code:        deepseek-ai/deepseek-v4-pro or deepseek-ai/deepseek-v4-flash (NIM)
//   Reasoning:   deepseek-ai/deepseek-v4-pro (NIM)
//   Review:      z-ai/glm-5.1 (NIM)
//   Long ctx:    moonshotai/kimi-k2.6 (NIM)
//   Fallback pool: glm-5.1, gemma-4-31b-it, qwen3.5-122b-a10b, mistral-small-4-119b-2603
//   Voice/TTS:   resembleai/chatterbox-multilingual-tts (NIM, TTS-only lane)
const M = (MX && MX.MODELS) || {
  ...
  SWARM:      { provider: 'nvidia', model: 'moonshotai/kimi-k2.6' },
  FLASH:      { provider: 'nvidia', model: 'deepseek-ai/deepseek-v4-flash' },
  GEMMA:      { provider: 'nvidia', model: 'google/gemma-4-31b-it' },
  QWEN:       { provider: 'nvidia', model: 'qwen/qwen3.5-122b-a10b' },
  MISTRAL:    { provider: 'nvidia', model: 'mistralai/mistral-small-4-119b-2603' },
};
...
// v2.1 — Expanded per operator directive: all 8 NIM models in the pool.
const FB = {
  code:    ['minimaxai/minimax-m3', 'moonshotai/kimi-k2.6', 'z-ai/glm-5.1'],
  reason:  ['deepseek-ai/deepseek-v4-flash', 'moonshotai/kimi-k2.6', 'minimaxai/minimax-m3', 'z-ai/glm-5.1'],
  review:  ['moonshotai/kimi-k2.6', 'minimaxai/minimax-m3', 'z-ai/glm-5.1'],
  longctx: ['minimaxai/minimax-m3', 'z-ai/glm-5.1', 'qwen/qwen3.5-122b-a10b'],
  swarm:   ['minimaxai/minimax-m3', 'deepseek-ai/deepseek-v4-flash', 'z-ai/glm-5.1'],
};

const LANES = {
  // DEFAULT / chat lane → working MiniMax PLATFORM subscription (api.minimax.io),
  // NOT NVIDIA NIM. NIM keys are 403 and operator directive is: normal chat always
  // uses the paid MiniMax subscription. (Eddie, 2026-06-24.)
  code:    { provider: 'minimax', model: 'MiniMax-M3', agent: 'robot', label: 'MiniMax M3', for: 'code, general, quick answers (default)', fallbacks: FB.code },
  ...
  // v2.1 — Swarm lane: Kimi K2.6 is the swarm orchestration brain
  swarm:   { ...M.SWARM,  agent: 'wolf',   label: 'Kimi K2.6 Swarm', for: 'swarm orchestration, multi-agent coordination, parallel delegation', fallbacks: FB.swarm },
};
 ...
 */
// While the NVIDIA NIM key pool is dead (HTTP 403), redirect any nvidia lane to
// the working MiniMax platform so chat/delegation never hits a 403 or a
// provider/model mismatch. Flip PURPCLAW_NIM_DOWN=0 (env) the moment live NVIDIA
// keys are restored and full NIM lane routing comes back. (Eddie 2026-06-24.)
function nimGuard(r) {
  if (r && r.provider === 'nvidia' && process.env.PURPCLAW_NIM_DOWN !== '0') {
    return {
      ...r,
      provider: 'minimax',
      model: 'MiniMax-M3',
      label: (r.label || 'MiniMax M3') + ' (NIM down → MiniMax)',
...
function route(message, opts = {}) {
  if (opts.lane && LANES[opts.lane]) {
    return nimGuard({ ...LANES[opts.lane], lane: opts.lane, reason: 'explicit lane' });
  }
  if (opts.model) {
    // v2.1 — Even when model is explicitly passed, include fallback chain
    // from the default lane so a 429/403 can glide to NIM.
    const fb = (LANES[DEFAULT_LANE] && LANES[DEFAULT_LANE].fallbacks) || [];
    return nimGuard({ provider: opts.provider || 'nvidia', model: opts.model, lane: 'custom', label: opts.model, agent: null, reason: 'explicit model', fallbacks: fb });
  }
  ...
  return nimGuard({ ...lane, lane: best, reason: bestScore ? `classified '${best}' (score ${bestScore})` : 'default lane' });
}
