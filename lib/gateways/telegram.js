...
const TOKEN = (() => {
  ...
  const override = argv.find(a => a.startsWith('--token='));
  if (override) return override.slice('--token='.length);
  return process.env.TELEGRAM_BOT_TOKEN || '';
})();
const PERSONA = (() => {
  const argv = process.argv.slice(2);
  const override = argv.find(a => a.startsWith('--persona='));
  if (override) return override.slice('--persona='.length);
  return process.env.TELEGRAM_PERSONA || '';
})();
const PORT = parseInt(
  (() => {
    const argv = process.argv.slice(2);
    const override = argv.find(a => a.startsWith('--port='));
    return override ? override.slice('--port='.length) : (process.env.PORT || '7795');
  })(),
  10
);
...
function startHealth() {
  const server = http.createServer((req, res) => {
    ...
    if (url.pathname === '/version') {
      res.writeHead(200, { 'content-type': 'application/json' });
      res.end(JSON.stringify({ name: 'purpclaw-telegram-gateway', version: '0.2.0' }));
      return;
    }
