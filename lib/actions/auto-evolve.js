...
const MUTATOR = require(path.join(ROOT, 'lib', 'evolution', 'mutator'));
const FORGE = require(path.join(ROOT, 'lib', 'evolution', 'skill-forge'));

const ACTIONS = {
  pass: { fn: (opts) => MUTATOR.runPass(opts || {}), description: 'Run a mutation pass over recent evidence' },
  forge: { fn: (opts) => FORGE.run(opts || {}), description: 'Detect taxonomy gaps; propose new JOB_TYPES + archetypes' },
  status: { fn: () => MUTATOR.status(), description: 'List pending + applied mutations' },
  approve: { fn: (opts) => MUTATOR.approve(opts && opts.id), description: 'Apply a queued mutation by id' },
  reject: { fn: (opts) => MUTATOR.reject(opts && opts.id, opts && opts.reason), description: 'Reject a queued mutation' },
  history: { fn: (opts) => MUTATOR.history(opts || {}), description: 'Show last 20 applied mutations' },
  regressions: { fn: () => MUTATOR.regressions(), description: 'Show recent regression alerts' },
};
