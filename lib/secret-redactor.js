...
// Fast pre-screening regex derived directly from the RE rules above.
// Proof of Zero False Negatives:
// - RE[0], RE[1]: Require '='
// - RE[2]: Requires 'Bearer'
// - RE[3]: Requires 'eyJ'
// - RE[4]: Requires 'sk-' or 'sk_'
// - RE[5]: Requires 40+ continuous hex characters (guaranteed filtered if length < 40)
// - RE[6]: Requires ':' or '='
// If a string has length < 40 and contains none of '=', ':', 'sk-', 'sk_', 'eyJ', or 'Bearer',
// none of the RE rules can match mathematically.
const FAST_SECRET_PRECHECK = /[:=]|sk[-_]|eyJ|bearer/i;

function redact(input) {
  if (input == null) return input;
  let s = typeof input === 'string' ? input : String(input);

  // Performance optimization: skip full regex scan loop if string is under 40 chars
  // and contains no required secret trigger characters/tokens.
  if (s.length < 40 && !FAST_SECRET_PRECHECK.test(s)) return s;
