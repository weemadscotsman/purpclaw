...
async function getLiveServiceState() {
  const services = [
    ...
    { name: 'Memory Matrix',     port: 7880, path: '/recall',        file: 'memory_matrix_v2.py' },
    { name: 'Metrics',           port: 7890, path: '/health',        file: 'metrics_aggregator.js' },
    { name: 'Speech To Text',    port: 7896, path: '/health',        file: 'voice_stt.py' },
    { name: 'Mission Control',   port: 3000, path: '/mission',       file: 'app/', timeoutMs: 8000 },
  ];
...
async function buildSelfContextAsync(agentName = 'agent', taskContext = '') {
  ...
  [
    ...
    ['lib/memory-client.js', 'Memory matrix wrapper — recall(), ingest(), port 7880'],
  ...
  lines.push(
    ...
    '  - Recall memory:           POST http://127.0.0.1:7880/recall   {"query":"<text>"}',
    '  - Save a lesson to memory: POST http://127.0.0.1:7880/ingest   {"text":"<finding>","tags":[...]}',
