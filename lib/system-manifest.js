 ...
 *   - agents/divisions  ← agent_profiles.json
 *
...
function getServices() {
  const reg = safe(() => require(path.join(ROOT, 'service_registry')), null);
...
// ── Agents / divisions (authoritative: agent_profiles.json) ──
function getAgents() {
  const profiles = safe(() => require(path.join(ROOT, 'agent_profiles.json')), null);
  if (!profiles) return [];
  const arr = Array.isArray(profiles) ? profiles : Object.entries(profiles).map(([k, v]) => ({ key: k, ...(v && typeof v === 'object' ? v : {}) }));
  return arr.map(a => ({ key: a.key || a.name, name: a.name || a.key, division: a.division || null, role: a.role || null }));
}
