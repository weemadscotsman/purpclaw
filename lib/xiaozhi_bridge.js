...
async function handleRpc(req) {
  ...
  switch (method) {
    ...
    case 'tools/call': { 
      ...
      const slowTools = ['browser_open', 'browser_click', 'browser_type', 'browser_navigate', 'mouse_click', 'keyboard_type', 'execute_command', 'open_application', 'find_and_click'];
      if (slowTools.includes(name)) {
        console.log(`[BRIDGE] ⏳ Waiting 2500ms for OS to settle after ${name}...`);
        await new Promise(r => setTimeout(r, 2500));
      } else if (!name.startsWith('file_') && name !== 'system_status') {
        // Minor 1000ms delay for other physical/visual things just in case
