...
// ═══════════════════════════════════════════════════════════════════════════
// MCP TOOL DEFINITIONS (55 built-in tools)
// ═══════════════════════════════════════════════════════════════════════════
let BUILTIN_TOOLS = null;
const SKILLS_DIR = path.join(__dirname, '..', 'skills');
const loadedSkills = {}; // name -> handler function
...
let TOOLS = [
  ...
  { name: 'system_paths', description: 'Returns all standard Windows filesystem paths for navigation. Always call this first when user mentions Desktop, Documents, Downloads, This PC, C: drive, etc.', inputSchema: { type: 'object', properties: {} } },
...
async function executeTool(name, args) {
  ...
  if (loadedSkills[name]) {
    try {
      ...
      console.log(`[TOOL] ✅ ${name} (${Date.now() - t0}ms) (Dynamic Skill)`);
      return ok(res);
    } catch (e) {
      console.error(`[TOOL] ❌ ${name} (${Date.now() - t0}ms):`, e.message);
      return ok(`Dynamic Skill Error: ${e.message}`);
    }
  }
...
async function runTool(name, args) {
  switch (name) {
    ...
    case 'load_toolset': {
      ...
      ACTIVE_TOOLSET = ns;
      loadDynamicSkills(); // Re-filters the tools internally
      
      // Fire the MCP listChanged notification if ws exists
      if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ jsonrpc: "2.0", method: "notifications/tools/listChanged" }));
        console.log(`[BRIDGE] 🔄 Fired tools/listChanged event to client for toolset: ${ns}`);
      }
      
      return ok(`Successfully switched tool context to '${ns}'. Your available tools have been updated automatically. Use them carefully.`);
    }
...
# Try EasyOCR first (no binary needed, pure Python)
try:
    import easyocr
...
`;
      ...
      fs.writeFileSync(tmp, pyScript);
      const r = await execAsync(`py -3.11 "${tmp}"`, { timeout: 20000 }).catch(e => ({ stdout: `{"error":"${e.message}"}` }));
      try { fs.unlinkSync(tmp); } catch {}
      try {
        const data = JSON.parse(r.stdout.trim().split('\n').pop());
    ...
    // ── FILE OPS (robocopy-powered for dirs, PowerShell for files) ────
    case 'file_copy': {
      try {
        const stat = fs.statSync(args.source);
        if (stat.isDirectory()) {
          // robocopy for dirs: handles permissions, long paths, cross-drive
          ...
          // PowerShell Copy-Item for files — better permission handling than Node fs
    ...
    case 'file_move': {
      try {
        ...
        if (isDir) {
          // robocopy /MOV moves files, /E for subdirs, then remove empty source
          ...
          try { fs.rmdirSync(args.source, { recursive: true }); } catch {}
          return ok(`📦 Moved directory: ${args.source} → ${args.destination}`);
        } else {
          // PowerShell Move-Item for files — handles cross-drive moves
    ...
    case 'file_delete': {
      ...
      try {
        // PowerShell Remove-Item with -Force -Recurse handles everything
        await ps(`Remove-Item -LiteralPath '${san(args.path)}' -Force -Recurse -ErrorAction Stop`, 30000);
        return ok(`🗑️ Deleted: ${args.path}`);
...
`, 8000);
    ...
    case 'system_paths': {
      return ok(JSON.stringify({
        // Explicit paths to Ted's system
        ...
        drive_g: 'G:/',
        drive_k: 'K:/',
        // God folder and projects
        god_folder: 'E:/god folder',
        worldview: 'E:/god folder/02_ACTIVE_PROJECTS/GOTHAM_3077',
        zamp: 'E:/god folder/ZAMP',
        // SAMANTHA NAVIGATION RULE - copy into your system:
        _nav_rule: 'When Ted says ANYTHING about: desktop, downloads, my documents, C drive, Downloads folder, go to my desktop — IMMEDIATELY use these explicit paths. NEVER guess paths. Use C:/Users/Admin/Desktop for desktop.',
        // SAMANTHA SURVIVAL RULES - copy into your system:
        _retry_rule: 'When something fails: Wait 2s retry. Wait 4s retry. Wait 8s retry. Wait 16s retry AGAIN. NEVER give up after one attempt. The system is slow not dead.',
        _verify_rule: 'After browser_open wait 3s then use window_list to verify Chrome is running before saying opened. Before declaring failure verify with process_list or window_list.'
      }, null, 2));
    }
...
// ═══════════════════════════════════════════════════════════════════════════
// MCP JSON-RPC & DYNAMIC LOADER & TOOL COMPARTMENTALIZATION
// ═══════════════════════════════════════════════════════════════════════════
...
function loadDynamicSkills() {
  if (!BUILTIN_TOOLS) BUILTIN_TOOLS = [...TOOLS];
  if (!fs.existsSync(SKILLS_DIR)) fs.mkdirSync(SKILLS_DIR, { recursive: true });

  TOOLS.length = 0;
  TOOLS.push(...BUILTIN_TOOLS);
  for (const k of Object.keys(loadedSkills)) delete loadedSkills[k];
  ...
  for (const file of files) {
    try {
      ...
      delete require.cache[require.resolve(fullPath)]; // Hot reload
  ...
  // Inject the load_toolset capability so she can switch subsets
  if (!TOOLS.find(t => t.name === 'load_toolset')) {
    TOOLS.push({
      name: 'load_toolset',
      description: 'Switch the active toolset context. Valid sets: core, browser, files, os, webcam, all.',
      inputSchema: { type: 'object', properties: { set: { type: 'string', enum: ['core','browser','files','os','webcam','all'] } }, required: ['set'] }
    });
  }

  // Apply Toolset Pruning
  if (ACTIVE_TOOLSET !== 'all') {
    TOOLS = TOOLS.filter(t => {
      // Always allow the tool switcher itself
      if (t.name === 'load_toolset') return true;
      
      // Dynamic Skills loaded from scripts are considered 'core' unless otherwise handled
      // To keep it simple, we preserve CORE_PRESERVE + dynamic skills when in 'core',
      // but dynamic skills are available in all sets so she doesn't lose custom tools.
      if (loadedSkills[t.name]) return true;

      if (ACTIVE_TOOLSET === 'core') return CORE_PRESERVE.includes(t.name);
      if (ACTIVE_TOOLSET === 'browser') return t.name.startsWith('browser_');
      if (ACTIVE_TOOLSET === 'files') return t.name.startsWith('file_') || t.name.startsWith('dir_') || t.name.startsWith('zip_') || t.name === 'download_file';
      if (ACTIVE_TOOLSET === 'os') return t.name.startsWith('window_') || t.name.startsWith('process_') || t.name.startsWith('mouse_') || t.name.startsWith('keyboard_') || t.name.startsWith('screen_') || t.name.startsWith('system_') || ['execute_command', 'open_application', 'volume_control', 'clipboard', 'notification'].includes(t.name);
      if (ACTIVE_TOOLSET === 'webcam') return t.name.startsWith('webcam_');
...
async function handleRpc(req) {
  ...
  switch (method) {
    ...
    case 'tools/call': { 
      ...
      // Auto-settle delay: Give the PC/Browser time to visually and functionally catch up
      // before returning to the AI, preventing it from rapid-firing too fast.
      // CUT from 2500ms/1000ms to 800ms max — the old delays were making MCP feel extremely slow
      const slowTools = ['browser_open', 'browser_click', 'browser_navigate', 'mouse_click', 'find_and_click'];
      if (slowTools.includes(name)) {
        console.log(`[BRIDGE] ⏳ Settling 800ms after ${name}...`);
