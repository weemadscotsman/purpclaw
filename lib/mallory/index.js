 ...
 * SECURITY: Uses execSafe instead of exec to prevent shell command injection.
 */
async function getProcessList() {
  const { ok, stdout } = await execSafe('tasklist', ['/FO', 'CSV', '/NH'], { timeoutMs: 10000, windowsHide: true });
  if (!ok) return [];
...
async function killPid(pid, label = '') {
  ...
  const { ok, stdout, stderr } = await execSafe('taskkill', ['/F', '/PID', String(pid)], { timeoutMs: 10000, windowsHide: true });
  ...
  const entry = {
    ...
    label,
    killed: ok,
    output: !ok ? (stderr || 'failed') : stdout.trim(),
  };
