...
class AgentSession {
  ...
  async gitStatus() {
    ...
    try {
      const status = execSync('git status --porcelain', { cwd: this.cwd, encoding: 'utf8', timeout: 5000 });
      return status;
    } catch {
      return '';
    }
  ...
  async gitDiff() {
    const { execSync } = require('child_process');
    try {
      const diff = execSync('git diff --stat', { cwd: this.cwd, encoding: 'utf8', timeout: 5000 });
      return diff;
    } catch {
      return '';
    }
  ...
  async gitBranch() {
    const { execSync } = require('child_process');
    try {
      const branch = execSync('git branch --show-current', { cwd: this.cwd, encoding: 'utf8', timeout: 5000 });
      return branch.trim();
    } catch {
      return 'unknown';
    }
