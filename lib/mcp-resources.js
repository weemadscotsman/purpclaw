...
// ── resource: purpclaw://agents (canonical AGENT_REGISTRY.json) ─────────────
function readAgents() {
  let agents = [];
  try {
    agents = require('./agent-registry').listAgents().map(a => ({
      id: a.key,
      name: a.name,
      type: a.type,
      division: a.division,
...
function readAgent({ params }) {
  const id = params.id;
  let agent = null;
  try { agent = require('./agent-registry').getAgent(id); } catch {}
