...
const PIPE_ROOT = path.join(__dirname, '..', 'agent_work', 'pipelines');
...
// Compact a long artifact into a brief for the NEXT stage's prompt — the named
// fix: never hand a stage the full raw blob (that blackholed the Marketer).
function compact(text, n = 1200) {
  const t = String(text || '').trim();
  return t.length > n ? t.slice(0, n) + '\n…[full artifact on disk]' : t;
}

/**
 * Phase 7 content pipeline: Analyst → Writer → Marketer — ARTIFACT-FIRST.
 * Each stage: run → assert non-empty (zero-char = RED output_blackhole, the
 * doctrine line) → write stage-N-role.md to disk → hand the NEXT stage a COMPACT
 * brief + the file path, not the raw blob. Tracked as one registry job; writes
 * pipeline-report.json. onStage(name, role, text) fires for streaming UIs.
 */
async function runContentPipeline(topic, { onStage } = {}) {
  ...
  const dir = path.join(PIPE_ROOT, jid || `pipe-${String(topic).replace(/\W+/g, '-').slice(0, 30)}`);
  try { fs.mkdirSync(dir, { recursive: true }); } catch (_) {}
  const out = { topic, dir, stages: {}, artifacts: {} };

  // Run a stage, enforce non-empty, persist the artifact, register the output.
  const stage = async (n, role, label, prompt, opts) => {
    if (jid) registry.step(jid, `${CREW[role].name}: ${label}`);
    const text = await runRole(role, prompt, { trigger: 'pipeline', ...opts });
    if (!text || !text.trim()) {
      // Doctrine: a stage that ran but returned nothing is a RED failure.
      throw Object.assign(new Error(`output_blackhole: ${role} stage "${label}" returned 0 chars`), { blackhole: true, role });
    }
    const file = path.join(dir, `stage-${n}-${role}.md`);
    try { fs.writeFileSync(file, `# ${CREW[role].name} — ${label}\n\nTopic: ${topic}\n\n${text}\n`, 'utf8'); } catch (_) {}
    out.stages[role] = text; out.artifacts[role] = file;
    if (jid) registry.output(jid, file, { kind: role });
    if (onStage) onStage(label, role, text);
    return text;
  };

  try {
    const research = await stage(1, 'analyst', 'research', `Research this topic for a content piece. Give key findings, angles, and sources:\n${topic}`);

    const content = await stage(2, 'writer', 'draft',
      `Write a tight, structured blog post.\nTopic: ${topic}\n\nResearch findings:\n${compact(research, 2000)}`,
      { maxTokens: 1800 });

    // ARTIFACT-FIRST handoff: Marketer gets a COMPACT brief + the file path, NOT
    // the full ~5k Writer blob (that overflowed and returned 0 chars).
    const brief = `Topic: ${topic}\n\nContent excerpt (full draft at ${out.artifacts.writer}):\n${compact(content, 1200)}`;
    const social = await stage(3, 'marketer', 'social', `From this content brief, write 5 social posts (mix platforms):\n\n${brief}`);
    const strategy = await stage(4, 'marketer', 'strategy', `From this content brief, build a concrete promotion + monetization plan (channels, sequence):\n\n${brief}`)
      .catch(e => { if (e.blackhole) return ''; throw e; }); // strategy is the 2nd marketer call; tolerate a single blackhole
    out.stages.strategy = strategy;

    // pipeline-report.json — the durable record of the whole run.
    const report = { topic, dir, stages: Object.keys(out.artifacts), artifacts: out.artifacts, ok: true, at: new Date().toISOString() };
    try { fs.writeFileSync(path.join(dir, 'pipeline-report.json'), JSON.stringify(report, null, 2), 'utf8'); } catch (_) {}

    if (jid) registry.finish(jid, { status: 'complete', claim: `content pipeline for "${String(topic).slice(0, 60)}"`, proof: { result: 'pass', detail: `${Object.keys(out.artifacts).length} artifacts → ${dir}` } });
    out.ok = true;
    return out;
  } catch (e) {
    try { fs.writeFileSync(path.join(dir, 'pipeline-report.json'), JSON.stringify({ topic, dir, ok: false, error: e.message, blackhole: !!e.blackhole, artifacts: out.artifacts, at: new Date().toISOString() }, null, 2), 'utf8'); } catch (_) {}
    // Zero-char blackhole → RED with the exact failure class, not a vague fail.
    if (jid) registry.finish(jid, { status: 'failed', claim: e.message, proof: { result: 'fail', detail: e.blackhole ? 'output_blackhole' : 'error' } });
    out.ok = false; out.error = e.message; out.blackhole = !!e.blackhole;
    return out;
  }
}
