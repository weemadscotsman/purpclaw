...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/synthesize' && req.method === 'POST') {
    ...
    log(`synthesize "${text.slice(0, 60)}${text.length > 60 ? '...' : ''}" (voice=${voice}) to ${tempWav}`);
    
    const r = await runKokoro([text, tempWav]);
    if (!r.ok || !fs.existsSync(tempWav)) {
      log(`synthesize failed: code=${r.code} err=${(r.err || '').slice(0, 200)}`);
      return sendJson(res, 500, { ok: false, error: r.err || `exit ${r.code}` });
    }
    ...
    try {
      ...
      log(`synthesize file read failed: ${err.message}`);
