...
function extractOutput(towerResult) {
  ...
  if (towerResult.async === true && towerResult.workflowId) {
    return `[Orchestrator queued async workflow ${towerResult.workflowId}] status=${towerResult.status || 'pending'}; streamId=${towerResult.streamId || 'n/a'}. Poll ${towerResult.statusUrl || `/api/orchestrate/${towerResult.workflowId}`} for completion.`;
  }
  return (
    towerResult.output
    || towerResult.result
