 ...
 *   - lib/llm-provider          -> provider-agnostic LLM calls (.chat / .swarm)
 *   - lib/job-contract          -> classifyJob + createJobContract + gates
 *   - Tower :7790 /api/spawn    -> real agent dispatch
 *   - Orchestrator :7784        -> workflow alternative path
 *   - State Store :7783         -> persistence
 *   - EventBus :7782            -> optional progress publishing
 *
 * Failure modes never kill a job:
 *   - LLM unreachable          -> deterministic fallback planner/reviewer/synthesizer
 *   - Tower/orchestrator unreachable -> fail loudly with a blocker
 *   - State Store unreachable  -> in-memory only, archived to file on shutdown
 *   - Karen unavailable        -> built-in escalation rules
 *
 * Public API:
...
async function isOnline(port, healthPath = '/health') {
  // 1500ms was too tight: under heavy load (100% CPU, many live agents) the
  // tower/orchestrator probe is slower than that, so the harness wrongly
  // concluded "offline" and refused to dispatch even while agents were running.
  // Generous timeout + one retry kills the false-negative.
  for (let attempt = 0; attempt < 2; attempt++) {
    try { await getJSON(port, healthPath, 6000); return true; }
...
function fallbackPlan(goal) {
  ...
  const tag = trimmed.length > 96 ? `${trimmed.slice(0, 96)}- '    ` : trimmed;
  return [
...
async function decomposeGoal(goal, pastContext = '') {
  const system = `You are PURPCLAW's harness planner. Break the operator goal into 3-  --   7 concrete, independently-verifiable subtasks. Each subtask must produce a tangible artifact, decision, or piece of information that a swarm agent (penguin/dragon/wolf/owl/karen/etc.) can deliver.
...
async function escalateToKaren(job, subtask, history) {
  ...
  const system = `You are KAREN -  --    PURPCLAW's escalation specialist. A subtask has failed multiple times. Choose ONE action and explain crisply (no fluff, no apologies, structured like an incident report).
...
LAST_DELIVERABLE (first 800 chars): ${(history.at(-1)?.output || '').slice(0, 800)}`;
  ...
  try {
    ...
    // Karen-LLM unavailable -> built-in escalation policy
...
function fallbackKaren(subtask, history) {
  ...
  if (allChallenged && attempts <= 3) {
    return {
      action: 'retry',
      reason: `Subtask has been challenged ${attempts}- '   consecutively. One more refined attempt warranted before re-routing.`,
  ...
  if (subtask.contract?.preferredAgents?.length > 1) {
    return {
      action: 'reroute',
      reason: 'Rotating to alternate preferred agent - current agent has stalled.',
  ...
  return {
    ...
    ticketSummary: `${subtask.description} -  --    failed after ${attempts} attempts`
  };
}
...
function fallbackSynthesis(job) {
  ...
  if (accepted.length === 0) return 'No accepted deliverables - nothing to synthesise.';
...
async function synthesizeReport(job) {
  const acceptedOutputs = job.plan
    ...
    .join('\n\n');
  if (!acceptedOutputs) return 'No accepted deliverables - nothing to synthesise.';
...
async function rankPreferredAgents(intent, preferredAgents = []) {
  ...
  // Phantom agent names that leak in from failed-workflow score records
  // (every `|| 'unknown'` default) and stale planner output. If any of these
  // reach the tower it throws "Unknown agent: <x>" and the subtask returns an
  // empty deliverable '   so they must never survive routing.
  const DENY = new Set(['unknown', 'agent', 'numbers', 'chart', 'none', '']);

  const valid = (name) => {
    if (!name || DENY.has(name)) return false;
    if (registered && registered.size) return registered.has(name);
    return base.includes(name);
  };

  // NOTE: every term is filtered through valid() '   the previous version
  // appended `...base` unfiltered, which let 'unknown' through.
...
async function dispatchToTower(subtask, contract) {
  ...
  //
  // The tower's /api/spawn endpoint (agent_tower.js:962) expects a payload
  // shaped as { agentName, task, options }. The previous version of this
  // function used { agent, task, contract, source, intent } '   the tower
  // destructured agentName and got undefined '   400 'agentName and task
  // required' '   every dispatched subtask came back with a validation
  // error instead of real work product. Aligned to the tower's contract.
  ...
  try {
    if (await isOnline(PORTS.orchestrator, '/api/health')) {
      const result = await postJSON(PORTS.orchestrator, '/api/orchestrate/await', payload, 120_000);
...
function extractOutput(towerResult) {
  ...
  // Orchestrator returns an async workflow ack instead of a deliverable '  
  // { workflowId, status: 'pending', streamId, async: true }. The previous
  // fallback path leaked the whole metadata blob as the "deliverable" because
  // none of the .output/.result/etc. fields existed. Surface it as a clear
  // queued-workflow message instead so the gates don't have to grade metadata.
  if (towerResult.async === true && towerResult.workflowId) {
    return `[Orchestrator queued async workflow $towerResult.workflowId}] status=$towerResult.status || 'pending'}. Poll $towerResult.statusUrl || '/api/orchestrate/' + towerResult.workflowId} for completion.`;
  }
  return (
    towerResult.output
    || towerResult.result
...
async function rememberSubtaskLesson(job, subtask, success) {
  ...
  if (canonicalMemorySync) {
    try {
      const synced = await canonicalMemorySync.syncRecord(PURP_ROOT, 'harness-lessons', lesson);
      if (synced?.memoryId) lesson.memoryId = synced.memoryId;
      lesson.memorySync = synced?.imported ? 'canonical' : synced?.skipped ? 'already-synced' : 'failed';
    } catch {
      lesson.memorySync = 'failed';
    }
...
class HarnessEngine extends EventEmitter {
  ...
  async run(goal) {
    ...
    try {
      ...
      try {
        ...
        this.log('warn', `Planner LLM failed ($e.message}) -  --    using deterministic fallback plan`);
      ...
      for (const subtask of this.job.plan) {
        ...
        if (this.job.iteration >= this.job.maxIterations) {
          this.log('warn', `Iteration ceiling ($this.job.maxIterations}) reached -  --    halting`);
          break;
        }
      ...
      try {
        ...
        if (fish) {
          try {
            ...
            if (!fishSynthesisAudit.releaseReady) {
              this.log('warn', `-        Fish: synthesis $fishSynthesisAudit.overallVerdict} -  --    $fishSynthesisAudit.slapLog.length} correction(s) needed`);
              // Downgrade certainty in the synthesis header
              const header = `> **Accuracy Fish audit:** $fishSynthesisAudit.overallCertainty} -  --    $fishSynthesisAudit.slapLog.length} claim(s) need correction\n\n`;
            ...
            this.log('warn', `Fish synthesis audit failed: $e.message} -  --    proceeding without fish`);
        ...
        this.log('warn', `Synthesiser LLM failed ($e.message}) -  --    using deterministic fallback`);
  ...
  async runSubtask(subtask) {
    ...
    this.log('info', `- '     Subtask #$subtask.index + 1}: $subtask.description}`);
    ...
    while (subtask.attempts <= maxRetries) {
      ...
      if ((subtask.output || '').includes('[BLOCKED:')) {
        ...
        this.log('warn', `Subtask #$subtask.index + 1} blocked -  --    operator input required`);
      ...
      try {
        ...
        this.log('warn', `Reviewer LLM failed ($e.message}) -  --    using fallback review`);
      ...
      this.log('verdict', `Subtask #$subtask.index + 1} -> $review.verdict}: $review.reason}`);
      this.emitTrace('mirrorvale', `subtask.$review.verdict.toLowerCase()}`, review.reason.slice(0, 80), subtask.id);

      if (review.verdict === 'ACCEPTED') {
        // -  --   -  --    Fish: check subtask output before marking accepted -  --   -  --   -  --   -  --   -  --   -  --   -  --   -  --   -  --   -  --   -  --   -  --   -  --   -  --   -  --   -  --   
        // Even if the judge accepted it, fish might find unsupported claims.
        // HARD_SLAP / RED_SLAP / TANK_MODE -> override ACCEPTED to CHALLENGED.
        ...
        if (fish && subtask.output) {
          try {
            ...
            // RED_SLAP or TANK_MODE -> escalate to CHALLENGED
            if ([fish.VERDICT.RED_SLAP, fish.VERDICT.TANK_MODE].includes(fishResult.overallVerdict)) {
              ...
              this.job.scratchpad.push(`-        FISH OVERRIDE subtask #$subtask.index + 1}: $fishResult.overallVerdict} -  --    $correction.message}`);
              this.log('warn', `-        Fish overrode ACCEPTED -> CHALLENGED on subtask #$subtask.index + 1} ($fishResult.overallVerdict})`);
              review.verdict = 'CHALLENGED';
              review.reason = `Fish: $fishResult.overallVerdict} -  --    $fishResult.slapLog[0]?.reason || 'unsupported claim'}`;
              ...
              // HARD_SLAP -> log but keep ACCEPTED; surface in final report
              this.log('warn', `-        Fish HARD_SLAP on subtask #$subtask.index + 1} -  --    accepted but claim issues noted`);
            }
          } catch (e) {
      ...
      // CHALLENGED -> either retry, or escalate to Karen if we've challenged too many times
      ...
      if (challengedCount >= this.options.karenEscalateAfterFailures) {
        ...
        this.log('warn', `KAREN[$subtask.id}] action=$decision.action} -  --    $decision.reason}`);
        ...
        if (decision.action === 'reroute' && decision.newPreferredAgents) {
          ...
          this.job.scratchpad.push(`KAREN reroute #$subtask.index + 1}: agents -> $decision.newPreferredAgents.join(', ')}`);
      ...
      this.log('warn', `Subtask #$subtask.index + 1} challenged -  --    refining (attempt $subtask.attempts + 1}/$maxRetries + 1})`);
    }
  ...
  async finalise(finalState) {
    ...
    this.job.finishedAt = now();
    this.log('info', `Harness complete -  --    state=$finalState}`);
