...
const smith = {
  ...
  inject(technique, target = {}) {
    ...
    try {
      const corrupted = t.apply(target);
...
const neo = {
  ...
  detect(output = {}) {
    ...
    // Reordered code — check for syntax errors after reordering
    if (text.includes('var ') && text.includes('function') && /\b(const|let|var)\s+\w+\s*=\s*\w+\s*\(/.test(text)) {
      // Basic heuristic for code-like output
    }
    // Null/empty
