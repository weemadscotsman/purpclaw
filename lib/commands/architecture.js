...
async function run(args, ctx) {
  ...
  const services = {
    'CORE': [
      ...
      ['3000', 'mission-control', 'Next.js web UI (App Router)'],
    ],
    'COGNITIVE (Python)': [
      ...
      ['7895', 'autodream',       'Memory consolidation cycles after workflows'],
    ],
    'MEDIA + I/O (Python)': [
  ...
  ${col(C.cyan, '5.')} CLI streams events back to the user terminal
       └─→ Mission Control UI (:3000) also subscribes for live dashboard

  ${col(C.cyan, '6.')} On completion
       ├─→ Score updated in agent_score.json
       ├─→ AutoDream (:7895) may trigger memory consolidation
