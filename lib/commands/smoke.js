...
async function run(args, ctx) {
  ...
  try {
    ...
    const raw = execSync(`${pm2Bin} pm2 jlist`, { cwd: PURP_DIR, encoding: 'utf8', windowsHide: true, stdio: ['ignore', 'pipe', 'ignore'], timeout: 6000 });
  ...
  try {
    ...
    const ing = await httpPost(7880, '/ingest', { content: `synthetic smoke memory tag=${tag}`, source: 'purpclaw-smoke' }, 5000);
    const recall = await httpPost(7880, '/recall', { query: tag, limit: 3 }, 5000);
  ...
  if (!quick) {
    ...
    try {
      ...
      const ok = r.code === 200 && (r.body?.workflowId || r.body?.success);
  ...
  try {
    ...
    const sample = 'LLM_API_KEY=sk-test-1234567890abcdef1234567890abcdef';
