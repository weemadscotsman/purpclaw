...
async function cmdDeploy() {
  ...
  await execSafe('ssh', [
    ...
    '-i', cfg.keyPath,
    `${cfg.user}@${cfg.host}`,
    `docker load -i /tmp/purpclaw.tar && docker run -d --name purpclaw --restart unless-stopped -p 3000:3000 -p 7780:7780 ${tag}`
  ], { timeoutMs: 60_000 });
  ...
  if (health.ok && health.stdout?.trim() === '200') {
    ...
    console.log(`  🌐 http://${cfg.host}:3000`);
  } else {
...
async function cmdStatus() {
  ...
  const result = await execSafe('ssh', [
    ...
    '-i', cfg.keyPath,
    `${cfg.user}@${cfg.host}`,
    `docker ps --filter name=purpclaw --format '{{.Status}}' && echo '---' && curl -s -o /dev/null -w 'Web: HTTP %{http_code}' http://localhost:3000/ && echo '' && curl -s -o /dev/null -w 'API: HTTP %{http_code}' http://localhost:7780/api/health`
  ], { timeoutMs: 15_000 });
