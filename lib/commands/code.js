...
async function searchSemantic(query, topK = 5) {
  ...
  try { qvec = (await embedBatch([query]))[0]; }
    ...
    // Ollama offline at query time — fallback to keyword
    return searchKeyword(query, topK, idx);
  }

  // Build a per-record score via a fast inlined dot product. Avoid the
  // .embedding getter (which allocates a view) by using the contiguous
  ...
  return { results: topVecs.map(v => ({ score: dotOne(v), file: v.file, content: v.content })) };
}
