...
const COMMANDS = {
  ...
  chaos:     { run: async (args) => {
    ...
    if (sub === 'reset') return sn.neo.reset().message;
    return 'usage: chaos inject <technique> | random | ledger | round | reset';
  }, desc: 'Smith-Neo chaos engine: chaos inject <tech> | random | ledger | round | reset' },
  help:       { run: async () => {
