...
async function getStackContext(PURP_DIR) {
  ...
  lines.push('- `purpclaw smoke` — 13-check end-to-end self-test (CI-ready, exits 1 on failure)');
  lines.push('- `purpclaw safe-start --core` — wake the 16-service stable baseline (one at a time, cascade-safe)');
  lines.push('- `purpclaw safe-start --dark` — wake the defined-but-dark cluster safely');
  lines.push('- `purpclaw safe-stop --dark` — put the dark cluster back to sleep');
  ...
  lines.push('- If a service is offline, recommend `purpclaw safe-start <name>` — NOT raw `pm2 start`. On Windows, starting multiple services at once can trigger a cmd-window spawn cascade. safe-start launches one-at-a-time with a stabilisation watch and a circuit breaker.');
