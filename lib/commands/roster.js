...
async function run(args, ctx) {
  ...
  if (canonical && Array.isArray(canonical.agents)) {
    ...
    const profileBacked = agents.filter(a => (a.sources || []).includes('agent_profiles.json'));
    const towerBacked = agents.filter(a => (a.sources || []).includes('agent_tower.js'));
    const missingTower = profileBacked.filter(a => !(a.sources || []).includes('agent_tower.js'));
    const towerOnly = towerBacked.filter(a => !(a.sources || []).includes('agent_profiles.json'));
    const personaOrphans = agents.filter(a =>
      a.type === 'persona' &&
      !(a.sources || []).some(s => s === 'agent_tower.js' || s === 'agent_profiles.json')
    );
    ...
    if (wantJson) {
      console.log(JSON.stringify({
        ...
        missing_tower_binding: missingTower.map(a => a.key || a.name),
        tower_only: towerOnly.map(a => a.key || a.name),
    ...
    console.log(col(C.gray, `  Profile without tower: ${col(missingTower.length ? C.yellow : C.green, missingTower.length)}`));
    console.log(col(C.gray, `  Tower without profile: ${col(towerOnly.length ? C.yellow : C.green, towerOnly.length)}\n`));

    if (wantMissing) {
      for (const a of missingTower) console.log(`  ${col(C.cyan, String(a.key || a.name).padEnd(18))} ${col(C.gray, `${a.division || ''} ${a.role || ''}`)}`);
      return;
    }
    if (wantOrphans) {
