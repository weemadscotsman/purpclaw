...
async function run(args, ctx) {
  ...
  if (useDark) {
    ...
    names = getEcosystemNames(PURP_DIR);
    ...
    console.log(`    purpclaw safe-start --all                start the entire ecosystem`);
    ...
    console.log(`  ${col(C.cyan, 'Defined-but-dark services:')}  ${col(C.yellow, DARK_SERVICES.join(', '))}`);
    console.log(`  ${col(C.gray, 'These are off by default. Use --dark to wake them safely.')}\n`);
