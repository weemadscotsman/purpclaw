...
function wakePm2Daemon(PURP_DIR) {
  // `pm2 jlist` can fail on a first install before the daemon exists. `pm2 ping`
  // starts the daemon if needed, then jlist works normally.
  try {
    spawnSync(pm2Cmd(), pm2Args(['ping']), {
      cwd: PURP_DIR,
      windowsHide: true,
      encoding: 'utf8',
...
async function run(args, ctx) {
  ...
  let state = runPm2Json(PURP_DIR);
  if (!state) state = wakePm2Daemon(PURP_DIR);
  if (!state) {
