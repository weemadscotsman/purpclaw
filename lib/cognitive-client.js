...
async function reportEvent(evt) {
  try {
    return await httpPost(PORTS.diagnostics, '/event', {
...
async function diagnose(context = {}) {
  ...
  try {
    ...
    const [findings, vote] = await Promise.all([
      httpGet(PORTS.diagnostics, '/findings').catch(() => null),
      httpGet(PORTS.diagnostics, '/vote').catch(() => null),
    ]);
...
async function diagnoseAgent(agentName) {
  if (!(await isOnline('diagnostics'))) return null;
  try {
    return await httpPost(PORTS.diagnostics, `/diagnose/${agentName}`, {});
  } catch { return null; }
}
...
async function assertFact(predicate, terms, prov = 'orchestrator') {
  try {
    return await httpPost(PORTS.rules, '/assert', {
...
async function retractFact(predicate, terms) {
  try {
    return await httpPost(PORTS.rules, '/retract', { predicate, terms });
  } catch { return null; }
}
...
async function queryFacts(predicate, terms = []) {
  try {
    const result = await httpPost(PORTS.rules, '/query', {
...
async function checkConstraint(predicate, terms) {
  try {
    return await httpPost(PORTS.rules, '/check', { predicate, terms });
  } catch { return null; }
}
...
async function addRule(head, body) {
  try {
    return await httpPost(PORTS.rules, '/rule', { head, body });
  } catch { return null; }
}
...
// â”€â”€ NEURO-SYMBOLIC BRIDGE (port 7884) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

async function getNeuroStats() {
  try {
    return await httpGet(PORTS.neuro, '/stats');
  } catch { return null; }
}
...
async function liftPattern(pattern, source = 'purpclaw', confidence = 0.9, metadata = {}) {
  ...
  try {
    return await httpPost(PORTS.neuro, '/lift/pattern', payload);
  } catch (firstError) {
...
async function queryNeuro(factType = 'pattern_detected', limit = 10) {
  const q = `/query?fact_type=${encodeURIComponent(factType)}&limit=${encodeURIComponent(limit)}`;
  try {
