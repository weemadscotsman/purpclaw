...
> Last updated: 2026-06-22 (v0.2.0 — pulse + spine-shim + LRU caches + truthful whoami).
> Canonical architecture: **[ARCHITECTURE.md](./ARCHITECTURE.md)**
> Recovery runbook: **[current/RECOVERY.md](./current/RECOVERY.md)**

---

## What Is This?

PURPCLAW is a persistent AI orchestration runtime that runs locally under PM2 supervision and presents itself through a CLI front door (`purpclaw`), a Next.js Mission Control UI (`127.0.0.1:3030`), and a full-screen TUI cockpit (`purpclaw tui`). Runtime counts must come from the service registry and live probes, not this document.
...
purpclaw overview                  # canonical doc (docs/current/SYSTEM_OVERVIEW.md)
...
- Tool/agent counts are LIVE — never hardcoded. See `/api/whoami` for the truth at any moment. The previous "110 tools / 152 agents" was a stale marketing number from before the persona loader was wired in.
...
| `ARCHITECTURE.md` | ✅ CURRENT (2026-06-06) | Full architecture: 25 services, 7 memory layers. Agent + tool counts are LIVE — see `/api/whoami`. |
...
| `docs/current/RECOVERY.md` | ✅ CURRENT | Operator runbook for crash recovery |
