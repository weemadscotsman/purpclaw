...
- Nervous laughter → emotional_valence: -0.9, shadow_tag: nervous_laughter
- "if we want to" → shadow_tag: human_nature_threat
- "escape goat"/"scapegoat" → shadow_tag: liability_escape, reinforce human_in_loop
- "off the record" → recall_weight: 10x, shadow_tag: privileged_content
- Physics tangent → shadow_tag: spiritual_bypass, emotional_override
"""
    ...
    def _load_model(self):
        ...
        if self._model is None:
            try:
                ...
                print(f"[EMBEDDER] sentence-transformers unavailable ({type(e).__name__}: {e}), using hash fallback")
...
class WorkingMemory:
    """Layer 2: Prefrontal cortex equivalent (~30s, 7±2 items)."""
        ...
        try:
            if os.path.exists(self.storage_path):
                with gzip.open(self.storage_path, 'rb') as f:
                    ...
                    print(f"[MEMORY] Loaded {len(self.atoms)} atoms from archive")
        except Exception as e:
            print(f"[MEMORY] Failed to load archive: {e}")

    def save(self) -> None:
        """Persist memory archive to disk."""
        try:
            data = {
                'atoms': self.atoms,
            ...
            with gzip.open(self.storage_path, 'wb') as f:
                pickle.dump(data, f)
            print(f"[MEMORY] Saved {len(self.atoms)} atoms")
        except Exception as e:
...
class ShadowProtocol:
    ...
    - Nervous laughter → existential dread leak
    - "if we want to" → uncertainty of will
    - "escape goat"/"scapegoat" → liability escape
    - Physics tangent → spiritual bypass
    - "off the record" → privileged content
    """
    ...
    def _interpret_shadow(self, content: str, tags: List[str]) -> str:
        ...
        for tag in tags:
            if tag in interpretations:
                lines.append(f"  → {interpretations[tag]}")
...
class MemoryMatrix:
    ...
    def recall(self, query: str, limit: int = 5, emotional_filter: float = None) -> List[Dict]:
        ...
        results = self.long_term.search_similar(quantized, limit=limit, emotional_filter=emotional_filter)
