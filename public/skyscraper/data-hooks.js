/* data-hooks.js — REAL data only, no mocks.
...
async function tryFetchJson(url, timeoutMs = 2500, init = {}) {
  try {
    const r = await fetch(url, { ...init, signal: withTimeout(timeoutMs) });
    if (!r.ok) return null;
...
async function tryProxy(port, path, timeoutMs = 2500) {
  ...
  if (p) return p.data !== undefined ? p.data : p;
  // direct (requires CORS on backend)
  const d = await tryFetchJson(`http://localhost:${port}${path}`, timeoutMs);
  return d ? (d.data !== undefined ? d.data : d) : null;
}

async function tryProxySend(port, path, body = {}, timeoutMs = 8000, method = 'POST') {
  const init = {
    method,
  ...
  const proxyUrl = `/api/service-proxy?port=${port}&path=${encodeURIComponent(path)}`;
  const p = await tryFetchJson(proxyUrl, timeoutMs, init);
  if (p) return p.data !== undefined ? p.data : p;
  const d = await tryFetchJson(`http://localhost:${port}${path}`, timeoutMs, init);
  return d ? (d.data !== undefined ? d.data : d) : null;
}
...
// known services to health-check
const SERVICE_LIST = [
  { name: 'unified_api',    port: 7780, path: '/api/health',    key: 'api' },
  { name: 'eventbus',       port: 7782, path: '/state',         key: 'eventbus' },
  { name: 'state_store',    port: 7783, path: '/health',        key: 'state' },
  { name: 'orchestrator',   port: 7784, path: '/api/pipeline',  key: 'orchestrator' },
  { name: 'modal_logic',    port: 7785, path: '/health',        key: 'modal',       optional: true },
  { name: 'diagnostics',    port: 7786, path: '/diagnose',      key: 'diagnostics', optional: true },
  { name: 'rules_engine',   port: 7787, path: '/health',        key: 'rules',       optional: true },
  { name: 'agent_tower',    port: 7790, path: '/tower/status',  key: 'tower' },
  { name: 'gatekeeper',     port: 7791, path: '/api/status',    key: 'gatekeeper' },
  { name: 'memory_matrix',  port: 7880, path: '/health',        key: 'memory',      optional: true },
  { name: 'neuro_bridge',   port: 7884, path: '/health',        key: 'bridge',      optional: true },
  { name: 'vision_monitor', port: 7889, path: '/health',        key: 'vision',      optional: true },
  { name: 'autodream',      port: 7895, path: '/dream/status',  key: 'autodream',   optional: true },
  { name: 'swarm_coord',    port: 7898, path: '/status',        key: 'swarm',       optional: true },
];
...
function DataProvider({ children }) {
  ...
  const pipeline = mission.data?.pipeline || { active: [], completed: [], queue: { items: [] } };
...
Object.assign(window, {
  ...
  buildFloors, tryProxy, tryProxySend, tryFetchJson,
});
