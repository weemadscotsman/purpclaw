...
const CONTROL_API_PORT = parseInt(process.env.CONTROL_API_PORT) || 7780;
const KOKORO = process.env.KOKORO_BAT || 'C:\\Users\\Admin\\.openclaw\\kokoro_send.bat';
const KOKORO_LONG = process.env.KOKORO_LONG_BAT || 'C:\\Users\\Admin\\.openclaw\\kokoro_long_send.bat';
...
function getBackoffDelay(attempts) {
  const delay = BASE_RECONNECT_MS * Math.pow(2, attempts);
...
function connectToControlAPI() {
  ...
  controlApiSocket.on('connect', () => {
    ...
    socketReady = true;
    controlApiAttempts = 0;  // reset backoff on success
  ...
  controlApiSocket.on('close', () => {
    ...
    console.log(`[bridge] Reconnecting in ${delay}ms (attempt ${controlApiAttempts})`);
